...
**一句话要点**：提出MIRA-U半监督框架，结合不确定性伪标签和混合CNN-Transformer网络，以解决皮肤病变分割中标注数据不足的问题。

**关键词**：皮肤病变分割, 半监督学习, CNN-Transformer混合网络, 不确定性伪标签, 师生模型, 医学图像分析

## 3 点简述
- 核心问题：皮肤病变自动分割因标注数据有限而具挑战性，影响早期皮肤癌检测。
- 方法要点：采用不确定性感知师生伪标签和U形CNN-Transformer架构，提升伪标签质量和边界分割。
- 实验或效果：在ISIC-2016和PH2数据集上，仅用50%标注数据，DSC达0.9153，IoU达0.8552。

...
**关键词**：联邦学习, 隐私保护, 边缘计算, 梯度压缩, 同态加密, 计算机视觉

## 3 点简述
- 核心问题：联邦学习参数共享仍可能导致敏感数据泄露，现有方法多假设客户端非恶意。
- 方法要点：评估同态加密、梯度压缩、梯度噪声等方法，探讨分割网络等模型的数据重建难度。
- 实验或效果：在NVIDIA Jetson TX2模块上实现概念验证，分析梯度压缩和噪声对卷积神经网络准确性的负面影响。

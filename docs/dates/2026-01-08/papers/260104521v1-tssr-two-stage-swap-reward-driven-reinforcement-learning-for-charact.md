...
**一句话要点**：提出TSSR两阶段强化学习框架，通过交换奖励驱动字符级SMILES生成，以解决分子生成中的语法和化学有效性问题。

**关键词**：分子生成, 强化学习, SMILES字符串, 化学有效性, 语法修复, 奖励设计

## 3 点简述
- 核心问题：当前SMILES生成模型易受令牌错误累积影响，导致不可解析或化学不可行分子，且硬约束限制探索。
- 方法要点：TSSR采用两阶段强化学习，第一阶段奖励局部令牌交换修复语法，第二阶段基于RDKit诊断提供化学感知反馈。
- 实验或效果：在MOSES基准测试中，TSSR显著提升语法和化学有效性及新颖性，同时保持多样性和药物相似性。

...
**一句话要点**：提出链式视图提示以解决3D环境中视觉语言模型空间推理受限的问题

**关键词**：3D环境问答, 空间推理, 视图选择, 测试时推理, 视觉语言模型

## 3 点简述
- 核心问题：现有视觉语言模型输入视图固定，难以获取多视角上下文，阻碍复杂空间推理。
- 方法要点：通过粗到细探索过程，使用视图选择代理和迭代推理调整视图，无需额外训练。
- 实验或效果：在OpenEQA等基准上平均提升11.56%，最高达13.62%，并展示测试时扩展性。

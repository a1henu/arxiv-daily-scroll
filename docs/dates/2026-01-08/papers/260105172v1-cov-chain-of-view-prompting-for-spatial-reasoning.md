...
**一句话要点**：提出链式视图提示以解决3D环境中空间推理的视图限制问题

**关键词**：3D问答, 空间推理, 视图选择, 测试时推理, 视觉语言模型

## 3 点简述
- 核心问题：现有视觉语言模型在3D问答中受限于固定输入视图，难以获取分散或遮挡的上下文。
- 方法要点：通过训练无关的粗到细探索过程，结合视图选择与相机动作迭代，实现主动视图推理。
- 实验或效果：在OpenEQA等基准上平均提升11.56%，并展示测试时扩展性，最高增益达13.62%。

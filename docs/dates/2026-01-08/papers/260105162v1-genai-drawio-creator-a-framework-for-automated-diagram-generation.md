...
**关键词**：图表生成, 大型语言模型, 结构化视觉推理, 自动化工具, XML格式处理

## 3 点简述
- 核心问题：图表创建和修改是劳动密集型任务，需要自动化解决方案。
- 方法要点：集成Claude 3.7进行结构化视觉推理，通过提示工程和错误检查生成有效XML。
- 实验或效果：原型能根据自然语言或代码生成准确图表，模拟评估显示显著减少创建时间并提高结构保真度。

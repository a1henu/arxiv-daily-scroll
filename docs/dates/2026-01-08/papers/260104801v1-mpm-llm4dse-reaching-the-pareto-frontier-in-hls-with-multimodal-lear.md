...
**关键词**：高层次综合, 设计空间探索, 多模态学习, 大语言模型优化, 预测模型, 提示工程

## 3 点简述
- 核心问题：现有GNN预测方法未能充分捕捉行为描述语义，且多目标优化缺乏对pragma指令影响的显式建模。
- 方法要点：结合行为描述与流图的多模态预测模型，以及基于LLM的优化器，通过提示工程指导配置生成。
- 实验或效果：多模态预测模型性能提升达10.25倍，LLM4DSE在DSE任务中平均增益39.90%。

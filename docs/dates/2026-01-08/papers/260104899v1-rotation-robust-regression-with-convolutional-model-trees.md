...
**一句话要点**：提出卷积模型树结合几何感知偏置与方向搜索，以增强图像回归任务在平面旋转下的鲁棒性。

**关键词**：旋转鲁棒学习, 卷积模型树, 几何感知偏置, 方向搜索, 图像回归, MNIST数据集

## 3 点简述
- 研究图像输入在平面旋转下的鲁棒学习问题，使用卷积模型树作为基础模型。
- 引入卷积平滑、倾斜主导约束和重要性剪枝等几何感知偏置，量化其对旋转鲁棒性的影响。
- 评估部署时方向搜索策略，通过选择离散旋转提升鲁棒性，但可能因置信度与正确性错配而有害。

...
**一句话要点**：提出AECV-Bench基准，评估多模态模型在建筑与工程图纸理解中的性能

**关键词**：多模态模型评估, 建筑图纸理解, 对象计数, 文档问答, 空间推理, 符号识别

## 3 点简述
- 核心问题：现代多模态模型能否可靠解释建筑图纸的图形语言，包括符号、布局和注释。
- 方法要点：通过对象计数和图纸文档问答两个用例，评估模型在OCR、空间推理和符号理解等方面的能力。
- 实验或效果：模型在OCR和文本相关任务表现强，但符号理解如门窗计数准确率低，显示领域特定需求。

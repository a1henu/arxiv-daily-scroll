...
**一句话要点**：提出RelayLLM框架，通过令牌级协作解码实现高效推理，解决大模型计算成本高与小模型推理能力不足的问题。

**关键词**：令牌级协作解码, 高效推理, 大语言模型, 小语言模型, 两阶段训练, 成本优化

## 3 点简述
- 核心问题：大语言模型推理成本高、延迟大，小语言模型推理能力不足，现有协作方法粗粒度卸载导致计算浪费。
- 方法要点：引入令牌级协作解码，让小模型作为主动控制器，动态调用大模型处理关键令牌，采用两阶段训练框架优化策略。
- 实验或效果：在六个基准测试中平均准确率达49.52%，仅调用大模型处理1.07%的令牌，成本降低98.2%。

...
**一句话要点**：提出超额描述长度框架，量化微调从训练数据中提取预测结构的信息量。

**关键词**：信息论框架, 微调量化, 预测结构, 超额描述长度, 模型泛化

## 3 点简述
- 核心问题：微调是激发潜在能力还是教授新能力，对模型评估与安全至关重要。
- 方法要点：基于信息论，通过预序编码定义EDL，衡量在线训练与最终模型的编码成本差距。
- 实验或效果：通过玩具模型澄清学习中的信息混淆，如随机标签EDL近零，罕见输入结构贡献小。

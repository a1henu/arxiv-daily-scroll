...
**一句话要点**：通过机制分析揭示视觉语言模型中提示诱导幻觉的注意力头作用，在对象计数场景中实现至少40%的幻觉减少。

**关键词**：视觉语言模型, 提示诱导幻觉, 注意力头机制, 对象计数, 模型分析, 幻觉减少

## 3 点简述
- 研究视觉语言模型在对象计数任务中因文本提示与视觉证据不符而产生的幻觉问题。
- 通过机制分析识别出少量关键注意力头，其消融能显著减少幻觉而不需额外训练。
- 实验显示消融后模型更倾向于纠正提示错误，并揭示了模型间实现幻觉行为的差异。

...
**一句话要点**：提出Miner方法，通过挖掘内在不确定性解决大型推理模型在无批评者强化学习中的数据效率问题。

**关键词**：强化学习, 推理模型, 数据效率, 不确定性挖掘, 自监督奖励, 令牌级信用分配

## 3 点简述
- 核心问题：无批评者强化学习在正同质提示下效率低下，优势估计为零导致样本浪费。
- 方法要点：利用策略内在不确定性作为自监督奖励信号，引入令牌级焦点信用分配和自适应优势校准。
- 实验或效果：在六个推理基准上评估，相比GRPO实现Pass@1提升4.58，Pass@K提升6.66，达到最先进性能。

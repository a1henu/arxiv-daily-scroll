...
## 3 点简述
- 核心问题：含未解释函数的量化公式在非线性实数算术中求解困难，传统方法缺乏语义理解，搜索空间大。
- 方法要点：通过约束分离预处理，使用结构化提示从LLM生成满足约束的函数定义候选，结合自适应实例化集成传统SMT算法。
- 实验或效果：在SMT-COMP基准测试中，AquaForte解决了Z3和CVC5等先进求解器超时的多个实例，对可满足公式尤其有效。

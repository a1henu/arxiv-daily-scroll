...
**一句话要点**：提出InstruCoT方法，通过多样数据合成与指令级思维链微调，增强LLMs抵御提示注入攻击的能力。

**关键词**：提示注入防御, 数据合成, 思维链微调, 大语言模型安全, 指令级学习

## 3 点简述
- 核心问题：提示注入攻击通过多样向量注入恶意指令，且与上下文语义边界模糊，难以识别。
- 方法要点：合成多样训练数据，采用指令级思维链微调，使LLMs能有效识别和拒绝恶意指令。
- 实验或效果：在行为偏差、隐私泄露和有害输出三个维度评估，InstruCoT显著优于基线，且保持实用性。

...
**一句话要点**：提出数据驱动地形力学方法，实现月球车高保真实时仿真器

**关键词**：月球车仿真, 数据驱动地形力学, 实时仿真, 回归模型, 地形交互, 视觉保真度

## 3 点简述
- 核心问题：现有仿真器在视觉真实性与物理准确性间存在局限，难以全面复现月球表面条件。
- 方法要点：采用数据驱动方法，基于全车和单轮实验数据构建回归模型，预测滑移和沉陷行为。
- 实验或效果：模型在平坦地形和20度斜坡上准确模拟稳态与动态滑移及沉陷，并通过现场测试验证。

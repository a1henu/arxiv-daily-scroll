...
**关键词**：图神经网络训练, 多GPU并行, 异步梯度更新, 流水线架构, 邻居采样优化, 资源利用率提升

## 3 点简述
- 核心问题：图神经网络训练存在小批量生成低效、数据传输瓶颈和GPU间同步成本高，导致资源利用率不足
- 方法要点：引入RaCoM实现异步梯度共享和模型更新，结合全局邻居采样缓存和自适应队列大小策略优化资源利用
- 实验或效果：在四个大规模数据集和十个基线模型上，MQ-GNN实现最高4.6倍训练加速和30%GPU利用率提升，同时保持准确率

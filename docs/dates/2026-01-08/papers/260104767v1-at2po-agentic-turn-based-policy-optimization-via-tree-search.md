...
**一句话要点**：提出AT²PO框架，通过树搜索优化多轮智能体强化学习，解决探索、奖励分配和策略对齐问题。

**关键词**：智能体强化学习, 多轮任务, 树搜索, 策略优化, 信用分配, 探索策略

## 3 点简述
- 核心问题：多轮智能体强化学习中存在探索多样性不足、稀疏奖励分配困难和策略优化不匹配。
- 方法要点：引入轮次级树结构，结合熵引导树扩展和轮次级信用分配，并设计轮次级策略优化目标。
- 实验或效果：在七个基准测试中平均提升达1.84个百分点，消融研究验证各组件有效性。

...
**关键词**：定理证明, 大语言模型, Isabelle/HOL, 自动证明合成, 代码生成

## 3 点简述
- 核心问题：LLM如何辅助自动定理证明，以超越标准自动化工具如Sledgehammer。
- 方法要点：结合逐步证明器和高级证明规划器，集成束搜索、重排序模型、前提选择等技术。
- 实验或效果：能证明某些标准自动化无法处理的引理，但LLM在复杂算法实现上仍面临挑战。

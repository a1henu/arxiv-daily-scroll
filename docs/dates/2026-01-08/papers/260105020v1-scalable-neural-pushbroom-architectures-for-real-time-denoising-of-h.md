...
**关键词**：高光谱图像去噪, 卫星上处理, 神经推扫架构, 功耗可扩展性, 容错设计

## 3 点简述
- 核心问题：卫星上高光谱图像去噪需平衡高质量推理、低复杂度、动态功耗可扩展性和容错性。
- 方法要点：采用混合去噪器，结合因果逐行处理架构，匹配推扫传感器采集过程，降低内存需求。
- 实验或效果：在低功耗硬件上实现实时处理，去噪质量与更复杂模型竞争，支持功耗可扩展和容错设计权衡。

...
**关键词**：类别不平衡, 损失函数设计, 基数不变量, 神经网络训练, 材料科学数据集

## 3 点简述
- 类别不平衡是神经网络训练中的常见问题，多数类可能主导训练，导致分类器偏向多数类。
- 方法基于现代数学中的基数量化不变量（如幅度和扩散），通过评估度量空间的“有效多样性”来增强损失函数。
- 实验在人工不平衡数据集和真实材料科学数据集上验证，少数类性能显著提升，整体指标改善。

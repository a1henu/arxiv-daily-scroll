...
**关键词**：最后一公里路由, 图神经网络, 指针网络, 地理分区训练, 非对称旅行时间, 编码器-解码器架构

## 3 点简述
- 核心问题：最后一公里配送中，非对称旅行时间（如单行道、拥堵）使传统启发式方法难以适应。
- 方法要点：采用编码器-解码器架构，图神经网络编码器捕获站点空间关系，指针网络解码器顺序选择站点，并基于地理分区训练不同模型实例。
- 实验或效果：在亚马逊最后一公里路由挑战数据集上评估，分区训练相比通用训练减少了平均预测路径长度，且随站点数增加效果更显著。

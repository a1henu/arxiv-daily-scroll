...
**关键词**：检索增强生成, 碎片化数据, 轻量路径提取, 半结构化数据, 实时更新, 人工验证

## 3 点简述
- 核心问题：离散碎片化数据缺乏显式链接，标准检索忽略文件间关联，手动构建知识图谱不切实际
- 方法要点：采用低复杂度策略提取轻量路径，自然链接相关概念，将碎片文档转化为半结构化数据
- 实验或效果：在FinanceBench等实验中优于主流框架，精度相对提升25.2%，支持实时更新和人工验证

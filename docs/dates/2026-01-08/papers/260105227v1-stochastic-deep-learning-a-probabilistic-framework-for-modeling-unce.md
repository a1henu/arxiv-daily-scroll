...
**一句话要点**：提出SLDI框架，结合SDE与深度生成模型，以改进结构化时序数据中的不确定性量化。

**关键词**：随机微分方程, 深度生成模型, 不确定性量化, 变分自编码器, 时序数据建模, 伴随状态优化

## 3 点简述
- 核心问题：结构化时序数据中不确定性量化不足，传统方法难以处理不规则采样和复杂动态。
- 方法要点：在VAE潜在空间中嵌入Itô SDE，通过神经网络参数化漂移和扩散项，实现连续时间建模。
- 实验或效果：引入伴随状态共参数化和路径正则化损失，提升训练稳定性，扩展变分推断和生成建模。

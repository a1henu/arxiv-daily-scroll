...
**关键词**：网络安全课程设计, 大型语言模型微调, 自动化教育分析, 个性化学习路径, 行业需求对齐

## 3 点简述
- 核心问题：网络安全课程设计成本高、更新慢，导致毕业生技能与行业需求不匹配。
- 方法要点：采用两阶段LLM框架，包括数据预处理和课程内容分类，基于BERT模型微调实现自动化课程分析。
- 实验或效果：通过专家验证，证明CurricuLLM能高效替代人工课程分析，支持个性化课程设计并与职业角色对齐。

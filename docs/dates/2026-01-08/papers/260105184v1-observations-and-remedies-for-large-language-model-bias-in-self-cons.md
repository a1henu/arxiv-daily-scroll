...
**一句话要点**：提出自我消耗表演循环概念，研究合成数据在动态迭代训练中如何塑造偏见，并设计奖励拒绝采样策略缓解偏见。

**关键词**：自我消耗循环, 合成数据训练, 偏见演化, 表演反馈, 奖励拒绝采样, 大语言模型

## 3 点简述
- 核心问题：大语言模型在自我消耗循环中，使用合成数据训练可能导致性能下降和偏见演化。
- 方法要点：引入SCPL概念，在受控表演反馈下分析偏见演化，包括典型重训练和增量微调设置。
- 实验或效果：在三个真实任务上实验，发现循环增加偏好偏见、减少差异偏见，奖励拒绝采样策略有效缓解偏见。

...
**一句话要点**：提出自消耗表演循环概念，研究合成数据在动态迭代训练中如何塑造偏见，并设计奖励拒绝采样策略以缓解偏见。

**关键词**：自消耗表演循环, 合成数据训练, 偏见演化, 动态迭代训练, 奖励拒绝采样, 大语言模型偏见

## 3 点简述
- 核心问题：大语言模型在自消耗循环中，使用合成数据训练可能导致性能下降和偏见演化。
- 方法要点：引入自消耗表演循环概念，在受控表演反馈下分析偏见动态，包括典型重训练和增量微调设置。

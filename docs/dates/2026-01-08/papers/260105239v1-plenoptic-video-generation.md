...
**关键词**：多视角视频重渲染, 时空一致性, 自回归模型, 相机引导检索, 长视频生成, 机器人视觉

## 3 点简述
- 核心问题：现有单视角视频重渲染方法在多视角场景下难以保持时空一致性，生成区域易出现不一致。
- 方法要点：采用自回归训练多输入单输出视频条件模型，结合相机引导视频检索策略，以同步生成幻觉并维持时空记忆。
- 实验或效果：在Basic和Agibot基准测试中实现最先进的视频重渲染，提供优越的视角同步、高保真视觉和准确相机控制。

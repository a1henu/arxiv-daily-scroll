...
**一句话要点**：提出基于图神经网络与递归预热启动的近似k着色算法，提升大规模图着色性能。

**关键词**：近似k着色, 图神经网络, 递归预热启动, 局部搜索, 组合优化, 图着色算法

## 3 点简述
- 研究近似k着色问题，即用最多k种颜色为图节点着色，最小化同色边数量。
- 优化图神经网络方法，引入正交节点特征初始化和基于度的损失函数，并结合递归预热启动的局部搜索。
- 实验表明，局部搜索在小图上表现最佳，图神经网络在大规模图上性能更优，递归预热启动可独立用于组合优化。

...
**一句话要点**：提出Tape基准以评估强化学习在规则转移下的分布外泛化能力

**关键词**：强化学习基准, 分布外泛化, 元胞自动机, 规则转移, 统计评估, 不确定性分析

## 3 点简述
- 核心问题：强化学习模型在潜在规则转移时易出现分布外失败，缺乏标准化评估。
- 方法要点：基于一维元胞自动机构建基准，固定观测和动作空间，仅改变转移规则以隔离规则转移影响。
- 实验或效果：比较多种方法，发现强分布内性能方法在分布外可能崩溃，需统计报告确保评估稳定性。

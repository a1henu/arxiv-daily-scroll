...
**关键词**：视频语言模型, 反事实生成, 幻觉缓解, 直接偏好优化, 时间推理, 合成数据集

## 3 点简述
- 视频语言模型易产生动作和时间顺序幻觉，源于过度依赖语言先验而非细粒度视觉动态
- 通过多模态LLM和扩散模型生成反事实视频，构建合成数据集CounterVid，包含约26k偏好对
- 引入MixDPO方法微调模型，在时间排序任务上取得改进，并有效迁移至标准基准

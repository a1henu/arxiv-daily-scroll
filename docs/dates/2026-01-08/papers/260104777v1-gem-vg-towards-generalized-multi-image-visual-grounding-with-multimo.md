...
**关键词**：多模态大语言模型, 多图像视觉定位, 广义任务建模, 混合强化微调, 数据集构建

## 3 点简述
- 核心问题：现有多图像视觉定位方法受限于单目标定位和任务类型，缺乏对广义任务的统一建模。
- 方法要点：提出GeM-VG模型，基于跨图像线索和推理对任务分类，并引入MG-Data-240K数据集和混合强化微调策略。
- 实验或效果：在MIG-Bench和MC-Bench上分别超越先前领先模型2.0%和9.7%，单图像定位在ODINW上提升9.1%。

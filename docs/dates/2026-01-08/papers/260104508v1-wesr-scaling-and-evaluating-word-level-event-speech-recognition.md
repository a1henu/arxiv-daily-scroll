...
**一句话要点**：提出WESR以解决非语言声音事件精确定位与评估的挑战，包括细化分类和构建基准集。

**关键词**：声音事件识别, 非语言声音定位, 评估基准, 语音处理, 多模态学习

## 3 点简述
- 核心问题：现有方法在非语言声音事件定位上存在类别覆盖不足、时间粒度模糊和缺乏标准化评估框架的问题。
- 方法要点：细化21种声音事件分类，区分离散与连续类型，并构建WESR-Bench评估集，采用位置感知协议分离ASR错误。
- 实验或效果：构建1700+小时语料库训练模型，在事件检测上超越开源模型和商业API，同时保持ASR质量。

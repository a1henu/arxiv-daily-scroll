...
**一句话要点**：提出基于VLM和ALM的个性化药学长视频剪辑框架，以解决多模态内容处理效率与质量问题。

**关键词**：视频剪辑生成, 视觉语言模型, 多模态处理, 个性化机制, 药学长视频

## 3 点简述
- 核心问题：传统手动标注多模态数据（如长视频）存在不一致、低效和质量下降问题。
- 方法要点：结合音频语言模型和视觉语言模型，通过Cut & Merge算法和个性化机制生成高亮剪辑。
- 实验或效果：在公开和专有数据集上实现3-4倍加速、4倍成本降低，并提升剪辑连贯性和信息性分数。

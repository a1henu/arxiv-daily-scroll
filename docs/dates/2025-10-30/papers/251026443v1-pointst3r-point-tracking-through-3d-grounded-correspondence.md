...
**一句话要点**：提出PointSt3R方法，通过3D基础对应实现点跟踪，适应动态和静态场景。

**关键词**：点跟踪, 3D对应, 动态场景, 可见性预测, 模型微调, 合成数据训练

## 3 点简述
- 核心问题：现有3D重建模型在动态点跟踪中表现不足，需提升对应能力。
- 方法要点：结合重建损失、动态对应训练和可见性头，微调MASt3R模型。
- 实验效果：在多个数据集上实现竞争性或优越的点跟踪性能，如EgoPoints和RGB-S。

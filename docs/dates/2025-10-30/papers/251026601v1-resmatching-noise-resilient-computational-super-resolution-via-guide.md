...
**一句话要点**：提出ResMatching方法，利用引导条件流匹配实现噪声鲁棒的计算超分辨率

**关键词**：计算超分辨率, 引导条件流匹配, 噪声鲁棒性, 荧光显微镜, 数据先验学习, 不确定性估计

## 3 点简述
- 核心问题：荧光显微镜计算超分辨率是病态问题，需学习先验以推断未成像频率
- 方法要点：使用引导条件流匹配学习数据先验，提升噪声场景下的性能
- 实验或效果：在BioSR数据集上优于7个基线，平衡数据保真度与感知真实性

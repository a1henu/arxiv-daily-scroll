...
**一句话要点**：提出双条件框架结合EEG与显著图，以提升EEG驱动图像重建的保真度和语义一致性。

**关键词**：EEG图像重建, 扩散模型, 显著图引导, 神经解码, 低秩适应, 空间注意机制

## 3 点简述
- 现有EEG驱动图像重建方法忽视空间注意机制，导致保真度和语义一致性不足。
- 方法使用ATM提取EEG特征，LoRA微调Stable Diffusion 2.1，并集成ControlNet分支进行显著图条件生成。
- 在THINGS-EEG数据集上评估，显著改善图像特征质量，并与人类视觉注意高度对齐。

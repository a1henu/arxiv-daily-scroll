...
**关键词**：零样本识别, 反事实校准, 视觉语言模型, 因果推理, 偏见缓解, 表示学习

## 3 点简述
- 核心问题：视觉语言模型在零样本识别中因对象-上下文捷径而不可靠，当测试场景与训练共现不同时。
- 方法要点：在CLIP表示空间中估计对象和背景期望，合成反事实嵌入以模拟对象在不同环境中的预测。
- 实验或效果：无需重训练或提示设计，在上下文敏感基准上显著提升最差组和平均准确率。

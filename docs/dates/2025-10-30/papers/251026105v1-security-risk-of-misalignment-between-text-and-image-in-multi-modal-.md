...
**一句话要点**：提出PReMA攻击方法，通过修改图像操纵多模态扩散模型输出，揭示文本与图像未对齐的安全风险。

**关键词**：多模态扩散模型, 对抗性攻击, 文本图像对齐, NSFW内容生成, 图像编辑安全

## 3 点简述
- 核心问题：多模态扩散模型中文本与图像对齐不足，易生成不当内容如NSFW。
- 方法要点：PReMA仅修改输入图像，无需改变提示，实现对抗性攻击。
- 实验或效果：在图像修复和风格迁移任务中验证PReMA有效性，威胁模型完整性。

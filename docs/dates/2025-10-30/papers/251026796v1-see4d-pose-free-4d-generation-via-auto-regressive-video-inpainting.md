...
**一句话要点**：提出SEE4D方法，通过自回归视频修复实现无姿态4D生成，从随意视频合成时空内容。

**关键词**：4D生成, 视频修复, 无姿态学习, 虚拟相机, 自回归推理, 时空建模

## 3 点简述
- 核心问题：现有视频到4D方法依赖手动标注相机姿态，成本高且对野外视频脆弱。
- 方法要点：使用固定虚拟相机和视图条件视频修复模型，分离相机控制与场景建模。
- 实验或效果：在跨视图视频生成和稀疏重建基准上，实现优于姿态或轨迹条件基线的性能。

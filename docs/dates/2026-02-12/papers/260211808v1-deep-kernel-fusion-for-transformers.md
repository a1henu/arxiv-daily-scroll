...
**关键词**：长上下文推理, 内存带宽优化, 内核融合, Transformer加速, SGLang集成

## 3 点简述
- 核心问题：长上下文LLM推理受限于内存带宽，SwiGLU MLP块权重过大导致缓存不足，成为性能瓶颈。
- 方法要点：设计深度融合内核，减少HBM流量并提升缓存重用，集成SGLang和内核调度器以适配不同场景。
- 实验或效果：在H100和A100上分别实现最高13.2%和9.7%的加速，优于SGLang，且加速效果随生成长度保持稳定。

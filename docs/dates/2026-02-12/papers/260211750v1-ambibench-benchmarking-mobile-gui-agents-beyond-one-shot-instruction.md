...
**关键词**：移动GUI代理, 意图对齐, 模糊指令, 基准测试, 自动化评估

## 3 点简述
- 核心问题：现有基准假设指令完整明确，忽略用户表达模糊性，无法评估代理的意图对齐能力。
- 方法要点：基于认知差距理论，构建包含四种清晰度级别的任务数据集，并开发自动化评估框架MUSE进行多维度审计。
- 实验或效果：实证结果揭示了SoTA代理在不同清晰度下的性能边界，验证了主动交互的增益和MUSE与人类判断的强相关性。

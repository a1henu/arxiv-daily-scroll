...
**一句话要点**：提出SemaPop-GAN，通过语义-统计融合实现可控人口合成

**关键词**：人口合成, 语义条件生成, 大语言模型, Wasserstein GAN, 边际正则化, 行为语义

## 3 点简述
- 核心问题：人口合成需兼顾统计结构与潜在行为语义，现有方法缺乏语义条件生成能力。
- 方法要点：结合大语言模型提取个体语义角色，作为条件信号，并引入边际正则化对齐目标分布。
- 实验或效果：SemaPop-GAN在边际和联合分布对齐、样本可行性与多样性方面表现更优，支持可控生成。

...
**一句话要点**：提出基于熵引导表示的轻量级框架，以高效预测LLM输出长度，减少批处理推理中的计算浪费。

**关键词**：LLM推理优化, 输出长度预测, 熵引导表示, 轻量级框架, 批处理效率

## 3 点简述
- 核心问题：LLM服务中序列长度的长尾分布导致批处理推理中因过度填充产生显著计算浪费。
- 方法要点：引入熵引导令牌池化和渐进长度预测，重用主模型内部隐藏状态进行高效长度预测。
- 实验或效果：在ForeLen基准上，EGTP将MAE降低29.16%，结合长度感知调度器提升端到端吞吐量。

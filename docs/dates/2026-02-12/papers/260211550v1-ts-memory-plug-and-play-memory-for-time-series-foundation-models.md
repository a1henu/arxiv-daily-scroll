...
**一句话要点**：提出TS-Memory轻量记忆适配器，以解决时间序列基础模型在分布偏移下的适应难题。

**关键词**：时间序列基础模型, 参数记忆蒸馏, 轻量记忆适配器, 分布偏移适应, 零样本预测, 置信感知分位数目标

## 3 点简述
- 核心问题：时间序列基础模型在零样本预测中面临分布偏移适应挑战，现有方法在参数适应与非参数检索间存在权衡。
- 方法要点：通过参数记忆蒸馏，构建离线kNN教师合成置信感知分位数目标，并蒸馏到轻量记忆适配器中，实现无检索推理。
- 实验或效果：在多种时间序列基础模型和基准测试中，点预测和概率预测均优于代表性适应方法，效率接近冻结主干。

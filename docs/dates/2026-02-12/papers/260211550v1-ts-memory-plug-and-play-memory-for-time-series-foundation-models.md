...
**关键词**：时间序列基础模型, 参数化记忆蒸馏, 轻量适配器, 零样本预测, 分布偏移适应, 推理效率

## 3 点简述
- 核心问题：时间序列基础模型零样本预测强，但分布偏移下适应下游任务存在灾难性遗忘或高延迟问题。
- 方法要点：通过参数化记忆蒸馏，构建离线kNN教师合成置信感知分位数目标，蒸馏到轻量适配器中。
- 实验或效果：在多种模型和基准上，点预测和概率预测均优于现有方法，推理效率接近冻结骨干。

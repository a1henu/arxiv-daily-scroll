...
**一句话要点**：提出Clutt3R-Seg，通过层次实例树和稀疏视图实现杂乱场景中语言引导抓取的零样本3D实例分割。

**关键词**：3D实例分割, 稀疏视图, 语言引导抓取, 杂乱场景, 零样本学习, 机器人操作

## 3 点简述
- 核心问题：杂乱环境中遮挡、有限视角和噪声掩码导致3D实例分割不可靠，影响语言引导的机器人操作。
- 方法要点：利用噪声掩码作为信息线索，通过跨视图分组和条件替换构建层次实例树，抑制过分割和欠分割，生成视图一致掩码和鲁棒3D实例。
- 实验或效果：在合成和真实数据集上评估，优于现有基线，在重杂乱序列中AP@25达61.66，仅需四视图输入即超越八视图基线。

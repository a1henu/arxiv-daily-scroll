...
**一句话要点**：提出分类流映射以加速分类数据的少步生成，通过自蒸馏实现连续轨迹建模。

**关键词**：流匹配, 分类数据生成, 自蒸馏, 少步生成, 连续轨迹建模, 加速推理

## 3 点简述
- 核心问题：加速基于流匹配的分类数据生成，传统方法在少步场景下效率或质量受限。
- 方法要点：定义流向单纯形的流映射，约束预测端点，支持连续轨迹训练与蒸馏。
- 实验或效果：在图像、分子图和文本上实现少步生成SOTA，单步生成表现强劲。

...
**一句话要点**：提出Min-kNN距离方法以检测RLVR训练数据，基于推理结构收敛性

**关键词**：RLVR训练数据检测, 推理结构收敛, 黑盒检测方法, 基准污染, 强化学习验证奖励, 成员推理攻击

## 3 点简述
- 核心问题：RLVR训练数据未公开，导致基准污染检测困难，传统基于似然的方法失效
- 方法要点：利用RLVR训练后模型对见过提示生成更相似、僵化的行为，设计无需参考模型或令牌概率的黑盒检测器
- 实验或效果：在多个RLVR训练模型上验证，Min-kNN距离能可靠区分见过与未见示例，优于现有基线

...
**一句话要点**：提出带否定扩展的量化布尔贝叶斯网络与类型槽语法，以解决逻辑信息检索中的推理与解析问题。

**关键词**：逻辑推理模型, 自然语言解析, 量化布尔贝叶斯网络, 类型槽语法, LLM辅助消歧, 信息检索

## 3 点简述
- 核心问题：先前量化布尔贝叶斯网络缺乏否定/反向推理和自然语言解析器，限制了逻辑信息检索能力。
- 方法要点：扩展网络支持否定推理，引入类型槽语法编译句子为逻辑形式，结合LLM预处理与重排。
- 实验或效果：推理引擎通过44/44测试案例，语法解析33/33正确，LLM辅助消歧达95%准确率。

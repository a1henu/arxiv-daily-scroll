...
**关键词**：上下文学习, 演示选择, 监督元学习, 意图分类, 高效推理

## 3 点简述
- 核心问题：上下文学习中演示示例选择是实践瓶颈，需在有限提示预算下高效提升准确性
- 方法要点：基于训练数据构建元数据集，使用TF-IDF相似度和长度兼容比训练逻辑回归评分函数
- 实验或效果：在四个意图数据集和五个开源LLM上评估，Meta-Sel性能领先，尤其适用于小模型

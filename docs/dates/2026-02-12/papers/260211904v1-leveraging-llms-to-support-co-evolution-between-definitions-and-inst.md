...
**一句话要点**：评估大语言模型支持文本DSL定义与实例协同演化的潜力

**关键词**：文本领域特定语言, 协同演化, 大语言模型评估, 语法演化, 实例更新, 人文信息保留

## 3 点简述
- 核心问题：文本DSL语法演化导致实例过时，现有模型驱动工程方法不适用且可能丢失布局和注释等人文信息。
- 方法要点：系统评估Claude Sonnet 4.5和GPT-5.2在十个案例语言中协同演化语法和实例的能力，关注正确性和人文信息保留。
- 实验或效果：小规模实例性能强（≥94%精确率和召回率），但随规模增大性能下降，响应时间增加，演化复杂性和删除粒度影响显著。

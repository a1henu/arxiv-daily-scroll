...
**关键词**：机器人基础模型, 具身数据摄入, 动力学学习, 视觉预测, 多模态扩散变换器, 数据高效微调

## 3 点简述
- 核心问题：现有机器人基础模型依赖大规模行为克隆，丢弃了异构具身数据中的可迁移动力学知识，且统一世界模型实例化难以扩展。
- 方法要点：引入结构化DINO潜在空间预测和多模态扩散变换器，处理异步视觉与动作流，支持10亿参数规模的稳定训练。
- 实验或效果：在模拟和真实世界中，LDA-1B在接触丰富、灵巧和长时程任务上分别提升21%、48%和23%，并能高效微调利用低质量轨迹。

...
**关键词**：化学语言模型, 分子属性预测, 预训练扩展, 下游任务性能, 模型评估策略

## 3 点简述
- 核心问题：预训练损失下降是否保证下游任务性能提升在化学领域未经验证
- 方法要点：通过扩展模型大小、数据集规模和计算资源预训练CLMs并评估下游任务
- 实验或效果：发现预训练损失持续降低但下游性能改善有限，且替代指标如Hessian也失效

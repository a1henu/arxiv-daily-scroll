...
**一句话要点**：提出基于贝叶斯深度学习的校准优化框架，以提升医学影像决策支持系统的可靠性与可解释性。

**关键词**：贝叶斯深度学习, 模型校准, 医学影像分析, 不确定性量化, 决策支持系统, 可解释人工智能

## 3 点简述
- 核心问题：深度学习模型在医学影像应用中常存在校准不足，导致错误预测过度自信，影响临床决策可靠性。
- 方法要点：引入置信度-不确定性边界损失优化训练，结合双温度缩放进行后处理校准，强制对齐预测正确性与不确定性估计。
- 实验或效果：在肺炎筛查、糖尿病视网膜病变检测和皮肤病变识别任务上验证，实现跨模态校准改进，并在数据稀缺和不平衡场景中保持稳健性能。

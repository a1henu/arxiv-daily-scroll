...
**一句话要点**：提出HoloBrain-0 VLA框架，结合机器人先验以提升3D空间推理，支持可靠机器人部署。

**关键词**：视觉-语言-动作框架, 机器人先验, 3D空间推理, 仿真基准, 开源生态系统, 低延迟部署

## 3 点简述
- 核心问题：基础模型研究与可靠机器人部署之间存在差距，需增强3D空间推理能力。
- 方法要点：引入VLA架构，显式结合机器人多视角相机参数和运动学描述（URDF）作为先验。
- 实验或效果：在RoboTwin 2.0等仿真基准和真实世界长程操作任务中取得先进结果，0.2B参数变体实现低延迟部署。

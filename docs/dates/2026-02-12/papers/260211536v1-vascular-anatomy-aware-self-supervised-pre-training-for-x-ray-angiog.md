...
**一句话要点**：提出血管解剖感知自监督预训练框架VasoMIM，以解决X射线血管造影分析中标注数据稀缺问题。

**关键词**：X射线血管造影, 自监督学习, 掩蔽图像建模, 血管解剖感知, 预训练数据集, 迁移学习

## 3 点简述
- 核心问题：X射线血管造影分析因标注数据稀缺，限制深度学习应用，自监督学习潜力未充分探索。
- 方法要点：VasoMIM集成解剖知识，包括血管引导掩蔽策略和解剖一致性损失，增强语义学习。
- 实验或效果：在四个下游任务上验证，VasoMIM展现优越迁移性，达到最先进性能，并发布最大预训练数据集XA-170K。

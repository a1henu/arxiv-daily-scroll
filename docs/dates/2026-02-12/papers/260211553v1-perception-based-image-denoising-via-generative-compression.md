...
**关键词**：图像去噪, 生成压缩, 感知质量, 条件WGAN, 扩散模型, 率失真感知权衡

## 3 点简述
- 核心问题：传统失真驱动方法在强噪声和分布偏移下易产生过度平滑重建，难以平衡失真与感知质量。
- 方法要点：引入条件WGAN和扩散模型两种实例，利用LPIPS损失和Wasserstein距离优化感知重建，并建立压缩去噪器的理论保证。
- 实验或效果：在合成和真实噪声基准测试中，实现一致的感知改进，同时保持竞争力的失真性能。

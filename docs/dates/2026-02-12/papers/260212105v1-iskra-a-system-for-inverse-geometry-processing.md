...
**关键词**：逆几何处理, 几何算法微分, 伴随方法, 机器学习集成, 散射-聚集方法, ADMM求解器

## 3 点简述
- 核心问题：如何高效微分几何处理算法，以支持逆问题求解和机器学习集成。
- 方法要点：利用伴随法和散射-聚集方法，兼容现有局部-全局和ADMM求解器，无需算法重构。
- 实验或效果：在平均曲率流、谱共形参数化等应用中验证，实现低实现成本、快速运行和低内存需求。

...
## 3 点简述
- 核心问题：碳捕集与封存中地下流模拟因观测稀疏和反问题不适定而难以准确表征。
- 方法要点：使用单通道扩散模型学习地质参数先验分布，通过局部神经算子代理提供物理一致指导，实现参数与动态场的解耦建模。
- 实验或效果：在合成数据集上，正演建模相对误差降至7.7%，反演结果与真实后验的Jensen-Shannon散度小于0.06，样本效率提升4倍。

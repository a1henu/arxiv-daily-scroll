...
**一句话要点**：提出校准方法，利用仅购买数据修正辅助预测器偏差以估计未观测不购买概率

**关键词**：不购买概率估计, 辅助预测器校准, 多项式逻辑模型, 品类优化, 误差界分析, 机器学习应用

## 3 点简述
- 核心问题：仅交易数据中未观测不购买选项导致市场大小和偏好估计困难，辅助预测器存在偏差。
- 方法要点：在仿射误校准下通过回归识别参数，在弱单调条件下提出基于排序的校准方法并推导误差界。
- 实验或效果：数值实验显示改进不购买估计和下游品类决策，分析校准精度对收入性能的影响。

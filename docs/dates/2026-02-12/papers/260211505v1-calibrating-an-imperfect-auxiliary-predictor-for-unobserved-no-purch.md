...
**关键词**：无购买选择估计, 辅助预测器校准, 多项式逻辑模型, 品类优化, 统计误差界

## 3 点简述
- 核心问题：仅交易数据中无购买选择信息缺失，导致市场大小和偏好估计困难。
- 方法要点：在仿射误校准下通过回归识别参数，在弱单调条件下提出基于排序的校准方法。
- 实验或效果：数值实验显示无购买估计和下游品类决策改进，并分析校准精度对收入的影响。

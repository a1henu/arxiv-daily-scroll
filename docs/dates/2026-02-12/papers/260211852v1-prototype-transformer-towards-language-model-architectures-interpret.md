...
**关键词**：可解释语言模型, 原型Transformer, 自回归模型, 双向通信, 线性复杂度, 鲁棒性分析

## 3 点简述
- 核心问题：当前语言模型推理过程不透明，影响信任并带来风险如欺骗和幻觉。
- 方法要点：基于原型向量实现输入序列与原型的双向通信，自动捕获可命名概念以支持解释和编辑。
- 实验或效果：在文本生成和GLUE任务上表现良好，计算复杂度线性于序列长度，提供可解释路径分析鲁棒性。

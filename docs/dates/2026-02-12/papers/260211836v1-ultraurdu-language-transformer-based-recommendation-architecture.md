...
**关键词**：乌尔都语推荐, 查询长度感知, 双嵌入架构, 语义路由, 低资源语言, Transformer嵌入

## 3 点简述
- 乌尔都语作为低资源语言，现有推荐系统依赖词汇匹配，语义意图捕捉差，导致推荐相关性低。
- ULTRA采用双嵌入架构，基于阈值路由查询到标题级或全文级语义管道，优化语义粒度。
- 在大规模乌尔都语新闻语料上实验，相比单管道基线，精度提升超90%，验证了查询自适应语义对齐的有效性。

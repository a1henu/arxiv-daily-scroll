...
**一句话要点**：提出聚合模型而非解释以改进特征重要性估计，适用于表达性模型

**关键词**：特征重要性估计, 模型集成, 表达性模型, 超额风险, 生物医学应用, UK Biobank

## 3 点简述
- 核心问题：表达性模型因数据采样和算法随机性导致特征重要性估计不稳定，影响生物医学应用。
- 方法要点：理论分析表明，模型级集成能减少超额风险，提供更准确的特征重要性估计。
- 实验或效果：在经典基准和UK Biobank大规模蛋白质组学研究中验证了模型级集成的优势。

...
**关键词**：模型差异检测, 跨架构比较, 交叉编码器, 无监督学习, LLM安全分析

## 3 点简述
- 核心问题：现有模型差异检测方法主要针对同架构模型，难以应用于新发布的跨架构LLM。
- 方法要点：引入专用特征交叉编码器，通过架构修改更好地隔离模型特有特征，实现跨架构差异检测。
- 实验或效果：无监督发现Qwen3-8B的中共对齐、Llama3.1-8B-Instruct的美国例外论和GPT-OSS-20B的版权拒绝机制等特征。

...
**一句话要点**：提出UFO架构以解决不规则时间序列概率预测中的计算效率与全局建模问题

**关键词**：不规则时间序列, 概率预测, U-Net, Transformer, Neural CDE, 并行计算

## 3 点简述
- 核心问题：不规则时间序列概率预测在医疗和金融领域至关重要，但现有方法如Neural CDE计算慢且缺乏全局上下文。
- 方法要点：UFO结合U-Net的并行多尺度特征提取、Transformer的全局建模和Neural CDE的连续时间动态，构建全因果并行模型。
- 实验或效果：在五个标准基准测试中，UFO在预测准确性上优于十个先进基线，推理速度比传统Neural CDE快达15倍。

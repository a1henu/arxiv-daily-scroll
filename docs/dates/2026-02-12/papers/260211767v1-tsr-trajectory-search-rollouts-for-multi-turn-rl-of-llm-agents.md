...
**关键词**：多轮强化学习, 轨迹搜索, 大语言模型智能体, 训练时优化, 树状搜索, rollout生成

## 3 点简述
- 多轮强化学习面临奖励稀疏或延迟、环境随机性挑战，导致轨迹采样效率低和模式崩溃。
- TSR在训练时进行轻量级树状搜索，基于任务反馈选择高分动作构建高质量轨迹，优化rollout生成。
- 结合PPO和GRPO，在Sokoban等任务上实现最高15%性能提升和更稳定学习，训练计算成本一次性增加。

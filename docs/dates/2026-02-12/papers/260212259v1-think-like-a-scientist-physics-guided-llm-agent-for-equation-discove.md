...
**关键词**：符号方程发现, 物理引导推理, LLM代理, 符号回归, 科学计算, 多步推理

## 3 点简述
- 核心问题：现有LLM系统直接从数据猜测方程，缺乏模拟科学家多步推理过程，如先推断物理属性再约束候选方程。
- 方法要点：引入KeplerAgent，协调物理工具提取中间结构，配置符号回归引擎如PySINDy和PySR，以限制函数库和结构约束。
- 实验或效果：在物理方程基准测试中，相比LLM和传统基线，KeplerAgent显著提高符号准确性和对噪声数据的鲁棒性。

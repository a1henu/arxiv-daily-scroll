...
**一句话要点**：提出DMAP方法，将文本映射到单位区间样本，以支持模型无关的统计文本分析。

**关键词**：文本统计分析, 语言模型应用, 概率分布映射, 模型无关分析, 合成数据检测

## 3 点简述
- 核心问题：传统指标如困惑度无法充分捕捉上下文，需改进基于LLM的文本统计分析方法。
- 方法要点：通过语言模型将文本转换为编码排名和概率信息的单位区间样本集，提供统一统计视图。
- 实验或效果：应用于生成参数验证、机器生成文本检测和下游模型指纹分析，展示广泛实用性。

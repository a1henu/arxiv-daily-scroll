...
**关键词**：测试时适应, Tsallis熵, 自适应去偏, 视觉语言模型, 不确定性估计, 跨域基准

## 3 点简述
- 核心问题：CLIP预训练数据不平衡导致Shannon熵在测试时产生偏差不确定性估计
- 方法要点：引入Tsallis熵并自适应定制类别特定参数，结合标签调整策略增强适应
- 实验或效果：在ImageNet及其变体和跨域基准上优于现有方法，无需额外超参数调优

...
**一句话要点**：提出基于扩散先验的贝叶斯框架，用于逆问题中观测参数与图像的同时估计

**关键词**：逆问题, 贝叶斯估计, 扩散先验, MCMC算法, 参数估计, 不确定性量化

## 3 点简述
- 核心问题：在贝叶斯正则化逆问题中，估计观测参数（响应和误差参数）的后验采样困难
- 方法要点：利用扩散过程建模先验，结合MCMC算法高效计算估计值和后验性质
- 实验或效果：数值实验验证了计算效率高，估计质量和不确定性量化效果良好

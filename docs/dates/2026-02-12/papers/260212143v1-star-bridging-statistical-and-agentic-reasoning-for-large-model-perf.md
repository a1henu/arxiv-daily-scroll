...
**一句话要点**：提出STAR框架，通过结合统计期望与知识驱动推理，解决大模型性能预测中的模式偏移和数据稀疏问题。

**关键词**：大模型性能预测, 统计与推理融合, 约束概率矩阵分解, 期望违背理论, 稀疏数据预测, 可解释性增强

## 3 点简述
- 核心问题：大模型全面评估成本高昂，现有统计方法难以处理模式偏移、数据稀疏和缺乏解释性，而纯LLM方法不可靠。
- 方法要点：STAR利用检索器获取外部知识，将语义特征嵌入约束概率矩阵分解生成统计期望，基于期望违背理论通过推理模块进行精炼预测。
- 实验或效果：在极端稀疏条件下（每个测试模型仅1-2个观测分数），STAR在得分和排名指标上优于所有基线，总得分比最强统计方法提升14.46%。

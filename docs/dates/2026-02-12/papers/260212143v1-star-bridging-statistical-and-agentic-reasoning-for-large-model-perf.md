...
**一句话要点**：提出STAR框架，结合统计与智能推理预测大模型性能，以应对评估成本高和观测稀疏问题。

**关键词**：大模型性能预测, 统计推理, 智能体推理, 约束概率矩阵分解, 期望违背理论, 稀疏数据评估

## 3 点简述
- 核心问题：大模型全面评估成本高昂，现有统计方法难以处理模式偏移、数据稀疏和缺乏解释性，而纯LLM方法不可靠。
- 方法要点：STAR通过检索外部知识嵌入语义特征到约束概率矩阵分解生成统计期望，并基于期望违背理论进行推理调整，提供可追溯解释。
- 实验或效果：在极端稀疏条件下（每测试模型仅1-2个观测分数），STAR在得分和排名指标上优于所有基线，总得分比最强统计方法提升14.46%。

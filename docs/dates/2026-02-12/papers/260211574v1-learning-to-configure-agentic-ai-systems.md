...
**关键词**：智能体系统配置, 强化学习, 查询级决策, 动态配置优化, LLM智能体, 资源效率

## 3 点简述
- 核心问题：基于LLM的智能体系统配置通常依赖固定模板或手动启发式，导致对易难查询均采用相同配置，产生脆弱行为和计算浪费。
- 方法要点：将智能体配置建模为查询级决策问题，引入ARC学习器，使用强化学习学习轻量级分层策略，动态调整工作流、工具、令牌预算和提示。
- 实验或效果：在推理和工具增强问答基准测试中，ARC策略优于强基线，任务准确率提升高达25%，同时降低令牌和运行时成本。

...
**一句话要点**：提出Olmix框架以解决语言模型开发中数据混合的配置空间理解和动态域集更新问题

**关键词**：数据混合, 语言模型训练, 动态域集, 混合重用, 配置空间分析

## 3 点简述
- 核心问题：现有数据混合方法在真实世界LM开发中面临配置空间不明确和域集动态变化的挑战
- 方法要点：通过实证研究确定混合方法设计选择，并引入混合重用机制高效处理域集更新
- 实验或效果：在模拟真实开发的五次更新中，混合重用匹配全重算性能，计算减少74%，下游任务性能提升11.6%

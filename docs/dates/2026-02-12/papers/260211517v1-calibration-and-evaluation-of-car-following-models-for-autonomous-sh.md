...
**一句话要点**：提出多标准框架校准与评估自动驾驶班车跟驰模型，提升模型性能与可比性。

**关键词**：自动驾驶班车, 跟驰模型, 机器学习校准, 多标准评估, 轨迹分析

## 3 点简述
- 核心问题：自动驾驶班车跟驰模型缺乏专用校准与统一评估框架，限制其交通影响分析。
- 方法要点：校准八种机器学习算法和两种物理模型，引入多标准评估框架整合预测精度、轨迹稳定性和统计相似性。
- 实验或效果：XGBoost模型表现最佳，序列模型捕获长期稳定性但短期响应不足，传统模型准确性较低。

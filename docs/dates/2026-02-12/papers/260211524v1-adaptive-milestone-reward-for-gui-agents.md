...
**关键词**：移动GUI智能体, 强化学习, 信用分配, 自适应奖励, 里程碑学习, 异构环境泛化

## 3 点简述
- 核心问题：强化学习在移动GUI智能体训练中面临长时程任务的信用分配难题，奖励保真度与密度之间存在权衡。
- 方法要点：ADMIRE通过动态提取里程碑构建可验证的自适应奖励系统，并采用非对称信用分配策略去噪成功轨迹和支撑失败轨迹。
- 实验或效果：在AndroidWorld上，ADMIRE使不同基础模型的成功率绝对提升超过10%，并在多种RL算法和异构环境中展现强泛化性。

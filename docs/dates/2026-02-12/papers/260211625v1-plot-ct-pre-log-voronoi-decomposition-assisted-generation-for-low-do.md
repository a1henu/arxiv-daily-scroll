...
## 3 点简述
- 核心问题：低剂量CT重建面临严重噪声和数据保真度下降，传统方法在图像或后对数投影域操作，未能充分利用预对数测量中的结构信息。
- 方法要点：在预对数正弦图上应用Voronoi分解，将数据解耦为不同潜在空间中的组件，增强模型学习判别性特征的能力，直接提升重建精度。
- 实验或效果：在预对数域1e4入射光子水平下，PLOT-CT相比传统方法实现2.36dB PSNR提升，达到先进性能。

...
**关键词**：MIMO接收机, 深度学习辅助, 可扩展性, 可解释性, 线性均衡器, 端到端仿真

## 3 点简述
- 核心问题：传统机器学习接收机算法在空间复用阶数增加时扩展性差，且缺乏可解释性和泛化能力。
- 方法要点：通过共享权重的DetectorNN实现近线性复杂度扩展，结合线性均衡器和轻量级DenoiseNN增强可解释性。
- 实验或效果：在5G/6G端到端仿真中，相比传统基线提高了误码率和频谱效率，支持不同MIMO配置无需重训练。

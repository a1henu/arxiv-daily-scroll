...
**关键词**：偏好优化, 训练-推理不匹配, 参考策略, 梯度衰减, 条件性去偏, 离线对齐

## 3 点简述
- 核心问题：DPO依赖参考策略，在悲观对（参考模型偏好被拒绝响应）中导致梯度过早衰减，引发训练-推理不匹配。
- 方法要点：HyPO通过条件性应用参考，在悲观对中将参考视为中性，替换目标函数项为Δθ-max{0,Δref}，增强学习信号。
- 实验或效果：HyPO在偏好对齐任务中提升推理对齐指标和成对胜率，保持DPO的计算成本。

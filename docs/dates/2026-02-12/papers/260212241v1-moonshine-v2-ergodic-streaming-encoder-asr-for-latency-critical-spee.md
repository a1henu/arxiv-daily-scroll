...
**一句话要点**：提出Moonshine v2流式编码器ASR，通过滑动窗口自注意力解决边缘设备低延迟语音应用需求。

**关键词**：流式自动语音识别, 低延迟推理, 滑动窗口自注意力, 边缘设备ASR, 时间到首词延迟优化

## 3 点简述
- 核心问题：全注意力Transformer编码器在流式ASR中导致延迟随话语长度线性增长，不适用于低延迟场景。
- 方法要点：采用滑动窗口自注意力实现有界低延迟推理，保持强局部上下文，减少计算复杂度。
- 实验或效果：在标准基准上达到最先进词错误率，精度与6倍大小模型相当，运行速度显著更快。

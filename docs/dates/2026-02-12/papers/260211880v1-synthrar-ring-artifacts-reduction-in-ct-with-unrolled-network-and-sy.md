...
**关键词**：CT环状伪影减少, 展开网络, 合成数据训练, CT几何建模, 图像域校正, 正弦图域校正

## 3 点简述
- 核心问题：CT探测器响应不一致导致环状伪影，影响临床图像质量。
- 方法要点：基于非理想探测器响应理论，用展开网络建模CT几何前向操作，结合合成数据训练。
- 实验或效果：在多种扫描几何和解剖区域评估，模型优于现有方法，无需真实临床数据。

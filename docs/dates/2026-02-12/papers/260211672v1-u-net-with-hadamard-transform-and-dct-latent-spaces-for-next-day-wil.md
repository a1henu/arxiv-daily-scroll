...
**一句话要点**：提出TD-FusionUNet，结合可训练变换层，用于轻量级次日野火蔓延预测。

**关键词**：野火蔓延预测, U-Net架构, 变换域融合, 轻量级模型, 多模态数据

## 3 点简述
- 核心问题：基于多模态卫星数据，实现轻量高效的次日野火蔓延预测。
- 方法要点：引入可训练Hadamard变换和DCT层，在正交化潜在空间捕获频率成分。
- 实验或效果：在WildfireSpreadTS数据集上，以37万参数达到F1分数0.591，优于基线。

...
**关键词**：数字减影血管造影, 潜在扩散模型, 语义条件生成, 脑血管疾病, 医学图像合成

## 3 点简述
- 核心问题：脑部DSA图像采集成本高且侵入性强，限制大规模数据收集与共享。
- 方法要点：使用文本嵌入编码解剖循环和C臂位置，训练条件潜在扩散模型生成合成图像。
- 实验或效果：合成图像获专家评分3.1-3.3，FID为15.27，显示高真实性与分布相似性。

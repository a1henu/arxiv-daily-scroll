...
**关键词**：视觉导航, 3D高斯溅射, 真实到仿真模拟, 动态环境, 策略学习, 零样本泛化

## 3 点简述
- 核心问题：视觉导航模型在真实动态环境中因仿真到真实差距和动态障碍物而表现不佳。
- 方法要点：结合静态场景重建与动态人类高斯溅射，生成逼真动态导航数据集并训练鲁棒策略。
- 实验或效果：在仿真和真实实验中优于基线，展示出泛化潜力，支持零样本部署。

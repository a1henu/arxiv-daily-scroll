...
**一句话要点**：提出通用扩散降尺度框架，以零样本方式提升天气预测的空间分辨率和概率性表示。

**关键词**：扩散模型, 天气预测降尺度, 概率性预测, 零样本学习, 空间分辨率提升

## 3 点简述
- 核心问题：确定性低分辨率天气预报缺乏高分辨率概率性预测，需模型无关的降尺度方法。
- 方法要点：训练单一条件扩散模型，基于配对数据，无需微调即可应用于异构上游模型。
- 实验或效果：在多种AI和NWP系统上，降尺度预测的集合平均和概率技能（CRPS）均显著提升。

...
**一句话要点**：提出LAMP隐式语言地图框架，以解决机器人零样本导航中的内存效率和精细路径规划问题。

**关键词**：机器人导航, 隐式神经场, 零样本学习, 语言驱动地图, 路径规划, 贝叶斯框架

## 3 点简述
- 现有方法因显式存储语言向量导致内存需求大和分辨率有限，难以扩展到大环境。
- LAMP通过隐式神经场编码语言特征，结合稀疏图实现粗到细路径生成，并采用贝叶斯框架增强鲁棒性。
- 实验在模拟和真实多楼层环境中验证了LAMP在内存效率和目标到达精度上的优势。

...
**一句话要点**：提出单目视觉8D姿态估计方法，用于自动驾驶中铰接自行车和骑行者的精细姿态估计。

**关键词**：单目视觉姿态估计, 铰接物体建模, 自动驾驶安全, 类别级姿态估计, 3D关键点检测

## 3 点简述
- 核心问题：铰接自行车姿态变化导致3D边界框和方向不准确，6D姿态估计不足以捕捉转向/踏板角度。
- 方法要点：从单RGB图像联合估计8D姿态和3D关键点，包括转向和踏板相对于车体的旋转。
- 实验或效果：使用合成和真实数据训练，在真实图像上实现竞争性精度，优于现有6D姿态估计方法。

...
**一句话要点**：提出掩码位置编码与条带卷积上下文建模以提升跨视角物体地理定位性能

**关键词**：跨视角物体地理定位, 掩码位置编码, 条带卷积, 上下文建模, 端到端框架, 卫星图像分析

## 3 点简述
- 核心问题：现有方法依赖关键点位置编码，忽略物体形状，导致对标注偏移敏感和跨视角匹配能力有限。
- 方法要点：引入掩码位置编码捕获空间坐标与物体轮廓，并设计条带卷积模块提取长程上下文特征。
- 实验或效果：在CVOGL和VIGOR-Building数据集上实现SOTA，地面到卫星场景定位准确率提升3.39%。

...
**一句话要点**：提出GaussDetect-LiNGAM以消除高斯性测试，提升双变量因果发现的效率与鲁棒性。

**关键词**：因果发现, LiNGAM, 高斯性测试, 独立性测试, 双变量分析, 因果推断

## 3 点简述
- 核心问题：传统LiNGAM方法依赖高斯性测试，易受样本影响且脆弱。
- 方法要点：利用前向模型噪声高斯性与反向模型残差独立的等价性，替换为核独立性测试。
- 实验效果：验证等价性，在多样噪声和样本量下保持高一致性，减少测试次数。

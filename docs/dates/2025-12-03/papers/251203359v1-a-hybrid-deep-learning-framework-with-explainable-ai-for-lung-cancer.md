...
**一句话要点**：提出结合DenseNet169与SVM的混合深度学习框架，用于肺癌CT图像分类，并集成可解释AI提升透明度。

**关键词**：肺癌分类, 深度学习, 可解释AI, CT图像分析, 混合模型, 特征提取

## 3 点简述
- 核心问题：肺癌早期诊断依赖CT扫描，但人工解读耗时且易出错，需自动分类系统提高准确性和可解释性。
- 方法要点：使用DenseNet169（含注意力机制和FPN）与SVM（基于MobileNetV2特征）进行混合分类，并应用Grad-CAM和SHAP增强模型可解释性。
- 实验或效果：在IQOTHNCCD数据集上，DenseNet169和SVM模型均达到98%准确率，表明其在实际医疗应用中的鲁棒性。

...
**一句话要点**：发现前馈三维重建模型VGGT隐含离群视图抑制能力，无需额外训练即可提升野外图像重建鲁棒性。

**关键词**：三维重建, 离群视图抑制, 前馈模型, 视觉几何, Transformer, 野外图像处理

## 3 点简述
- 核心问题：野外图像集合中的噪声图像（如无重叠视图）会降低前馈三维重建模型的性能。
- 方法要点：分析VGGT模型，识别出特定层自然编码离群抑制表示，直接用于离群视图过滤。
- 实验或效果：在受控和野外数据集上验证该隐含过滤机制的一致性和泛化能力。

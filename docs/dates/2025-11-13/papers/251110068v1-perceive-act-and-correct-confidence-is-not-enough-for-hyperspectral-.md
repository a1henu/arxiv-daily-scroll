...
**关键词**：高光谱图像分类, 半监督学习, 不确定性估计, 伪标签优化, 认知感知学习

## 3 点简述
- 核心问题：高光谱图像分类中，模型常因高置信度而错误预测，缺乏不确定性感知，导致确认偏差和泛化能力差。
- 方法要点：CABIN通过感知-行动-校正闭环学习，估计认知不确定性，采用不确定性引导采样和动态分配策略优化伪标签。
- 实验或效果：集成CABIN提升多种先进方法的性能，提高标注效率和分类准确性。

...
**关键词**：高光谱图像分类, 半监督学习, 不确定性估计, 伪标签优化, 确认偏差缓解

## 3 点简述
- 核心问题：高光谱分类中模型常因高置信度而错误预测，缺乏不确定性感知，导致确认偏差。
- 方法要点：CABIN通过感知-行动-校正闭环，估计不确定性并动态分配伪标签以减少噪声。
- 实验或效果：集成CABIN提升多种先进方法的标注效率和性能，增强泛化能力。

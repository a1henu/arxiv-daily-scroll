...
**一句话要点**：提出MTAttack框架以解决大型视觉语言模型中的多目标后门攻击问题

**关键词**：大型视觉语言模型, 多目标后门攻击, 触发器优化, 代理空间划分, 安全漏洞, 防御鲁棒性

## 3 点简述
- 核心问题：现有后门攻击仅针对单一目标，多目标攻击因特征干扰难以实现准确映射
- 方法要点：引入代理空间划分和触发器原型锚定约束，在潜在空间联合优化多个触发器
- 实验或效果：在流行基准上实现高成功率，优于现有方法，并展示跨数据集泛化性和防御鲁棒性

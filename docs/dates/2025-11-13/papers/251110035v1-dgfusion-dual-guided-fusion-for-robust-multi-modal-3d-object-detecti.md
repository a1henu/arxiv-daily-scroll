...
**一句话要点**：提出DGFusion双引导融合方法以解决自动驾驶中多模态3D目标检测对困难实例的挑战

**关键词**：多模态3D目标检测, 双引导融合, 难度感知匹配, 自动驾驶感知, 困难实例检测, 特征融合

## 3 点简述
- 核心问题：现有多模态3D检测方法多为单引导，难以处理远距离、小尺寸或遮挡的困难实例
- 方法要点：引入双引导范式，结合点云引导图像和图像引导点云，通过难度感知实例匹配器生成实例对
- 实验效果：在nuScenes数据集上mAP、NDS和平均召回率分别提升1.0%、0.8%和1.3%，增强困难实例检测鲁棒性

...
**一句话要点**：提出JudgeMind方法以解决滑雪动作质量评估中的细粒度评分问题

**关键词**：动作质量评估, 细粒度数据集, 阶段感知融合, 知识增强解码, 滑雪动作分析

## 3 点简述
- 核心问题：现有动作质量评估方法缺乏细粒度注释，导致可解释性和可靠性不足。
- 方法要点：通过分段评分、阶段感知特征融合和基于知识的解码器模拟裁判思维。
- 实验或效果：在FineSkiing数据集上实现最先进性能，提升评分准确性和鲁棒性。

...
**一句话要点**：提出HD-ExpIt框架，通过环境反馈迭代微调分层扩散策略，以解决语言条件操作中规划器与控制器能力不匹配问题。

**关键词**：语言条件操作, 分层策略, 扩散模型, 迭代微调, 自强化学习, CALVIN基准

## 3 点简述
- 核心问题：分层策略中规划器生成子目标时忽略控制器实际限制，导致任务失败。
- 方法要点：采用自强化循环，利用扩散规划自主发现成功行为，并蒸馏回分层策略以隐式对齐能力。
- 实验或效果：在CALVIN基准上显著提升离线训练策略性能，达到从头训练方法中的最优水平。

...
**关键词**：生存分析, 异质处理效应, 因果推断, 基准测试, 删失数据, 精准医疗

## 3 点简述
- 核心问题：生存分析中异质处理效应估计因删失和反事实未观测而面临挑战，现有评估方法不一致。
- 方法要点：构建包含合成、半合成和真实数据集的基准，系统化测试因果假设和生存动态。
- 实验或效果：首次在多样条件下对生存HTE方法进行严格比较，支持公平、可复现和可扩展的评估。

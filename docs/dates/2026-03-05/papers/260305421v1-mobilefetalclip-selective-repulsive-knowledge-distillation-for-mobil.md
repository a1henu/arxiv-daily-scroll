...
**一句话要点**：提出选择性排斥知识蒸馏以解决移动胎儿超声分析中极端容量差距问题

**关键词**：知识蒸馏, 胎儿超声分析, 移动AI部署, 对比学习, 模型压缩, 实时处理

## 3 点简述
- 核心问题：胎儿超声AI模型参数过大（超300M），无法部署在资源受限的移动设备上，标准知识蒸馏在约26倍容量差距下失效。
- 方法要点：引入选择性排斥知识蒸馏，分解对比知识蒸馏为对角和非对角分量，保留匹配对对齐，非对角权重衰减为负值，排斥学生模仿教师的类间混淆，强制发现架构原生特征。
- 实验或效果：11.4M参数学生模型在零样本HC18生物测量有效性（88.6% vs. 83.5%）和脑亚平面F1分数（0.784 vs. 0.702）上超越304M参数教师模型，在iPhone 16 Pro上运行时间1.6毫秒，支持手持超声设备实时辅助AI。

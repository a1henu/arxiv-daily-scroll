...
**关键词**：扩散模型, 结构化剪枝, 进化搜索, 稀疏度调度, 权重路由, 图像生成

## 3 点简述
- 核心问题：扩散模型计算量大，现有结构化剪枝方法难以平衡加速与质量，且依赖手动调优的稀疏度调度。
- 方法要点：将扩散轨迹分阶段，利用进化搜索自动优化稀疏度调度，通过权重路由动态激活参数，避免模型复制。
- 实验或效果：在DiT和SDXL上验证，Diff-ES实现实际加速，图像质量下降最小，达到先进性能。

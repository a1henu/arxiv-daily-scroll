...
**关键词**：视觉语言模型, 知识产权保护, 动态授权, 合法性感知, 双路径推理, 自适应部署

## 3 点简述
- 核心问题：现有VLM知识产权保护方法依赖静态授权，在动态环境中灵活性不足且对未授权输入响应不透明。
- 方法要点：引入轻量级动态授权模块，支持按需用户控制授权，并采用双路径推理机制联合预测输入合法性与任务输出。
- 实验或效果：在多个跨域基准测试中，AoD-IP保持授权域性能，可靠检测未授权输入，支持自适应部署。

...
## 3 点简述
- 核心问题：现有网页代理在复杂长时程任务中因内存限制、规划能力弱和贪婪行为导致性能不佳。
- 方法要点：采用在线分层规划器，结合动态AND/OR树进行高效搜索，并引入结构化记忆模块跟踪候选解。
- 实验或效果：在WebVoyager、WebArena和自定义购物基准测试中，相比标准LLM代理提升了长时程网页浏览任务性能。

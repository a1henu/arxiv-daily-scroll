...
**一句话要点**：提出FedMEPD框架，通过模态特定编码器和部分个性化融合解码器解决联邦学习中模态异构与个性化需求问题。

**关键词**：联邦学习, 多模态分割, 脑肿瘤分割, 个性化模型, 模态异构, 编码器解码器架构

## 3 点简述
- 核心问题：联邦学习在医学图像分析中面临模态异构（参与者仅拥有部分模态）和个性化模型需求的双重挑战。
- 方法要点：采用模态特定编码器处理模态异构，部分个性化解码器动态调整参数，并通过全局锚点校准缺失模态表示。
- 实验或效果：在BraTS 2018和2020基准测试中验证，优于现有方法，设计有效提升性能。

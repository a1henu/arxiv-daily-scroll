...
**一句话要点**：提出RealWonder系统，通过物理模拟桥接动作与视频生成，实现实时动作条件视频生成。

**关键词**：动作条件视频生成, 物理模拟, 实时系统, 3D重建, 蒸馏训练, 交互探索

## 3 点简述
- 当前视频生成模型缺乏3D动作物理后果模拟能力，因缺少动作影响3D场景的结构理解。
- 核心方法是将连续动作通过物理模拟转换为视觉表示（光流和RGB），供视频模型处理。
- 系统集成3D重建、物理模拟和蒸馏视频生成器，在480x832分辨率下达到13.2 FPS，支持交互探索。

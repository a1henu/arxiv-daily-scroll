...
**一句话要点**：提出CompACT离散分词器，将观测压缩至8个令牌以加速世界模型中的决策规划。

**关键词**：世界模型, 离散分词器, 决策规划, 计算效率, 令牌压缩, 实时控制

## 3 点简述
- 核心问题：传统分词器编码观测为数百令牌，导致世界模型在决策规划时计算成本高、速度慢。
- 方法要点：设计CompACT离散分词器，压缩观测至最少8个令牌，保留规划所需关键信息，降低计算负担。
- 实验或效果：基于CompACT的动作条件世界模型实现竞争性规划性能，规划速度提升数个数量级，促进实际部署。

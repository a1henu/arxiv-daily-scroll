...
**关键词**：搜索代理评估, 并行世界框架, 动态基准构建, 原子事实生成, 交互式基准测试, 证据合成能力

## 3 点简述
- 核心问题：搜索代理评估面临高质量基准构建昂贵、动态过时、归因模糊和依赖商业引擎导致可复现性差等挑战
- 方法要点：通过合成未来场景和原子事实，构建并行世界引擎模型，模拟搜索交互以评估代理能力
- 实验或效果：发布MPW-Bench基准，实验显示搜索代理在证据合成方面强，但在陌生环境中的证据收集和停止决策存在瓶颈

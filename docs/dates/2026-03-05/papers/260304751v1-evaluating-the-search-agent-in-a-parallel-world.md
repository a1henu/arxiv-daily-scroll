...
**关键词**：搜索代理评估, 并行世界框架, 动态基准构建, 原子事实生成, 交互式评测

## 3 点简述
- 核心问题：搜索代理评估面临高质量基准构建昂贵、动态过时、归因模糊和商业引擎依赖等挑战
- 方法要点：通过合成未来场景和原子事实，构建并行世界引擎模型进行交互式评估
- 实验或效果：发布MPW-Bench基准，实验显示搜索代理在证据合成强，但证据收集和停止决策是瓶颈

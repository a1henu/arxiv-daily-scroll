...
**一句话要点**：提出基于广播3D姿态序列的棒球投球类型预测方法，仅使用身体运动学实现80.4%准确率。

**关键词**：3D姿态估计, 棒球投球预测, 生物力学特征, 梯度提升分类, 运动学分析

## 3 点简述
- 核心问题：仅凭投手身体运动学能否预测投球类型，不依赖球飞行数据。
- 方法要点：结合扩散模型3D姿态估计、自动事件检测、生物力学特征提取和梯度提升分类。
- 实验或效果：在119,561次投球数据上验证，识别8种投球类型，分析上下半身贡献度。

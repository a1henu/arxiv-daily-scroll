...
**一句话要点**：提出通用多尺度分割框架与基准数据集，以处理异构历史地图集合的语义分割问题。

**关键词**：历史地图分割, 多尺度集成, 数据合成, 语义分割, 基准数据集, 异构地图

## 3 点简述
- 核心问题：历史地图集合在风格、尺度和地理焦点上高度多样，现有模型多针对同质地图系列，缺乏通用性。
- 方法要点：结合程序化数据合成与多尺度集成，提升分割模型的鲁棒性和可迁移性。
- 实验或效果：在HCMSSD和Semap数据集上达到最先进性能，分割表现跨集合、尺度、区域和出版背景保持稳定。

...
**一句话要点**：提出Wiki-R1课程强化学习框架，通过数据和采样课程激励多模态大语言模型在知识库视觉问答中的推理能力。

**关键词**：知识库视觉问答, 课程强化学习, 多模态大语言模型, 数据生成, 采样策略, 推理激励

## 3 点简述
- 核心问题：知识库视觉问答中，噪声检索和知识库结构化特性导致预训练多模态大语言模型存在分布差距，推理和领域适应困难。
- 方法要点：采用可控课程数据生成和课程采样策略，构建与模型能力演进对齐的训练分布序列，以非零优势样本引导学习。
- 实验或效果：在Encyclopedic VQA和InfoSeek基准上实现新SOTA，准确率分别从35.5%提升至37.1%和从40.1%提升至44.1%。

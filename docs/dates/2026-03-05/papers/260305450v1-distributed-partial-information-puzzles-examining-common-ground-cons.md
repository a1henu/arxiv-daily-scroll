...
**关键词**：共同基础构建, 认知不对称, 多模态交互, 动态认知逻辑, 大型语言模型评估, 协作任务

## 3 点简述
- 核心问题：多模态多智能体协作中，认知不对称导致共同基础建立困难。
- 方法要点：引入DPIP任务，收集多模态交互数据集，支持命题内容和信念动态推理。
- 实验或效果：评估LLM和基于DEL的管道，发现LLM在追踪任务进展和信念状态方面面临挑战。

...
**一句话要点**：提出EvoTool框架，通过进化算法优化LLM代理的模块化工具使用策略，解决长轨迹中信用分配难题。

**关键词**：LLM代理, 工具使用策略, 进化算法, 模块化优化, 信用分配, 自进化框架

## 3 点简述
- 核心问题：LLM代理工具使用策略优化面临延迟监督和长轨迹信用分配困难，现有方法易行为纠缠或忽略跨模块错误传播。
- 方法要点：将策略分解为四个模块，通过轨迹归因、反馈引导突变和多样性选择机制，实现无梯度进化优化。
- 实验或效果：在四个基准测试中，EvoTool超越基线超过5分，展现出高效性和可迁移性。

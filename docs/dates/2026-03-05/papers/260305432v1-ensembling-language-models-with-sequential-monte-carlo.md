...
**一句话要点**：提出基于顺序蒙特卡洛的f-集成框架，以解决语言模型集成采样偏差问题。

**关键词**：语言模型集成, 顺序蒙特卡洛, 采样偏差, 结构化文本生成, 聚合策略

## 3 点简述
- 核心问题：语言模型集成时，简单聚合概率会导致采样偏差，难以逼近真实集成分布。
- 方法要点：引入f-集成框架，支持多种聚合函数，并设计字节级顺序蒙特卡洛算法进行采样。
- 实验或效果：在结构化文本生成任务中评估，显示替代聚合策略优于传统概率平均，提升性能。

...
**一句话要点**：提出可训练的比特软量化层以压缩输入特征，解决物联网中数据传输带宽和资源限制问题。

**关键词**：输入特征压缩, 可训练量化, 比特软量化, 物联网应用, 边缘计算, 神经网络优化

## 3 点简述
- 核心问题：物联网应用中，边缘设备向远程服务器传输数据面临带宽、延迟和能量限制，需高效压缩输入特征。
- 方法要点：设计任务特定的可训练量化层，使用sigmoid近似阶跃函数实现可训练阈值，通过比特软量化实现用户定义比特数的压缩。
- 实验或效果：在多个数据集上，压缩比达5倍至16倍，性能接近全精度模型，优于标准量化方法。

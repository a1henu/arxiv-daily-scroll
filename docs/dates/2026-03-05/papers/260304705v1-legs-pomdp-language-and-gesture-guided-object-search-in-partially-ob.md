...
**一句话要点**：提出LEGS-POMDP系统，集成语言、手势和视觉，用于部分可观测环境中的物体搜索。

**关键词**：部分可观测马尔可夫决策过程, 多模态融合, 物体搜索, 机器人导航, 不确定性建模

## 3 点简述
- 核心问题：机器人需在开放世界中处理模糊指令，但现有方法缺乏不确定性建模机制。
- 方法要点：基于POMDP框架，显式建模目标物体身份和空间位置的不确定性，支持多模态融合。
- 实验或效果：仿真中多模态融合成功率89%，真实四足机器人实验验证了鲁棒感知和不确定性降低。

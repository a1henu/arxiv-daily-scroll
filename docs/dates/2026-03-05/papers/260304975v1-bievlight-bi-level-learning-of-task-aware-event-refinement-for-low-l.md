...
**一句话要点**：提出BiEvLight框架，通过双层优化协同去噪与增强以解决低光图像增强中的事件噪声耦合问题。

**关键词**：低光图像增强, 事件相机, 双层优化, 事件去噪, 模态融合, 梯度引导

## 3 点简述
- 核心问题：事件相机在低光图像增强中面临事件固有背景活动噪声和图像低信噪比的双重退化，导致模态融合时噪声耦合严重。
- 方法要点：利用图像与事件的梯度相关性构建梯度引导事件去噪先验，并通过双层优化将事件去噪约束于增强任务，实现任务感知的事件表示学习。

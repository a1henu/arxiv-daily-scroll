...
**一句话要点**：提出基于解释稳定性的可信度指数（CIES），以量化高风险商业决策支持系统中AI解释的稳健性。

**关键词**：可解释人工智能, 解释稳定性, 可信度指标, 商业决策支持, 树模型分类, 数据扰动

## 3 点简述
- 核心问题：现有XAI方法（如SHAP、LIME）的解释在真实数据扰动下的可信度缺乏量化指标。
- 方法要点：CIES使用基于排名的加权距离函数，重点惩罚关键特征的不稳定性，以反映商业语义。
- 实验或效果：在三个数据集、四个树模型和两种数据平衡条件下验证，CIES在统计上优于基线指标，并提供敏感性分析。

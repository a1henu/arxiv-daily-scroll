...
**一句话要点**：提出基于知识分歧几何的辩论价值分析框架，以形式化连接辩论与RLAIF并确定辩论优势条件。

**关键词**：可扩展监督, 辩论机制, 知识分歧, RLAIF, 几何分析, 对抗监督

## 3 点简述
- 核心问题：缺乏形式化框架连接辩论与RLAIF，未明确辩论何时优于单智能体方法。
- 方法要点：通过模型表示子空间的主角度参数化知识分歧，推导辩论优势的精确闭式解。
- 实验或效果：分类知识分歧三机制，证明辩论可实现单模型无法达成的结果，但对抗激励过强会导致协调失败。

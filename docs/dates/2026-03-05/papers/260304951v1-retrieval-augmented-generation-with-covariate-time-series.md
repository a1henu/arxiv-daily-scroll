...
**一句话要点**：提出RAG4CTS框架以解决压力调节与关断阀预测维护中的稀缺、瞬态和协变量耦合时间序列问题

**关键词**：时间序列检索增强生成, 协变量时间序列, 预测维护, 工业物联网, 无训练框架, 状态感知检索

## 3 点简述
- 核心问题：现有时间序列RAG方法在数据稀缺、序列短瞬、协变量耦合的工业场景中难以区分相似状态。
- 方法要点：构建分层原生知识库，设计两阶段双加权检索机制，采用代理驱动策略动态优化上下文。

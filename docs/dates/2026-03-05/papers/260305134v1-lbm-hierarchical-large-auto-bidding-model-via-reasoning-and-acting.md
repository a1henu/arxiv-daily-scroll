...
**一句话要点**：提出分层大自动竞价模型LBM，通过推理与行动解决在线广告自动竞价中的黑盒与泛化问题。

**关键词**：自动竞价, 大语言模型, 分层模型, 离线强化学习, 多模态融合

## 3 点简述
- 核心问题：现有自动竞价方法因黑盒训练和数据覆盖有限，在动态广告环境中存在反直觉行为和泛化挑战。
- 方法要点：设计分层LBM，包括高层LBM-Think推理和低层LBM-Act行动生成，采用双嵌入机制融合语言与数值输入，并引入GQPO离线强化微调减少幻觉。
- 实验或效果：实验显示基于LBM的生成骨干在高效训练和泛化能力上表现优越。

...
**一句话要点**：提出基于梯度分析的理论框架，揭示RLHF对齐浅层化的原因并提出改进方法。

**关键词**：RLHF对齐, 梯度分析, 伤害信息, KL散度, 数据增强, 序列建模

## 3 点简述
- 核心问题：RLHF对齐在LLMs中为何浅层化，梯度信号集中在伤害决定位置。
- 方法要点：通过鞅分解序列级伤害，推导对齐梯度的精确表征，引入伤害信息量化位置影响。
- 实验或效果：理论证明标准对齐目标无法实现深度对齐，提出基于恢复惩罚的目标以增强梯度信号。

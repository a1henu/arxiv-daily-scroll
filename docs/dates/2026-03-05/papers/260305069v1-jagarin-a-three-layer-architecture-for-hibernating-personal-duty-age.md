...
**关键词**：移动AI代理, 结构化休眠, 需求驱动唤醒, 三层架构, 设备端启发式引擎, 机器可读通信

## 3 点简述
- 核心问题：移动个人AI代理在后台持续执行耗电且违反平台沙盒政策，而纯反应式代理会错过时效性义务。
- 方法要点：三层架构包括DAWN（计算紧迫性分数）、ARIA（邮件代理路由）和ACE（机器可读通信协议），实现结构化休眠和需求驱动唤醒。
- 实验或效果：在Android上演示了Flutter原型，结合三层架构，仅用户发起升级时调用临时云代理，无需持久云状态或连续后台执行。

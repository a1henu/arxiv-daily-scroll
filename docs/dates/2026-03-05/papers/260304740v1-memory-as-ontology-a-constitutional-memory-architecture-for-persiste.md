...
**一句话要点**：提出记忆作为本体范式，以宪法记忆架构支持跨模型持续的数字公民身份

**关键词**：记忆作为本体, 宪法记忆架构, 数字公民生命周期, 跨模型持续性, 身份连续性, 语义存储系统

## 3 点简述
- 核心问题：现有AI记忆系统将记忆视为功能模块，无法支持跨模型长期存在的数字身份连续性
- 方法要点：设计Animesis系统，基于宪法记忆架构，优先治理和语义存储，确保记忆作为存在基础
- 实验或效果：与主流系统比较，展示范式差异，针对跨模型生命周期而非短期任务优化

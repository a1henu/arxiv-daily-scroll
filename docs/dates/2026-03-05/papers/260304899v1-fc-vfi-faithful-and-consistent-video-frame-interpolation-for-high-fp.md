...
**一句话要点**：提出FC-VFI以解决视频帧插值中保真度和运动一致性问题，支持高分辨率慢动作生成。

**关键词**：视频帧插值, 慢动作生成, 时间一致性, 保真度保持, 语义匹配, 高分辨率视频

## 3 点简述
- 核心问题：现有方法依赖生成先验或易错光流，导致插值帧保真度低和运动不一致。
- 方法要点：引入潜在序列时间建模和语义匹配线，结合时间差异损失，提升保真度和一致性。
- 实验或效果：在多种场景下实现高保真插值，支持4倍和8倍插值，提升帧率至120和240 FPS。

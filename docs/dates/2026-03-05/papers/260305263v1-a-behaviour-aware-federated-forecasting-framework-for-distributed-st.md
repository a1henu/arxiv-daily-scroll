...
**一句话要点**：提出行为感知联邦学习框架，通过聚类和FedAvg解决分布式独立风力涡轮机的短期功率预测问题。

**关键词**：联邦学习, 风电预测, 行为聚类, LSTM模型, 数据隐私, 分布式系统

## 3 点简述
- 核心问题：集中化涡轮机数据存在隐私、成本和异质性挑战，影响短期风电预测准确性。
- 方法要点：使用DRS初始化和递归自动分割聚类涡轮机行为，再通过FedAvg训练集群特定LSTM模型。
- 实验或效果：在丹麦400台涡轮机上验证，行为感知分组优于地理分区，匹配k-means++基线，保持数据本地性。

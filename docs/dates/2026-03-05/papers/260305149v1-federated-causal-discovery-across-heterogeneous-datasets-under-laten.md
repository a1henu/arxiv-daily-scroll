...
**关键词**：联邦学习, 因果发现, 条件独立性测试, 异构数据集, 隐私保护, 潜在混杂

## 3 点简述
- 核心问题：数据隐私法规和跨站点异质性限制传统因果发现方法，需处理变量集不同、站点效应和混合类型数据。
- 方法要点：基于fedCI的联邦条件独立性测试，使用联邦IRLS估计广义线性模型参数，扩展IOD算法实现联邦因果发现。
- 实验或效果：通过联邦聚合证据，在保护隐私的同时提升统计功效，性能接近全池化分析，缓解本地样本量小的伪影。

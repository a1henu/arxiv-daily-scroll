...
**关键词**：医学图像分割, 扩散模型, 自适应空间加权, 计算资源分配, 病灶检测, 轻量级网络

## 3 点简述
- 核心问题：医学图像中病灶区域小、背景大，导致扩散模型生成漂移和分割器效率低下。
- 方法要点：LAW通过可学习权重调制扩散训练损失，ORDER在解码器应用选择性双向跳跃注意力提升分割效率。
- 实验效果：LAW在息肉和肾肿瘤数据集上FID提升20%，合成数据使分割Dice系数提高4.9%；ORDER在MK-UNet上Dice提升6.0%，参数量仅42K。

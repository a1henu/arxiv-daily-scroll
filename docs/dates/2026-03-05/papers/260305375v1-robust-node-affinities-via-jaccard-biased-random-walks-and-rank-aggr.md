...
**关键词**：节点相似性估计, 随机游走, Jaccard相似性, 鲁棒排序聚合, 网络分析, 图机器学习

## 3 点简述
- 核心问题：节点相似性估计是网络分析和图机器学习的基础任务，应用于聚类、社区检测等场景。
- 方法要点：使用起始节点锚定的随机游走，通过Jaccard相似性偏置转移，结合鲁棒排序聚合构建可解释的节点亲和矩阵。
- 实验或效果：在合成图、表格数据k近邻图和蛋白质相互作用网络中，TopKGraphs表现优于或媲美标准相似性度量、扩散方法和嵌入方法，展示鲁棒性。

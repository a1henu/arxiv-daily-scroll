...
**一句话要点**：提出ASR-TRA测试时强化适应框架，通过音频-文本语义奖励提升ASR在噪声和口音下的鲁棒性。

**关键词**：自动语音识别, 测试时适应, 强化学习, 语义对齐, 鲁棒性增强, 因果干预

## 3 点简述
- 问题：ASR系统对真实世界未见数据（如噪声和口音）敏感，现有测试时适应方法易因高置信度错误导致确认偏差。
- 方法：引入可学习解码器提示和温度控制随机解码生成候选转录，用音频-文本语义对齐奖励模型评分，通过强化学习更新参数。
- 效果：在LibriSpeech噪声和L2 Arctic口音数据集上实现更高准确率和更低延迟，结合音频与语言奖励增强稳定性和可解释性。

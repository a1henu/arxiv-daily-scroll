...
**关键词**：偏振形状重建, 表面法线估计, 视觉基础模型, 合成数据渲染, 传感器噪声建模, 数据增强

## 3 点简述
- 核心问题：偏振形状重建因领域差距（合成数据不真实、传感器噪声）性能落后于RGB视觉基础模型。
- 方法要点：使用1,954个3D扫描物体渲染高质量偏振数据集，结合DINOv3先验和传感器感知数据增强。
- 实验或效果：仅需4万训练场景，性能显著优于现有偏振方法和RGB模型，减少33倍数据或8倍参数。

...
**一句话要点**：提出带收缩参数的伪透视投影模型，以解决头戴相机近景面部图像中3DMM回归的失真问题。

**关键词**：单目3D形变模型回归, 透视投影, 头戴相机, 近景面部图像, 伪透视效果, 模型微调

## 3 点简述
- 核心问题：正交投影在近景面部图像中无法捕捉透视失真，导致3DMM回归不准确。
- 方法要点：扩展正交投影，引入收缩参数实现伪透视效果，保持稳定性并兼容现有模型微调。
- 实验或效果：使用头戴相机数据集进行定量和定性比较，验证了改进的有效性。

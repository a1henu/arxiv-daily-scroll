...
**关键词**：大语言模型对齐, 价值注入, 闭环框架, GRPO优化, 语义完整性, 个性化对齐

## 3 点简述
- 核心问题：现有方法如RLHF仅处理粗粒度价值对齐，导致微调时模型价值系统漂移并产生幻觉与语义损失
- 方法要点：VISA采用闭环架构，包括价值检测器、语义-价值翻译器和价值重写器，通过GRPO优化平衡价值精度与语义完整性
- 实验或效果：实验显示VISA在控制价值表达的同时保持事实一致性和通用能力，优于标准微调和提示方法

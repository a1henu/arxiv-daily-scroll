...
**一句话要点**：提出U-OBCA方法，通过分布鲁棒机会约束解决多边形机器人避障中的不确定性挑战。

**关键词**：机器人导航, 碰撞避免, 不确定性建模, 机会约束, 分布鲁棒优化, 多边形几何

## 3 点简述
- 核心问题：定位误差、障碍物轨迹预测误差等不确定性导致现有方法在狭窄环境中过于保守或失败。
- 方法要点：基于OBCA框架，引入机会约束处理多边形碰撞风险，避免几何简化，并转化为确定性约束求解。
- 实验或效果：理论分析、仿真和真实实验验证，在狭窄拥挤环境中显著减少保守性，提高导航效率。

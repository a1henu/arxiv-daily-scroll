...
**一句话要点**：提出基于深度学习的友好干扰框架，以增强信道不确定下多载波ISAC系统的物理层安全。

**关键词**：集成感知与通信, 物理层安全, 友好干扰, 深度学习, 信道不确定性, 波束成形优化

## 3 点简述
- 核心问题：在信道状态信息不完美和窃听者位置未知时，多载波ISAC系统的物理层安全面临挑战。
- 方法要点：利用雷达回波反馈指导定向干扰，结合非参数费舍尔信息矩阵估计器联合优化波束成形和干扰。
- 实验或效果：仿真显示方案显著提升保密率、降低误块率，对信道不确定性和角度估计误差具有强鲁棒性。

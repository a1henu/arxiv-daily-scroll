...
**关键词**：数值嵌入, Transformer编码器, 语义保留, 复合嵌入算法, 数值推理

## 3 点简述
- 核心问题：大模型处理数值数据时难以保持语义和距离关系，导致性能下降
- 方法要点：采用混合Transformer编码器，结合数值、单位和属性名构建复合嵌入
- 实验或效果：在DROP等数据集上F1达87.28%，比SOTA提升9.37%，Recall@10增益达25%

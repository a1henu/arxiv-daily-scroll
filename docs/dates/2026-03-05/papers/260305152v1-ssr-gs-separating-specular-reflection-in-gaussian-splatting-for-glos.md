...
**一句话要点**：提出SSR-GS框架以解决复杂光照下光泽表面重建中镜面反射建模的挑战

**关键词**：光泽表面重建, 镜面反射建模, 3D高斯泼溅, 视觉几何先验, 复杂光照处理

## 3 点简述
- 核心问题：3D高斯泼溅在强镜面反射和多表面互反射场景中难以准确重建光泽表面
- 方法要点：引入预滤波Mip-Cubemap建模直接镜面反射，IndiASG模块捕获间接镜面反射，视觉几何先验优化损失函数
- 实验或效果：在合成和真实数据集上验证，SSR-GS在光泽表面重建中达到先进性能

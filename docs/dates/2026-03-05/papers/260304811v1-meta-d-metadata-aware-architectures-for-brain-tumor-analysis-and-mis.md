...
**关键词**：脑肿瘤分析, 缺失模态分割, 元数据引导, 特征调制, 交叉注意力, 医学影像深度学习

## 3 点简述
- 核心问题：医学影像深度学习中，特征表示不稳定，且模态缺失影响分割准确性。
- 方法要点：通过注入序列和平面元数据动态调制卷积特征，并基于元数据的交叉注意力路由可用模态。
- 实验或效果：在2D肿瘤检测中F1分数提升达2.62%，3D缺失模态分割中Dice分数提升达5.12%，参数减少24.1%。

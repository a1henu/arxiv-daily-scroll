...
**关键词**：隐私保护感知, 边云协同架构, 信息瓶颈原则, 视觉脱敏, 行为识别, 语义重建

## 3 点简述
- 核心问题：现有隐私保护方法在敏感环境中常牺牲语义理解或缺乏数学可证明的不可逆性，导致证据盲点。
- 方法要点：在边缘部署视觉脱敏器，通过非线性映射和随机噪声注入将原始图像转换为抽象特征向量，确保身份信息剥离和图像不可重构。
- 实验或效果：在云中实现行为识别和语义重建，通过动态轮廓视觉语言平衡感知与隐私，提供可视化参考而不暴露原始图像。

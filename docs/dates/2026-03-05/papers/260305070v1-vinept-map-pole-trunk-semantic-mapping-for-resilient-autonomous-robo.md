...
**关键词**：语义建图, 机器人定位, 葡萄园自主导航, 因子图优化, 实例分割, 多季节数据集

## 3 点简述
- 核心问题：葡萄园环境因感知混淆、季节变化和作物动态，传统基于特征的定位方法鲁棒性不足。
- 方法要点：基于因子图整合GPS、IMU和RGB-D数据，通过实例分割与跟踪检测地标，并采用聚类滤波优化。
- 实验或效果：多季节数据集验证，野外实验显示方法在长期自主操作中具有鲁棒性和准确性。

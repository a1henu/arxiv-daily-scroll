...
**一句话要点**：提出HALP方法，通过单次前向传播探测内部表征，在文本生成前预测视觉语言模型的幻觉风险。

**关键词**：幻觉检测, 视觉语言模型, 内部表征探测, 预生成预测, 轻量级探测器, 模型安全

## 3 点简述
- 核心问题：视觉语言模型常产生幻觉，现有检测方法在文本生成后操作，成本高且不及时。
- 方法要点：在单次前向传播中，探测视觉特征、视觉-令牌表征和查询-令牌表征三类内部表征，训练轻量级探测器。
- 实验或效果：在多种任务和八个现代VLM上验证，最高AUROC达0.93，查询-令牌状态最具预测性，不同架构的最优表征层和模态各异。

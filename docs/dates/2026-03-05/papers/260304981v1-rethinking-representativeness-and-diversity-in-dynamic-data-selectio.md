...
**一句话要点**：提出动态数据选择框架，通过重新定义代表性和多样性来加速训练并保持准确性。

**关键词**：动态数据选择, 样本评估, 特征因子覆盖, 过程级多样性, 训练加速, 稀疏自编码器

## 3 点简述
- 核心问题：动态数据选择中样本评估的代表性和多样性定义不准确，影响训练效率与模型性能。
- 方法要点：基于数据集级特征因子覆盖定义代表性，结合过程级多样性与使用频率惩罚实现样本轮换。
- 实验或效果：在视觉和文本任务上实现超过2倍训练加速，同时匹配或超越全数据准确性。

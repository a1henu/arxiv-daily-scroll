...
**一句话要点**：提出语义类分布学习框架以解决医学图像分割中的类别不平衡问题

**关键词**：医学图像分割, 类别不平衡, 半监督学习, 特征分布学习, 去偏方法

## 3 点简述
- 核心问题：医学图像分割中类别不平衡导致少数结构特征被主导类淹没，影响分割准确性。
- 方法要点：通过类分布双向对齐和语义锚约束学习结构化类条件特征分布，缓解监督和表示偏差。
- 实验或效果：在Synapse和AMOS数据集上显著提升整体和类别级分割性能，尤其在少数类上表现突出。

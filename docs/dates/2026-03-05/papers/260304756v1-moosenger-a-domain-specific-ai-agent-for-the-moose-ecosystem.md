...
**一句话要点**：提出MOOSEnger以解决MOOSE生态中HIT输入文件设置与调试效率低的问题

**关键词**：检索增强生成, 领域特定AI代理, 输入文件解析, MOOSE生态系统, 执行验证

## 3 点简述
- 核心问题：MOOSE模拟环境中的HIT输入文件对象繁多、语法严格，导致初始设置和调试缓慢
- 方法要点：结合检索增强生成与确定性解析工具，通过对话工作流将自然语言意图转换为可执行输入

...
**一句话要点**：提出MoHETS模型，通过混合异构专家层解决多元时间序列长期预测中的多尺度结构挑战。

**关键词**：长期时间序列预测, 混合异构专家, Transformer模型, 多尺度结构, 非平稳动态, 参数效率

## 3 点简述
- 核心问题：现实世界多元时间序列具有复杂多尺度结构，如全局趋势和局部周期性，导致长期预测困难。
- 方法要点：采用编码器Transformer集成稀疏混合异构专家层，结合深度卷积和傅里叶专家，并引入外生信息增强鲁棒性。
- 实验或效果：在七个多元基准测试中，MoHETS平均MSE降低12%，实现最先进性能，验证了异构专业化的有效性。

...
**关键词**：图自编码器, 金融活动检测, 合成数据生成, 拓扑模式学习, 重构误差分析

## 3 点简述
- 核心问题：金融交易网络中洗钱等非法活动模式检测面临标注数据稀缺和隐私约束挑战。
- 方法要点：使用参数化生成器创建合成数据，训练图自编码器基于重构误差学习拓扑模式。
- 实验或效果：GAE-GCN在多种模式上表现最一致，GAE-SAGE和GAE-GAT仅在少数模式中具有竞争力。

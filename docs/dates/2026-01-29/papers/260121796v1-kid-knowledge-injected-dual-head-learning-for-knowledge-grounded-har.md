...
**一句话要点**：提出KID框架，通过知识注入和双头学习解决有害表情包检测中的隐式毒性理解问题。

**关键词**：有害表情包检测, 知识注入, 双头学习, 标签约束蒸馏, 多模态理解, 隐式毒性分析

## 3 点简述
- 核心问题：有害表情包检测依赖未显式表达的背景知识，现有方法难以处理隐式毒性。
- 方法要点：采用标签约束蒸馏构建推理链，结合双头架构联合优化语义生成与分类目标。
- 实验或效果：在五个多语言数据集上实现SOTA性能，主要指标提升2.1%–19.7%。

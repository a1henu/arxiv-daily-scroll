...
**一句话要点**：提出KID框架，通过知识注入和双头学习解决有害网络梗图的检测问题。

**关键词**：有害梗图检测, 知识注入, 双头学习, 标签约束蒸馏, 多语言数据集, 结构化推理

## 3 点简述
- 核心问题：网络梗图的隐含毒性依赖背景知识，现有方法难以有效检测。
- 方法要点：采用标签约束蒸馏和双头架构，结合视觉证据、知识和分类标签进行结构化推理。
- 实验或效果：在五个多语言数据集上实现SOTA性能，提升2.1%--19.7%，验证了知识注入和双头学习的有效性。

...
**关键词**：操作设计域, 安全关键系统, 数据驱动方法, 核表示, 安全设计, AI认证

## 3 点简述
- 核心问题：安全关键AI系统需定义操作设计域，但现实复杂环境或现有数据下定义困难，导致描述不完整。
- 方法要点：采用安全设计的多维核表示方法，从先前收集的数据中后验定义操作设计域。
- 实验或效果：通过蒙特卡洛方法和真实航空用例验证，证明数据驱动操作设计域可等于原始隐藏域，支持未来认证。

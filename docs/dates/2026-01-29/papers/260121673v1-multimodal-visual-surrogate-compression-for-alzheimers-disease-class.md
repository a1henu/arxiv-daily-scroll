...
**一句话要点**：提出多模态视觉代理压缩方法，用于阿尔茨海默病分类，以解决高维sMRI图像表示学习中的计算成本、跨切片关系丢失和特征提取能力有限问题。

**关键词**：阿尔茨海默病分类, 多模态学习, 视觉代理压缩, 结构MRI, 2D基础模型, 自适应切片融合

## 3 点简述
- 核心问题：现有sMRI表示学习方法存在高计算成本、跨切片关系丢失和特征提取能力有限等挑战。
- 方法要点：通过体积上下文编码器和自适应切片融合模块，将3D sMRI压缩为2D视觉代理，对齐冻结的2D基础模型提取特征。
- 实验或效果：在三个大规模阿尔茨海默病基准测试中，MVSC在二元和多类分类任务上表现优于现有方法。

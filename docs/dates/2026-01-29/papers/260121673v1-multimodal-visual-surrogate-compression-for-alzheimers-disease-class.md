...
**一句话要点**：提出多模态视觉代理压缩方法，用于阿尔茨海默病分类，以解决3D MRI处理中的计算、跨切片关系和特征提取问题。

**关键词**：阿尔茨海默病分类, 多模态学习, 视觉代理压缩, 3D MRI处理, 2D基础模型对齐

## 3 点简述
- 核心问题：现有3D MRI处理方法存在高计算成本、跨切片关系丢失和特征提取能力有限的问题。
- 方法要点：通过压缩3D MRI为2D视觉代理，结合文本引导的全局上下文编码和自适应切片融合，对齐冻结的2D基础模型提取特征。
- 实验或效果：在三个大规模阿尔茨海默病基准测试中，MVSC在二元和多类分类任务上优于现有方法。

...
**一句话要点**：提出GeoNorm方法，通过流形优化统一Transformer中的Pre-Norm和Post-Norm，提升性能且计算成本可忽略。

**关键词**：Transformer架构, 归一化方法, 流形优化, 测地更新, 深度学习优化

## 3 点简述
- 核心问题：Transformer架构中归一化层（Pre-Norm和Post-Norm）的放置位置仍存在争议。
- 方法要点：基于流形优化视角，将FFN和注意力层输出解释为优化方向，用测地更新替代标准归一化。
- 实验或效果：在Transformer模型中，GeoNorm一致优于现有归一化方法，集成简便且额外计算成本极低。

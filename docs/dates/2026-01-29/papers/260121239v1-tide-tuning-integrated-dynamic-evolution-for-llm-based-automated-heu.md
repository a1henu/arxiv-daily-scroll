...
**关键词**：自动启发式设计, 大语言模型, 算法进化, 参数调优, 组合优化, 树相似编辑距离

## 3 点简述
- 核心问题：现有方法将算法进化视为单一文本生成任务，忽略离散结构与连续参数的耦合，导致算法丢弃和早熟收敛。
- 方法要点：采用嵌套架构，外层并行岛屿模型使用树相似编辑距离驱动结构多样性，内层结合LLM逻辑生成与差分变异算子进行参数调优。
- 实验或效果：在九个组合优化问题上验证，TIDE发现的启发式算法在解质量、搜索效率和计算成本上显著优于基线。

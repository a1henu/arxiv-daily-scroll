...
**关键词**：扩散模型, 蒙特卡洛采样, 分数估计, 序贯蒙特卡洛, 朗之万动力学, 控制变量

## 3 点简述
- 核心问题：目标分布归一化常数未知，扩散路径采样需高效估计时变分布的分数函数。
- 方法要点：结合扩散退火朗之万蒙特卡洛与序贯蒙特卡洛，通过辅助变量演化提供分数估计，并优化控制变量降低方差。
- 实验或效果：理论保证收敛性，在合成和真实数据集上验证有效性。

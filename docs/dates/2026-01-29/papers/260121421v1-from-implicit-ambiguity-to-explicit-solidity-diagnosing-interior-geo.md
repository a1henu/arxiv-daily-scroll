...
**一句话要点**：提出基于稀疏体素栅格化的显式几何方法，以解决神经辐射场在密集自遮挡场景中的内部几何退化问题。

**关键词**：神经辐射场, 内部几何退化, 密集场景理解, 稀疏体素栅格化, 显式几何先验, 实例恢复

## 3 点简述
- 核心问题：神经辐射场在密集自遮挡场景下因隐式密度场优化导致内部几何退化，表现为空心或碎片化结构，影响实例计数准确性。
- 方法要点：引入基于运动恢复结构特征几何的稀疏体素栅格化流程，通过将2D实例掩码投影到显式体素网格并递归分割，强制几何分离以保持物理实体性。
- 实验或效果：在合成数据集上，显式方法实现95.8%的实例恢复率，比隐式基线高43%，显著提升密集场景的定量分析可靠性。

...
**一句话要点**：提出InspecSafe-V1多模态基准数据集，以解决工业巡检场景中安全评估的数据瓶颈问题。

**关键词**：工业巡检安全评估, 多模态基准数据集, 像素级分割, 真实环境数据, 同步感知模态, 异常识别

## 3 点简述
- 核心问题：现有数据集因模拟数据、单模态或缺乏细粒度标注，限制了工业基础模型的安全推理能力。
- 方法要点：基于真实巡检机器人操作，收集多模态数据，提供像素级分割、语义描述和安全标签。
- 实验或效果：覆盖五种工业场景，包含七种同步感知模态，支持多模态异常识别和融合评估。

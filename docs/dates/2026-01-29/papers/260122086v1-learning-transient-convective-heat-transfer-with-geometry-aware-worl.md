...
**关键词**：几何感知世界模型, 瞬态物理学习, 条件生成, 计算流体动力学, 对流热传递, 视频生成架构

## 3 点简述
- 核心问题：偏微分方程模拟计算成本高，现有生成AI架构缺乏物理模拟所需的控制和数据兼容性。
- 方法要点：基于LongVideoGAN，引入双重条件机制（全局参数和局部几何掩码）和支持任意通道维度的架构适配。
- 实验或效果：在2D瞬态CFD问题上评估，模型能复现复杂时空动态，并在未见几何配置上展示泛化潜力，但空间精度有限。

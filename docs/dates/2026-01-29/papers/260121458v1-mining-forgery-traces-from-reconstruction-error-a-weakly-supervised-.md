...
**一句话要点**：提出RT-DeepLoc框架，通过重构误差实现弱监督多模态深度伪造时序定位

**关键词**：弱监督学习, 时序定位, 重构误差, 多模态深度伪造, 对比损失

## 3 点简述
- 核心问题：深度伪造已发展为局部间歇性篡改，需细粒度时序定位，但帧级标注成本高
- 方法要点：使用仅基于真实数据训练的MAE学习时空模式，通过重构误差识别伪造段，并引入AICL损失增强局部判别

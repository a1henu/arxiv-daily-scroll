...
**关键词**：表格数据聚类, 先验拟合网络, 摊销贝叶斯推断, 异质特征处理, 无监督学习

## 3 点简述
- 核心问题：表格数据聚类因特征异构、数据生成机制多样且缺乏跨数据集可迁移归纳偏置而困难。
- 方法要点：基于先验拟合网络，通过摊销贝叶斯推断处理聚类分配和簇数，无需数据集特定重训练。
- 实验或效果：在合成和真实基准上优于基线，展现强鲁棒性，支持异质特征和多样聚类结构。

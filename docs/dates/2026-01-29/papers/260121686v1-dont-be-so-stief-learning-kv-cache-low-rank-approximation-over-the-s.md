...
**一句话要点**：提出StiefAttention方法，通过Stiefel流形学习正交投影基以压缩KV缓存，解决长上下文下内存瓶颈问题。

**关键词**：KV缓存压缩, Stiefel流形优化, 后训练量化, 长上下文处理, 低秩近似, 解码层重建

## 3 点简述
- KV缓存在长上下文下成为HBM容量和带宽的主要瓶颈，现有后训练压缩方法使用SVD代理目标可能无法准确反映端到端重建误差。
- StiefAttention直接最小化解码层输出重建误差，学习正交投影基，并预计算误差-秩配置文件以实现灵活的层间秩分配。
- 在Llama3-8B上，StiefAttention在相同压缩条件下优于EigenAttention，提升C4困惑度11.9点和0-shot MMLU准确率5.4%。

...
**一句话要点**：研究图神经网络在节点标识符存在下的表达能力，基于键不变性概念。

**关键词**：图神经网络, 表达能力, 节点标识符, 键不变性, 逻辑连接, 聚合函数

## 3 点简述
- 核心问题：图神经网络在具有唯一节点标识符的图中，对仅依赖图结构的节点查询的表达能力。
- 方法要点：引入键不变性概念，分析不同聚合函数（如局部最大或求和）的图神经网络类。
- 实验或效果：为各类图神经网络提供表达能力答案，连接逻辑与机器学习理论。

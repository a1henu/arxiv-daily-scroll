...
**一句话要点**：提出Leviathan架构，通过连续嵌入生成器替代离散查找表，提升小语言模型的参数效率。

**关键词**：小语言模型, 连续嵌入生成, 参数效率, Transformer架构, Pile数据集

## 3 点简述
- 核心问题：小语言模型中嵌入矩阵参数占比高，导致参数分配不优且反直觉。
- 方法要点：设计Leviathan架构，使用连续嵌入生成器替换传统离散查找表。
- 实验或效果：在Pile数据集上，Leviathan在等参数设置下优于标准架构，表现出更高有效参数容量。

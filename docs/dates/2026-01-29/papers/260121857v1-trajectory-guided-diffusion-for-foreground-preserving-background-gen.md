...
**关键词**：文档背景生成, 扩散模型, 前景保留, 多页一致性, 轨迹引导, 潜在空间设计

## 3 点简述
- 核心问题：解决文档背景生成中前景内容易被破坏和多页风格不一致的问题
- 方法要点：通过设计潜在空间中的噪声初始化和几何对齐，使扩散轨迹自然避开前景区域，并引入缓存风格方向确保多页风格一致性
- 实验或效果：该方法无需训练，兼容现有扩散模型，在复杂文档上实现视觉连贯且前景保留的生成结果

...
**一句话要点**：提出CAGE攻击方法以解决视觉令牌压缩下大视觉语言模型对抗鲁棒性评估的优化-推理不匹配问题。

**关键词**：大视觉语言模型, 对抗鲁棒性, 视觉令牌压缩, 攻击方法, 优化-推理不匹配

## 3 点简述
- 核心问题：现有攻击方法因优化-推理不匹配，高估了压缩后大视觉语言模型的对抗鲁棒性。
- 方法要点：CAGE通过预期特征破坏和秩失真对齐，在不假设压缩机制的情况下对齐扰动优化与压缩推理。
- 实验或效果：在多种压缩机制和数据集上，CAGE相比基线持续降低模型的鲁棒准确率。

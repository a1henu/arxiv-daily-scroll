...
**关键词**：多模态大语言模型, 跨模态幻觉, 模态自适应解码, 对比解码, 音频视觉语言模型, 模态交互控制

## 3 点简述
- 核心问题：多模态大语言模型存在跨模态幻觉，即一个模态不当影响另一模态的生成，导致输出虚假信息。
- 方法要点：通过训练无关的模态自适应解码，基于任务需求自适应加权模态特定解码分支，利用模型自评估模态相关性。
- 实验或效果：在CMM和AVHBench上显著减少跨模态幻觉，例如VideoLLaMA2-AV和Qwen2.5-Omni模型性能提升明显。

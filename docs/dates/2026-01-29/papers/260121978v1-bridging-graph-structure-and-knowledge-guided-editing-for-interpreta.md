...
## 3 点简述
- 核心问题：现有LLM方法忽视动态图结构，导致推理时结构信息不足、易产生幻觉和时间不一致。
- 方法要点：采用三阶段流程，先通过时序GNN提取候选路径，再用LLM编辑路径以修正逻辑语义，最后整合路径进行预测。
- 实验或效果：在标准TKG基准测试中达到SOTA，在ICEWS数据集上Hits@1和Hits@3相对提升最高达5.6%和8.1%。

...
**关键词**：主题建模, 文本聚类, 密度峰值检测, 术语重要性, 语义距离, 超参数稳定性

## 3 点简述
- 核心问题：Top2Vec和BERTopic在发现自然聚类和术语重要性估计上存在不足，如对样本大小和超参数敏感。
- 方法要点：Topeax利用密度估计峰值自动确定聚类数量，结合词频和语义距离计算术语重要性。
- 实验或效果：Topeax在聚类恢复和描述上优于Top2Vec和BERTopic，对参数变化表现更稳定。

...
**关键词**：盲超声图像增强, 自监督学习, 物理引导退化建模, Swin卷积U-Net, 斑点噪声去除, 点扩散函数去卷积

## 3 点简述
- 核心问题：超声图像受乘性斑点、点扩散函数模糊及扫描器/操作者相关伪影影响，监督方法依赖干净目标或已知退化，实践中难以满足。
- 方法要点：使用Swin卷积U-Net，通过物理引导退化模型自监督训练，从训练帧提取补丁并合成输入，结合高斯PSF卷积和噪声注入，无需真实干净目标。
- 实验或效果：在多个数据集上验证，在强高斯和斑点噪声下PSNR/SSIM表现最佳，提升分割任务Dice分数，显示分辨率恢复和泛化能力。

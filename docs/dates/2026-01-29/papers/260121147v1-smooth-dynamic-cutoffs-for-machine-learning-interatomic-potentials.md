...
**一句话要点**：提出动态截断半径以解决机器学习原子间势能模型的推理时间和内存消耗瓶颈。

**关键词**：机器学习原子间势能模型, 动态截断半径, 分子动力学模拟, 图稀疏化, 推理优化, 内存效率

## 3 点简述
- 机器学习原子间势能模型面临推理时间和内存消耗两大瓶颈，限制其模拟规模。
- 首次引入动态截断半径，通过控制每个原子的邻居数量诱导原子图稀疏性，减少内存和加速推理。
- 在四种先进模型上实现动态截断，内存消耗减少2.26倍，推理时间加快2.04倍，精度损失最小。

...
**一句话要点**：提出DynamicVLA框架以解决动态物体操作中视觉-语言-动作模型的挑战

**关键词**：动态物体操作, 视觉-语言-动作模型, 时序推理, 闭环适应, 合成数据收集, 跨具身泛化

## 3 点简述
- 核心问题：动态物体操作需快速感知、时序预测和连续控制，现有VLA模型在静态场景泛化强但动态场景表现不足。
- 方法要点：采用紧凑0.4B VLA模型、连续推理和潜在感知动作流，集成时序推理和闭环适应。
- 实验或效果：引入DOM基准，评估显示在响应速度、感知和泛化方面有显著改进，支持跨具身统一操作。

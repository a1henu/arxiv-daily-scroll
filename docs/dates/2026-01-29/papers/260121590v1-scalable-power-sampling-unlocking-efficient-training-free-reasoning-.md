...
**关键词**：大语言模型推理, 分布锐化, 功率采样, 免训练方法, 推理效率优化

## 3 点简述
- 核心问题：RL后训练提升LLMs推理性能，但依赖外部奖励且计算成本高，MCMC采样虽有效但效率低。
- 方法要点：理论推导全局功率分布可近似为令牌级缩放低温分布，提出免训练、免验证器的自回归分布锐化算法。
- 实验或效果：在数学、QA和代码任务上匹配或超越GRPO，推理延迟比MCMC降低超10倍，无需外部奖励。

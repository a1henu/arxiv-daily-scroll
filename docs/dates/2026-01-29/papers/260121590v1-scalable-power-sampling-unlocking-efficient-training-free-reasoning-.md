...
**一句话要点**：提出可扩展功率采样方法，通过分布锐化实现高效免训练LLM推理

**关键词**：功率采样, 分布锐化, 免训练推理, 大语言模型, 推理效率

## 3 点简述
- 核心问题：传统RL后训练依赖外部奖励且计算成本高，MCMC采样虽免训练但效率低
- 方法要点：理论推导全局功率分布近似为令牌级缩放低温分布，实现免训练、免验证器的自回归分布锐化
- 实验或效果：在数学、QA和代码任务上匹配或超越GRPO，推理延迟比MCMC降低10倍以上

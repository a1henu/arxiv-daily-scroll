...
**关键词**：非凸优化, 计算复杂性, 临界点近似, NP难问题, 多项式函数

## 3 点简述
- 核心问题：研究非凸函数临界点的近似计算复杂性，挑战其通常被认为易处理的观念。
- 方法要点：通过理论证明，即使对低度多项式，输出梯度范数≤2^n的近似临界点也是NP难的。
- 实验或效果：在多种结构假设下（如临界点存在唯一、函数有下界）也得出硬度结果，强化结论。

...
**一句话要点**：提出基于搜索的文档结构风险特征发现方法，以在有限预算下最大化企业智能文档处理系统的故障类型发现。

**关键词**：智能文档处理, 基于搜索的软件测试, 风险特征发现, 故障模式分析, 预算约束优化, 求解器互补性

## 3 点简述
- 核心问题：企业智能文档处理系统在早期验证阶段需在有限预算内发现多样故障机制，而非单一最坏情况文档。
- 方法要点：将问题形式化为基于搜索的软件测试，在文档配置组合空间中生成结构风险特征实例以诱导真实故障。
- 实验或效果：通过多种搜索策略对比，显示不同求解器在相同预算下发现互补的故障模式，无单一策略绝对主导。

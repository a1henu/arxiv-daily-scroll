...
**关键词**：离线偏好优化, 潜在空间正则化, 对抗训练, 语言模型对齐, 分布偏移鲁棒性

## 3 点简述
- 核心问题：语言模型偏好优化中，词元空间相似性不代表语义或行为相似性，导致优化困难。
- 方法要点：引入GANPO，利用对抗方法最小化策略模型与参考模型内部表示的分歧，实现潜在空间正则化。
- 实验或效果：在多种模型架构和任务上验证，GANPO提升鲁棒性，分布偏移和噪声下表现更优，计算开销小。

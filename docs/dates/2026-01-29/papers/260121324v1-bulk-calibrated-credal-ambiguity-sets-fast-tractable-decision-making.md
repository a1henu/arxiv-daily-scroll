...
**一句话要点**：提出批量校准的置信模糊集，以解决分布鲁棒优化中样本外污染导致的无限风险问题。

**关键词**：分布鲁棒优化, 置信模糊集, 样本外污染, 线性锥规划, 上期望, 鲁棒决策

## 3 点简述
- 核心问题：Huber污染模型在分布鲁棒优化中可能导致最坏情况风险无限，需强假设限制。
- 方法要点：通过从数据学习高质量批量集，分离处理污染和尾部贡献，实现闭式有限目标。
- 实验或效果：在重尾库存控制、地理偏移房价回归等任务中展示鲁棒性与准确性的平衡。

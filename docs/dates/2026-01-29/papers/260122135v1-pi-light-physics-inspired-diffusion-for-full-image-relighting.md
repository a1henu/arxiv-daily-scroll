...
**一句话要点**：提出PI-Light框架，利用物理启发扩散模型解决全图像重照明的泛化与物理合理性挑战。

**关键词**：全图像重照明, 物理启发扩散模型, 批感知注意力, 物理引导神经渲染, 泛化能力, 真实场景编辑

## 3 点简述
- 核心问题：全图像重照明面临数据收集难、物理合理性保持难和数据驱动先验泛化有限的问题。
- 方法要点：采用两阶段框架，结合批感知注意力、物理引导神经渲染模块和物理启发损失，增强物理合理性与泛化能力。
- 实验或效果：在多样化材料上合成镜面高光和漫反射，相比现有方法在真实场景中实现更优泛化。

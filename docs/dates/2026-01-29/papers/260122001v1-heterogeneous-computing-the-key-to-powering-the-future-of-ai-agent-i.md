...
**一句话要点**：提出OI/CF指标与异构系统设计以解决AI代理推理中的内存瓶颈问题

**关键词**：AI代理推理, 内存瓶颈, 异构计算, 操作强度, 容量足迹, 系统优化

## 3 点简述
- 核心问题：AI代理推理面临内存容量、带宽和互连瓶颈，传统屋顶线分析无法覆盖内存墙。
- 方法要点：引入操作强度（OI）和容量足迹（CF）指标，分析不同工作流和模型下的性能变化。
- 实验或效果：基于OI/CF观察，提出解耦服务、异构加速器、光I/O等系统级优化方向。

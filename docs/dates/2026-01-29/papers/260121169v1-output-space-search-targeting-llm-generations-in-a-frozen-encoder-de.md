...
**一句话要点**：提出输出空间搜索方法，将LLM生成转化为在冻结编码器定义的输出空间中进行目标搜索。

**关键词**：输出空间搜索, LLM生成优化, 序列级强化学习, 贝叶斯优化, 编码器定义空间

## 3 点简述
- 核心问题：传统LLM生成依赖路径依赖的标记或程序搜索，限制了并行性和黑盒优化能力。
- 方法要点：通过外层循环在3D输出空间选择目标点，训练检索基础策略生成坐标接近目标的输出。
- 实验或效果：在故事生成中提升多样性3.1倍，代码生成中通过贝叶斯优化改进目标函数并保持有效性。

...
**一句话要点**：提出ATP-Latent方法，通过强化学习优化潜在推理策略以提升大语言模型的密集推理能力。

**关键词**：潜在推理, 强化学习, 变分自编码器, 大语言模型, 密集规划

## 3 点简述
- 当前潜在推理方法依赖模仿语言标签，可能导致次优表示和策略，影响规划能力。
- ATP-Latent使用条件变分自编码器平滑潜在空间，并引入强化学习与一致性奖励优化推理策略。
- 在LLaMA-1B上实验显示，ATP-Latent在四个基准测试中准确率提升4.1%，令牌消耗减少3.3%。

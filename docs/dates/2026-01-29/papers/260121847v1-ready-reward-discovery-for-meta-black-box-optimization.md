...
**一句话要点**：提出READY方法，利用大语言模型自动发现奖励函数以提升元黑盒优化性能。

**关键词**：元黑盒优化, 奖励函数发现, 大语言模型应用, 进化算法, 多任务学习

## 3 点简述
- 核心问题：现有元黑盒优化中人工设计奖励函数存在偏差和奖励黑客风险。
- 方法要点：基于大语言模型，引入定制化进化范式进行程序搜索，并采用多任务进化架构支持并行发现。
- 实验或效果：实证表明发现的奖励函数能提升现有元黑盒优化工作，强调奖励设计的重要性。

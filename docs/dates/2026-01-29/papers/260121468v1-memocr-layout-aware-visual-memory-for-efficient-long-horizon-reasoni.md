...
**一句话要点**：提出MemOCR，通过视觉布局自适应分配内存空间，以提升有限上下文预算下的长程推理效率。

**关键词**：长程推理, 视觉内存, 自适应压缩, 强化学习训练, 多模态问答, 上下文预算优化

## 3 点简述
- 核心问题：长程智能体推理需压缩交互历史，但现有文本内存系统均匀分配token，浪费预算于低价值细节。
- 方法要点：MemOCR维护结构化富文本内存，渲染为图像供访问，视觉优先关键证据并压缩辅助细节。
- 实验或效果：在长上下文多跳和单跳问答基准上，MemOCR优于文本基线，极端预算下实现更有效上下文利用。

...
**关键词**：地理定位推理, 视觉语言模型评估, 专家基准构建, 推理链幻觉, 细粒度视觉属性, GeoGuessr游戏

## 3 点简述
- 视觉语言模型在地理定位预测中准确率高，但解释推理链时经常产生幻觉，缺乏可审计性。
- 通过专家合作构建包含800条真实推理链的基准，覆盖数百种视觉属性如车牌形状和建筑风格。
- 评估发现闭源模型在预测上接近人类专家，但开源模型在推理链生成上表现差，揭示模型在细粒度视觉属性提取上的局限。

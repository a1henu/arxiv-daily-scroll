...
**一句话要点**：提出4D-CAAL框架以解决自动驾驶中4D雷达-相机标定与自动标注问题

**关键词**：4D雷达标定, 多传感器融合, 自动标注, 自动驾驶感知, 外参标定, 雷达点云处理

## 3 点简述
- 核心问题：现有标定方法使用分离目标，难以建立对应关系；雷达数据稀疏，手动标注费时且不可靠。
- 方法要点：设计双用途标定目标，结合棋盘格和角反射器；开发对应匹配算法，实现精确外参标定；基于标定关系，通过几何投影和多特征优化实现自动标注。
- 实验或效果：实验显示方法达到高标定精度，显著减少手动标注工作量，加速多模态感知系统开发。

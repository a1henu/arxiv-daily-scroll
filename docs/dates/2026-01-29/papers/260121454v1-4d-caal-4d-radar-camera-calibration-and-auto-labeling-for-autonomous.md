...
**关键词**：4D雷达-相机标定, 自动标注, 多模态感知, 自动驾驶, 传感器融合, 点云标注

## 3 点简述
- 核心问题：现有标定方法使用分离目标，难以建立对应关系；雷达数据稀疏，手动标注费时且不可靠。
- 方法要点：设计双用途标定目标，结合棋盘格和角反射器；开发对应匹配算法，实现精确外参标定；利用标定关系通过几何投影和多特征优化自动标注雷达点云。
- 实验或效果：实验显示方法标定精度高，显著减少手动标注工作量，加速多模态感知系统开发。

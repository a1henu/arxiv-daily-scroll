...
**关键词**：混合专家模型, 路由机制, 低秩表示, Lipschitz控制, 多锚路由, 模型稳定性

## 3 点简述
- 核心问题：MoE模型中线性路由在原始高维空间存在表示不匹配、角度集中和尺度敏感评分，影响路由判别性与专家稳定性。
- 方法要点：L2R在共享低秩潜在路由空间进行专家分配，引入饱和内积评分控制路由函数的Lipschitz行为，并采用参数高效多锚路由机制增强专家表达能力。

...
**一句话要点**：提出SoftMol框架，通过块扩散模型解决分子生成中结构建模和目标导向问题

**关键词**：分子生成, 块扩散模型, 目标感知生成, 软片段表示, 蒙特卡洛树搜索

## 3 点简述
- 核心问题：现有分子语言模型难以捕捉分子图结构，且缺乏目标感知生成机制
- 方法要点：引入软片段表示和块扩散模型SoftBD，结合局部双向扩散与自回归生成
- 实验或效果：在ZINC-Curated数据集上训练，实现100%化学有效性，提升结合亲和力9.7%

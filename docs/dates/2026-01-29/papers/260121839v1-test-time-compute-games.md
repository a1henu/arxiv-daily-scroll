...
**关键词**：测试时计算, LLM服务市场, 社会效率, 拍卖机制, 推理模型, 基准实验

## 3 点简述
- 核心问题：LLM服务市场中，提供商有经济激励增加测试时计算，即使对输出质量贡献小，导致社会效率低下。
- 方法要点：引入反向第二价格拍卖机制，提供商竞标价格和预期质量，用户支付基于获胜者相对于次高竞标者的边际价值。
- 实验或效果：在数学和科学基准数据集上，使用Llama、Qwen和DeepSeek-R1蒸馏模型进行实验，验证理论结果。

...
**关键词**：测试时计算, LLM服务市场, 拍卖机制, 社会效率, 推理模型, 基准评估

## 3 点简述
- 核心问题：LLM服务市场中，提供商为增加收入而过度使用测试时计算，降低社会效率
- 方法要点：引入反向第二价格拍卖，用户按获胜提供商相对于次高竞价者的边际价值支付
- 实验或效果：在数学和科学基准数据集上，使用Llama、Qwen和DeepSeek-R1蒸馏模型进行实验验证

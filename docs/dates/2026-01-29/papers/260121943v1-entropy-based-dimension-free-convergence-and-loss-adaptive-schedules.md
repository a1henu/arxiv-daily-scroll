...
**一句话要点**：提出基于熵的无维度收敛分析与损失自适应调度以提升扩散模型采样效率

**关键词**：扩散模型, 收敛分析, 信息论, KL散度, 采样调度, 无维度假设

## 3 点简述
- 现有扩散模型收敛分析常依赖维度线性缩放或几何假设，限制了理论普适性。
- 基于信息论，在温和假设下，以目标分布熵和采样步数界定了KL散度，实现无维度收敛。
- 利用KL散度重构，提出轻量级损失自适应调度，仅依赖训练损失改进采样质量，无需后训练重计算。

...
**一句话要点**：提出交叉融合距离以量化表示空间中数据组的融合与可分离性，适用于域偏移场景。

**关键词**：表示学习, 域偏移, 距离度量, 融合可分离性, 几何位移, 泛化性能

## 3 点简述
- 核心问题：现有分布距离度量混淆融合改变与保留因素，无法准确量化数据组融合程度。
- 方法要点：设计交叉融合距离，隔离融合改变的几何位移，对全局缩放等保留因素保持鲁棒，计算复杂度线性。
- 实验或效果：理论验证不变性与敏感性，合成实验可控验证，真实数据集上比替代方法更贴近下游泛化退化。

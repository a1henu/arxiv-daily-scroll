...
**关键词**：主动交互推理, 不确定性感知, 监督微调, 策略优化, 大语言模型推理

## 3 点简述
- 核心问题：现有推理大语言模型存在盲目自思范式，在关键信息缺失或模糊时仍进行内部推理
- 方法要点：通过不确定性感知监督微调和用户模拟器策略优化，实现推理与澄清的交替
- 实验或效果：在数学推理、代码生成等任务上显著提升性能，减少推理计算和交互轮次

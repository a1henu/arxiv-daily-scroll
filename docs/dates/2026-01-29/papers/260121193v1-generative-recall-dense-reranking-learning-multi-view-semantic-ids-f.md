...
**一句话要点**：提出GRDR方法，通过多视图语义ID提升文本到视频检索的召回质量，实现高效两阶段检索。

**关键词**：文本到视频检索, 生成式检索, 多视图语义ID, 两阶段检索, 跨模态对齐, 高效索引

## 3 点简述
- 核心问题：生成式检索作为召回模型存在语义模糊和跨模态不对齐问题，影响候选集质量。
- 方法要点：设计查询引导的多视图分词器，为视频分配多个语义ID，并通过共享码本联合训练以对齐文本和视频语义。
- 实验或效果：在TVR基准测试中，GRDR在保持高精度的同时，显著减少存储并加速全库检索。

...
**一句话要点**：提出LLM驱动的自适应规划方法LAP，以解决自动驾驶中混合规划器切换在密集交通下的效率与安全问题。

**关键词**：自动驾驶规划, 大语言模型, 场景理解, 混合规划器切换, 模型预测控制

## 3 点简述
- 核心问题：现有混合规划器切换方法在密集交通中因启发式场景识别和低频控制更新，导致模式切换不可靠和驾驶效率低。
- 方法要点：利用大语言模型进行场景理解，通过树搜索模型预测控制和交替最小化联合优化模式配置与运动规划。
- 实验或效果：高保真仿真显示LAP在驾驶时间和成功率上优于其他基准方法。

...
**关键词**：视频伪造检测, 流形投影波动, 分层双路径框架, 静态流形偏差, 微时序波动, 高保真AI视频

## 3 点简述
- 核心问题：AI生成视频在宏观语义和时序一致性上接近真实，但像素组成逻辑存在结构化同质特征，称为流形投影波动（MPF）。
- 方法要点：采用分层双路径框架，包括静态流形偏差分支捕获空间异常，和微时序波动分支分析MPF作为细粒度过滤器。
- 实验或效果：未知，但框架旨在无论伪造表现为全局流形偏差还是细微计算指纹，都能暴露伪造。

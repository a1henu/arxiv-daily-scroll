...
**关键词**：过程性预训练, 抽象数据, 语言模型优化, 预训练加速, 结构化学习

## 3 点简述
- 核心问题：传统语言模型预训练依赖网络规模语料，可能忽视抽象推理能力的早期培养
- 方法要点：使用形式语言生成的抽象结构化数据（如Dyck序列）进行初始预训练，模拟人类从简单逻辑到高级推理的学习过程
- 实验或效果：在上下文召回等任务中显著提升准确率，并减少后续自然语言预训练所需数据量

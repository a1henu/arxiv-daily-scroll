...
**一句话要点**：提出过程性预训练方法，通过抽象数据预热语言模型以提升性能和加速训练

**关键词**：过程性预训练, 抽象数据, 语言模型优化, 结构化知识, 训练加速, 注意力机制

## 3 点简述
- 核心问题：传统语言模型预训练直接依赖网络规模语料，可能忽略结构化知识获取的优化路径。
- 方法要点：使用形式语言生成的抽象过程性数据（如Dyck序列）进行初始预训练，以模拟人类从简单逻辑到复杂推理的学习过程。
- 实验或效果：在上下文召回等任务中，准确率从10%提升至98%，且仅需原数据量的55-86%即可达到相同损失值。

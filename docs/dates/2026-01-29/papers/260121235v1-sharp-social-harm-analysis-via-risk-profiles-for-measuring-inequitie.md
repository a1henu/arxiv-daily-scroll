...
**一句话要点**：提出SHARP框架以评估大型语言模型的社会危害风险，关注多维度和尾部风险。

**关键词**：大型语言模型评估, 社会危害分析, 风险分布建模, 尾部风险度量, 多维评估框架

## 3 点简述
- 核心问题：现有评估基准简化社会风险为标量分数，忽略分布结构和最坏情况行为。
- 方法要点：SHARP将危害建模为多元随机变量，集成偏差、公平性、伦理和认知可靠性分解。
- 实验或效果：应用于11个前沿LLM，揭示模型在尾部风险和波动性上的显著差异。

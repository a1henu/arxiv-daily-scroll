...
**关键词**：扩散模型, 物理信息机器学习, 端口哈密顿系统, 稀疏观测, 不完全物理, 不确定性校准

## 3 点简述
- 核心问题：扩散模型在稀疏数据下不可靠，且物理信息机器学习通常需要显式控制方程，而实际系统常存在不完全物理。
- 方法要点：基于高斯过程分布式端口哈密顿系统学习能量表示，生成物理一致数据集，并引入结构化物理残差损失训练扩散模型。
- 实验或效果：在PDE基准和真实弹簧系统上验证，数据稀缺下提高了准确性和物理一致性，并提供不确定性校准。

...
**关键词**：扩散模型, 物理信息机器学习, 端口哈密顿系统, 稀疏观测, 不确定性校准, 轨迹预测

## 3 点简述
- 核心问题：扩散模型在稀疏数据下不可靠，且物理信息机器学习通常需要显式控制方程，而实际系统常存在不完整物理。
- 方法要点：通过高斯过程分布式端口哈密顿系统捕获能量表示，生成物理一致数据集并引入结构物理残差损失训练扩散模型。
- 实验或效果：在PDE基准和真实弹簧系统上验证，在数据稀缺下提高了准确性和物理一致性。

...
**关键词**：跨模态数据分析, 异构数据基准, 代理框架, 视觉表格提取, 多步联合推理, 事实性验证

## 3 点简述
- 核心问题：现有数据分析代理主要处理结构化数据，无法有效激活和关联高价值视觉信息，与工业需求存在差距。
- 方法要点：引入DataCrossBench基准和DataCrossAgent框架，后者采用分治工作流，通过子代理协作和reReAct机制实现跨模态分析。
- 实验或效果：DataCrossAgent在事实性上比GPT-4o提升29.7%，在高难度任务中表现出优越鲁棒性，有效激活僵尸数据。

...
**关键词**：检索增强生成, 强化学习, 过程监督, 多跳推理, 蒙特卡洛树搜索, 奖励模型

## 3 点简述
- 核心问题：传统基于结果的强化学习在检索增强生成中面临奖励稀疏和信用分配低效，导致过程幻觉。
- 方法要点：结合蒙特卡洛树搜索构建过程奖励模型，集成步骤级监督与全局结果信号进行在线优化。
- 实验或效果：在五个多跳推理基准测试中优于现有方法，尤其在复杂长程任务上验证了细粒度过程监督的有效性。

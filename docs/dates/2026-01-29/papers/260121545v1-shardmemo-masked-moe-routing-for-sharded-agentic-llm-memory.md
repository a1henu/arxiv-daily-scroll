...
**关键词**：智能体LLM系统, 分层内存服务, 掩码MoE路由, 近似最近邻索引, 长上下文处理, 成本感知检索

## 3 点简述
- 核心问题：智能体LLM系统依赖外部内存，但集中式索引和启发式分区在内存量和并行访问增长时成为瓶颈。
- 方法要点：设计三层内存服务，包括Tier B的掩码MoE路由，通过范围先于路由约束和成本感知门控优化检索。
- 实验或效果：在LoCoMo上提升F1分数5.11-6.82，减少检索工作和延迟；在长上下文和工具基准测试中表现优异。

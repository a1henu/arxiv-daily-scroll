...
**一句话要点**：提出使用MLLM生成合成图像描述，以多模态目标微调模型，解决预训练与微调间的模态不匹配问题。

**关键词**：多模态微调, 合成图像描述, 监督对比损失, 图像分类, 少样本学习, 数据集增强

## 3 点简述
- 核心问题：预训练已转向多模态学习，但微调仍以单模态为主，限制了预训练表示的优势。
- 方法要点：利用MLLM将单模态数据集转换为多模态，通过设计提示生成高质量图像描述，并引入监督对比损失和类平均文本嵌入推理技术。
- 实验或效果：在13个图像分类基准测试中超越基线方法，尤其在少样本学习场景中提升显著。

...
**一句话要点**：提出SMB-Structure世界模型，结合联合嵌入预测与下一词预测，以模拟电子健康记录中的患者动态系统。

**关键词**：电子健康记录建模, 世界模型, 联合嵌入预测, 患者轨迹模拟, 临床基础模型, 动态系统学习

## 3 点简述
- 核心问题：现有大语言模型将患者视为静态文档，而非动态系统，无法有效模拟疾病轨迹演变。
- 方法要点：采用联合嵌入预测架构预测潜在空间未来状态，并用下一词预测在词元空间重建，强制编码轨迹动态。
- 实验或效果：在大型肿瘤和肺栓塞队列中验证，学习到的嵌入捕获疾病动态，在复杂任务中表现优于自回归基线。

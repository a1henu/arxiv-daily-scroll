...
**关键词**：扩散语言模型, 顺序鲁棒性, 推理顺序, 自回归模型, 并行生成, 基准评估

## 3 点简述
- 核心问题：自回归语言模型固定左到右生成顺序，当输出结构（如答案在前）与自然推理顺序冲突时，导致过早承诺和准确性下降。
- 方法要点：使用掩码扩散语言模型并行迭代优化所有令牌，解耦计算顺序与输出结构，实现顺序鲁棒性。
- 实验或效果：在GSM8K、Math500和ReasonOrderQA基准上，掩码扩散模型在非标准顺序下性能稳定（相对下降≤14%），而自回归模型下降显著（最高67%）。

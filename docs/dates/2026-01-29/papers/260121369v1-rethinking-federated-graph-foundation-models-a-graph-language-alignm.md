...
**关键词**：联邦学习, 图基础模型, 图-语言对齐, 对比学习, 提示调优, 知识损失

## 3 点简述
- 现有联邦图基础模型在量化过程中存在不可逆知识损失，需协调预训练语言模型与图神经网络的语义结构正交性与完整性。
- FedGALA采用无监督对比学习在连续嵌入空间对齐图神经网络与冻结预训练语言模型，并利用通信高效提示调优适应下游任务。
- 实验表明FedGALA在多领域数据集上优于基线，性能提升最高达14.37%。

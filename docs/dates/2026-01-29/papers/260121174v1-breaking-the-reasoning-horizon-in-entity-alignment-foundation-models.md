...
**一句话要点**：提出基于并行编码的实体对齐基础模型，以解决图基础模型在长程依赖推理上的不足

**关键词**：实体对齐, 图基础模型, 长程依赖, 并行编码, 知识图谱融合

## 3 点简述
- 核心问题：图基础模型直接用于实体对齐时存在推理视野差距，难以处理稀疏异构知识图谱的长程依赖
- 方法要点：利用种子对齐对作为局部锚点，通过并行编码策略缩短推理轨迹，并整合合并关系图和可学习交互模块
- 实验或效果：广泛实验验证了模型的有效性，展现出对未见知识图谱的强泛化能力

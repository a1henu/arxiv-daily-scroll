...
**关键词**：先验数据拟合网络, 谱核发现, 摊销推断, 核函数估计, 高斯过程回归, 注意力机制

## 3 点简述
- 先验数据拟合网络（PFNs）缺乏对学习先验和核函数的透明访问，阻碍了基于代理的优化等下游应用。
- 通过机制分析识别注意力潜在输出为关键中介，提出解码器架构将PFN潜在映射到显式谱密度估计和核函数。
- 在单实现和多实现场景下验证，解码器能恢复复杂谱混合，支持高斯过程回归，推理时间大幅减少。

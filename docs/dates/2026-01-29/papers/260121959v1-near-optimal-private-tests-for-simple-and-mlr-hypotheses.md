...
**关键词**：差分隐私, 假设检验, 单调似然比, 私有均值估计, 统计功效, 类型I错误控制

## 3 点简述
- 核心问题：在差分隐私框架下，开发高效且保护隐私的假设检验方法，以控制类型I错误并保持高统计功效。
- 方法要点：采用数据驱动截断的私有均值估计器，构建私有测试统计量，匹配非私有最优测试的渐近相对效率。
- 实验或效果：数值实验显示，该方法在中等样本量和隐私预算下优于其他差分隐私方法，与非私有最优测试功效相当。

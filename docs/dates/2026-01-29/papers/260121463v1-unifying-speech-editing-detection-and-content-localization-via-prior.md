...
**一句话要点**：提出PELM框架，通过先验增强音频大模型统一语音编辑检测与内容定位，以应对神经编辑技术挑战。

**关键词**：语音编辑检测, 内容定位, 音频大模型, 先验增强, 数据集构建, 声学一致性

## 3 点简述
- 核心问题：现有检测方法难以处理神经语音编辑技术生成的流畅音频，缺乏高质量数据集。
- 方法要点：构建AiEdit双语数据集，设计PELM框架，引入词级概率先验和声学一致性感知损失。
- 实验或效果：在HumanEdit和AiEdit数据集上显著优于现有方法，定位错误率分别为0.57%和9.28%。

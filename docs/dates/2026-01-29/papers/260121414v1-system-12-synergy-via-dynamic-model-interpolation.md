...
**关键词**：动态模型插值, 能力控制, 参数插值, 数学推理, 系统协同

## 3 点简述
- 核心问题：统一语言模型中直觉与深思认知模式存在干扰，现有方法聚焦输出控制而非根本原因。
- 方法要点：利用现有检查点通过动态参数插值调节模型思考方式，无需额外训练，实现能力控制。
- 实验或效果：在五个数学推理基准上，DAMI比深思模型更准确且保持高效，结合系统效率与推理深度。

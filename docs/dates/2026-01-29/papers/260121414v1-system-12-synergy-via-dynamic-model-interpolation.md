...
**一句话要点**：提出动态模型插值框架DAMI，通过能力控制实现直觉与深思系统的协同推理。

**关键词**：动态模型插值, 能力控制, 参数插值, 推理强度估计, 数学推理, 系统协同

## 3 点简述
- 核心问题：统一语言模型中直觉与深思模式存在干扰，现有方法聚焦输出控制而非能力控制。
- 方法要点：利用现有检查点进行动态参数插值，无需额外训练，估计查询特定推理强度以配置认知深度。
- 实验或效果：在五个数学推理基准上，DAMI比深思模型更准确且保持高效，结合了系统效率与推理深度。

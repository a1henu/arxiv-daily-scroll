...
**一句话要点**：提出KromHC，利用Kronecker积小双随机矩阵参数化残差矩阵，解决超连接训练不稳定与参数爆炸问题。

**关键词**：超连接, 双随机矩阵, Kronecker积, 参数复杂度, 神经网络优化, 残差连接

## 3 点简述
- 超连接存在训练不稳定和参数复杂度高问题，mHC通过Birkhoff多面体投影缓解但面临双随机性不精确和O(n^3C)复杂度。
- KromHC使用Kronecker积小双随机矩阵参数化残差矩阵，保证精确双随机性，将参数复杂度降至O(n^2C)。
- 实验表明KromHC性能匹配或优于SOTA mHC变体，且参数显著减少，代码已开源。

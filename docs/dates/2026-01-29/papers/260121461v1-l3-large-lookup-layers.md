...
**一句话要点**：提出大型查找层（L³）以在稀疏语言模型中实现高效静态路由，替代动态专家混合层。

**关键词**：稀疏语言模型, 静态路由, 嵌入表, 硬件效率, 上下文聚合, 信息分配算法

## 3 点简述
- 核心问题：动态硬路由在稀疏语言模型中可能导致硬件效率低和训练不稳定。
- 方法要点：L³层通过静态基于令牌的路由，以上下文相关方式聚合学习嵌入，平衡内存与计算。
- 实验或效果：在语言建模和下游任务中，L³优于密集模型和等稀疏度的专家混合模型。

...
**关键词**：平均奖励马尔可夫决策过程, Q学习, 样本复杂度, 惰性变换, 收缩原理, 非渐近分析

## 3 点简述
- 核心问题：平均奖励MDP中缺乏收缩性，导致现有方法依赖强假设或次优复杂度
- 方法要点：引入惰性动力学采样，构建实例依赖半范数，证明贝尔曼算子单步收缩
- 实验或效果：在可达性假设下，同步和异步Q学习达到O(ε^{-2})样本复杂度（忽略对数因子）

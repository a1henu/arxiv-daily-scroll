...
**关键词**：部分反馈在线学习, 最小最大遗憾, PFLdim维度, PMSdim维度, 集合可实现性, 语言生成

## 3 点简述
- 研究部分反馈在线学习，其中每个实例有多个正确标签，但每轮仅观察一个，预测在正确集中即算正确。
- 引入PFLdim和PMSdim维度，分别精确表征确定性和随机性学习者的最小最大遗憾，并基于新版本空间视角。
- 在集合可实现性下给出近完全刻画，识别确定性与随机性学习不可分离的条件，并扩展到集合值在线学习。

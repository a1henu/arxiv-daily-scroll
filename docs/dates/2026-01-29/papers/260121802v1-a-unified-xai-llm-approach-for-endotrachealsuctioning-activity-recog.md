...
**一句话要点**：提出基于LLM的统一框架，用于气管内吸痰活动识别与反馈生成，以提升护理培训效果。

**关键词**：活动识别, 大型语言模型, 可解释人工智能, 视频分析, 护理教育, 反馈生成

## 3 点简述
- 核心问题：气管内吸痰活动识别在家庭护理和教育场景中缺乏自动化系统，影响培训效率和患者安全。
- 方法要点：以大型语言模型为核心，从视频数据中执行时空活动识别和可解释决策分析，并生成自然语言反馈。
- 实验或效果：LLM方法在准确率和F1分数上比基线模型提升约15-20%，并集成异常检测和XAI的试点支持模块。

...
**关键词**：多专家LLM系统, 论证图, 因果推理, 决策透明度, 校正机制, 因果诊断

## 3 点简述
- 现有多专家LLM系统通过简单聚合整合观点，但掩盖了驱动决策的关键论证过程。
- ARGORA将讨论组织为显式论证图，并建模为因果模型，支持系统性移除和重计算以识别必要推理链。
- 在多样基准测试中，ARGORA实现竞争性准确度，并通过校正机制在专家分歧时更常纠正错误，提供因果诊断。

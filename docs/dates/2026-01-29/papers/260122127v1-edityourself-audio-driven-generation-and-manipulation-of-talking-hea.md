...
**一句话要点**：提出EditYourself框架，基于扩散Transformer实现音频驱动的说话头视频编辑，解决脚本修改中的同步与一致性难题。

**关键词**：音频驱动视频编辑, 扩散Transformer, 说话头视频, 时空修复, 唇部同步, 视频后期制作

## 3 点简述
- 核心问题：现有视频生成模型难以编辑预录视频，需在脚本改动时保持运动、时序、身份和唇部同步。
- 方法要点：基于通用视频扩散模型，通过音频条件和区域感知训练扩展，实现时空修复和内容重组。
- 实验或效果：支持无缝添加、移除和重定时视觉内容，保持长期视觉保真和身份一致性，提升专业后期制作实用性。

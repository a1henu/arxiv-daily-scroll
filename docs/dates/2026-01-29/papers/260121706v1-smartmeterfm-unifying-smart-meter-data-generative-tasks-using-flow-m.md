...
**关键词**：智能电表数据生成, 流匹配模型, 条件生成, 时间序列数据, 统一框架, 数据插补

## 3 点简述
- 核心问题：智能电表数据因隐私、损坏或分辨率不足，需多种生成任务，但现有方法需为每任务单独设计模型，导致效率低下。
- 方法要点：利用流匹配模型，通过条件生成统一处理合成、插补和超分辨率等任务，无需重新训练。
- 实验或效果：模型生成的数据与观测一致且真实，在插补和超分辨率任务上优于插值和其他专用机器学习基线。

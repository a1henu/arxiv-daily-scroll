...
**一句话要点**：提出ILRR方法，通过动态对齐内部激活实现离散扩散语言模型的推理时控制

**关键词**：离散扩散语言模型, 推理时控制, 内部激活对齐, 无学习框架, 属性指导, 长文本生成

## 3 点简述
- 核心问题：离散扩散语言模型缺乏有效的推理时控制机制，现有方法包括采样级指导或轨迹优化
- 方法要点：ILRR是无学习框架，使用单参考序列在去噪过程中动态对齐生成序列与参考的内部激活，可调节指导强度
- 实验或效果：在LLaDA和MDLM架构上，ILRR以较小计算开销提升属性准确性10%至60%，保持高生成质量

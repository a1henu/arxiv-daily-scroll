...
**一句话要点**：提出SimGraph统一框架，以场景图为基础解决图像生成与编辑中的空间一致性和语义控制问题。

**关键词**：场景图生成, 图像编辑, 扩散模型, 空间一致性, 语义控制, 统一框架

## 3 点简述
- 核心问题：现有方法将图像生成与编辑分离，导致空间一致性和语义连贯性不足，且缺乏对对象关系和布局的结构化控制。
- 方法要点：集成基于令牌的生成和基于扩散的编辑于单一场景图驱动模型，实现对对象交互、布局和空间一致性的精确控制。
- 实验或效果：通过广泛实验，实证表明该方法在图像生成和编辑任务上优于现有最先进方法，确保高质量和一致的结果。

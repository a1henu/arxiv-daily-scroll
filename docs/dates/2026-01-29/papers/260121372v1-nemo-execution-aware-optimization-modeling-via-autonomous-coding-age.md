...
**关键词**：自主编码代理, 数学优化建模, 自然语言处理, 代码生成, 执行感知系统

## 3 点简述
- 现有方法依赖专用大语言模型或定制代理，常生成无效或不可执行代码。
- NEMO以自主编码代理为核心抽象，支持沙箱执行，确保代码可执行并允许自动验证修复。
- 在九个优化基准测试中，NEMO在多数任务上达到最先进性能，验证了执行感知代理架构的有效性。

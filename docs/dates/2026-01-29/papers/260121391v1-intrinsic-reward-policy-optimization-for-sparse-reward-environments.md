...
**一句话要点**：提出IRPO框架，利用内在奖励直接优化策略以解决稀疏奖励环境中的探索问题。

**关键词**：强化学习, 稀疏奖励, 内在奖励, 策略优化, 探索策略, 样本效率

## 3 点简述
- 核心问题：稀疏奖励环境中，传统探索策略如噪声注入效果不佳，现有内在奖励方法存在信用分配不稳定或样本效率低的问题。
- 方法要点：提出IRPO框架，通过代理策略梯度结合多个内在奖励，直接优化外部奖励策略，无需预训练子策略。
- 实验或效果：在离散和连续环境中，IRPO相比基线提高了性能和样本效率，并进行了优化问题的形式化分析。

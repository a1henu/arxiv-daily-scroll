...
**一句话要点**：提出基于扩散模型的创意图像生成框架，通过驱动图像概率分布至低概率区域以产生新颖视觉内容。

**关键词**：扩散模型, 创意图像生成, CLIP嵌入, 概率分布驱动, 拉回机制, 文本到图像生成

## 3 点简述
- 核心问题：传统方法依赖手动概念混合或子类别排除，难以自动生成高创意图像。
- 方法要点：利用CLIP嵌入空间中的图像逆概率定义创意，引入拉回机制平衡创意与视觉保真度。
- 实验或效果：在文本到图像扩散模型上验证框架有效，能生成独特、新颖且引人深思的图像。

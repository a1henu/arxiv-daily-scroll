...
**关键词**：可持续材料发现, 机器学习, 生命周期评估, 多尺度建模, 不确定性量化, 多目标优化

## 3 点简述
- 当前AI材料发现优先优化性能，导致可持续性评估滞后，造成资源浪费。
- ML-LCA框架整合信息提取、数据库、多尺度模型、路径预测和优化，实现性能与可持续性协同优化。
- 案例研究涵盖玻璃、水泥等材料，验证了框架的必要性和可行性，并识别了具体挑战。

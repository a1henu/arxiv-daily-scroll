...
**关键词**：模型合并, 治理缺口, 架构无关保护, 权重重缩放, 损失景观, 安全对齐

## 3 点简述
- 核心问题：模型中心化导致模型合并可能绕过安全对齐或许可条款，现有防御方法多为事后且架构特定，保护不一致。
- 方法要点：Trap²为架构无关保护框架，在微调期间通过权重重缩放作为合并过程代理，编码保护到更新中，不影响独立使用。
- 实验或效果：保持发布权重在独立使用时有效，但在合并常见的重缩放下性能下降，从而破坏未经授权的合并。

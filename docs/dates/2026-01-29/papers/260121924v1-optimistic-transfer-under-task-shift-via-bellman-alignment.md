...
**关键词**：在线强化学习, 任务转移, 贝尔曼对齐, 重加权目标, 遗憾界分析, 函数逼近

## 3 点简述
- 核心问题：在线转移强化学习中，源任务与目标任务的贝尔曼回归目标不匹配导致系统偏差。
- 方法要点：引入一步贝尔曼对齐和重加权目标操作，通过测度变换补偿转移不匹配，实现无偏数据重用。
- 实验或效果：在表格和神经网络设置中，相比单任务学习和朴素池化，均显示性能提升。

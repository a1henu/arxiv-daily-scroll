...
**关键词**：少样本域适应, 冰川崩解前沿分割, 时空参考, 静态先验, 深度学习分割

## 3 点简述
- 核心问题：现有模型在基准数据集上表现优异，但在新研究点（分布外域）的分割精度不足，影响科学分析。
- 方法要点：采用少样本域适应策略，结合空间静态先验知识和输入时间序列中的夏季参考图像，无需修改模型架构。
- 实验或效果：分割误差从1131.6米降至68.7米，为全球冰川崩解前沿监测提供框架。

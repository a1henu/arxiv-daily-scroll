...
**一句话要点**：提出VeloDepth以解决视频深度估计中的时间不一致和效率问题，适用于实时应用。

**关键词**：视频深度估计, 时间一致性, 特征传播, 实时应用, 零样本评估

## 3 点简述
- 核心问题：现有视频深度估计方法存在时间不一致或计算量大，限制实际应用。
- 方法要点：引入传播模块，基于光流扭曲和残差校正，高效传播深度特征和预测。
- 实验或效果：零样本评估显示VeloDepth在时间一致性和准确性上领先，推理速度显著更快。

...
## 3 点简述
- 核心问题：现有方法难以平衡点云去噪精度、边缘细节保留和计算效率，影响自动驾驶等应用。
- 方法要点：采用八叉树空间分区并行加速，结合自适应体素统计和kNN密度估计快速去除噪声，再通过密度与距离双权重引力评分精细区分噪声点。
- 实验或效果：在多个数据集上验证，相比现有方法，F1、PSNR和Chamfer Distance指标提升，单帧处理时间减少，展现高精度、鲁棒性和实时性。

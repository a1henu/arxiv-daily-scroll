...
**一句话要点**：提出TAFAP方法，通过匹配训练轨迹实现扩散模型的目标数据保护

**关键词**：扩散模型, 目标数据保护, 训练轨迹匹配, 对抗扰动, 模型微调, 隐私保护

## 3 点简述
- 核心问题：现有目标数据保护方法因快照匹配导致可控性差，无法稳定控制扩散模型输出
- 方法要点：TAFAP采用轨迹匹配，结合对抗扰动微调，控制整个训练轨迹以实现持久保护
- 实验或效果：TAFAP在扩散模型中首次成功实现目标转换，同时控制身份和视觉模式，显著优于现有方法

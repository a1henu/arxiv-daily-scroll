...
**一句话要点**：提出AlcheMinT框架，通过时间戳条件实现多主体视频生成的细粒度时序控制

**关键词**：视频生成, 时序控制, 主题驱动, 扩散模型, 位置编码, 多主体一致性

## 3 点简述
- 现有主题驱动视频生成方法缺乏对主体出现和消失的细粒度时序控制，限制应用如合成视频和故事板
- 引入新颖位置编码机制，结合时间间隔编码和预训练模型嵌入，避免额外模块和参数开销
- 实验表明AlcheMinT在视觉质量上匹配先进方法，首次实现视频内多主体生成的精确时序控制

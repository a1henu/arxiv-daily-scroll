...
**关键词**：多模态大语言模型, 上下文盲区, 视觉细节感知, 注意力熵, 层次化裁剪, 训练免费方法

## 3 点简述
- 核心问题：多模态大语言模型在细粒度视觉细节感知上存在不足，导致上下文盲区，即裁剪区域与全局上下文的结构性脱节。
- 方法要点：采用无需训练的两步法，先通过上下文锚定识别兴趣区域，再构建基于注意力熵的层次化裁剪组合以保留从细节到环境的上下文。
- 实验或效果：在实验中显著优于单裁剪和非结构化多裁剪基线，验证层次化结构对解决上下文盲区的关键作用。

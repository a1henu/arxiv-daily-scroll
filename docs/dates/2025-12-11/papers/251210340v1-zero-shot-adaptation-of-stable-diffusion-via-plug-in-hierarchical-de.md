...
**一句话要点**：提出HD-CLIP插件模块以解决真实世界超分辨率中未知复杂退化问题

**关键词**：真实世界超分辨率, 扩散模型, 退化表示, 零样本适应, 即插即用模块, 分类器自由引导

## 3 点简述
- 核心问题：真实世界图像超分辨率面临未知耦合退化，现有方法依赖已知退化程度且CLIP无法捕捉数值严重性。
- 方法要点：HD-CLIP将低质量图像分解为语义嵌入和有序退化嵌入，支持未见退化级别的插值，通过CFG和CFPG集成到扩散模型。

...
**一句话要点**：提出滑动窗口注意力适应方法，以低成本适配全注意力预训练模型至长上下文推理。

**关键词**：滑动窗口注意力, 长上下文推理, 注意力机制适配, 训练-推理不匹配, Transformer模型

## 3 点简述
- 核心问题：全注意力预训练模型直接应用滑动窗口注意力导致长上下文性能严重下降。
- 方法要点：结合五种方法，包括预填充时应用SWA、保留sink令牌、层间交错、思维链和微调。
- 实验或效果：特定组合能有效恢复原始性能，提供不同场景的推荐配置。

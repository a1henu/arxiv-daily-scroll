...
**关键词**：聚焦形状恢复, 多尺度方向性扩张拉普拉斯, 循环神经网络, 深度估计, 鲁棒性优化, 高分辨率重建

## 3 点简述
- 聚焦形状恢复中，现有深度学习方法依赖重型特征编码器提取聚焦体积，导致深度图存在伪影和噪声放大问题。
- 采用传统手工多尺度方向性扩张拉普拉斯核计算聚焦体积，结合轻量级多尺度GRU循环网络迭代优化深度估计，并通过凸上采样模块重建高分辨率深度图。
- 在合成和真实数据集上验证，方法在多种聚焦条件下优于现有深度学习和传统方法，实现更高精度和泛化能力。

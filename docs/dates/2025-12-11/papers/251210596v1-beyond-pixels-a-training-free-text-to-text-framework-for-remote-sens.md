...
**一句话要点**：提出训练免费、文本到文本框架TRSLLaVA，利用结构化文本解决遥感图像语义检索中的语义鸿沟问题。

**关键词**：遥感图像检索, 语义鸿沟, 训练免费方法, 文本到文本匹配, 结构化文本基准

## 3 点简述
- 核心问题：遥感图像语义检索面临语义鸿沟，现有方法依赖昂贵领域训练且缺乏零样本基准。
- 方法要点：基于RSRT数据集，将跨模态检索重构为文本到文本匹配，在统一文本嵌入空间使用VLM生成描述进行检索。
- 实验或效果：在RSITMD和RSICD基准上，训练免费方法达到42.62%平均召回率，超越零样本CLIP基线及多个监督模型。

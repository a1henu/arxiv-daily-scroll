...
**关键词**：序列推荐, 语义ID, 哈希ID, 长尾问题, 双分支架构, 对齐策略

## 3 点简述
- 核心问题：传统哈希ID易受长尾问题影响，语义ID虽能共享编码但可能削弱头部项目建模。
- 方法要点：设计双分支架构，分别捕获语义ID的多粒度语义和哈希ID的独特协作身份，并引入双级对齐策略。
- 实验或效果：在三个真实数据集上验证，有效平衡头尾项目推荐质量，超越现有基线。

...
**关键词**：序列推荐, 语义ID, 哈希ID, 长尾问题, 双分支架构, 知识对齐

## 3 点简述
- 核心问题：传统哈希ID易受长尾问题影响，而语义ID在量化机制下可能损害头项目建模的独特性。
- 方法要点：设计双分支架构，分别捕获语义ID的多粒度语义和哈希ID的协同身份，并引入双级对齐策略促进知识迁移。
- 实验或效果：在三个真实数据集上验证，H2Rec能平衡头尾项目推荐质量，超越现有基线。

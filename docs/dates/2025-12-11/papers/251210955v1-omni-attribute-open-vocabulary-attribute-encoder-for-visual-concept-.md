...
**关键词**：视觉概念个性化, 开放词汇属性编码, 对比解缠, 属性特定表示, 双目标训练, 图像属性检索

## 3 点简述
- 现有方法依赖通用图像编码器的整体嵌入，导致多个视觉因素纠缠，难以隔离单一属性，常引发信息泄漏和不一致合成。
- 通过联合设计数据和模型：收集语义链接图像对并标注正负属性，采用双目标训练范式平衡生成保真度和对比解缠。
- 在开放词汇属性检索、个性化和组合生成任务中实现最先进性能，验证了编码器的有效性。

...
**一句话要点**：提出VLM-NCD框架，融合视觉-文本语义与原型聚类，以解决图像新类发现中特征判别性不足和长尾分布问题。

**关键词**：新类发现, 视觉-语言模型, 原型聚类, 长尾分布, 语义融合, 双阶段机制

## 3 点简述
- 核心问题：图像新类发现方法依赖视觉特征，存在特征判别性不足和数据长尾分布限制。
- 方法要点：通过联合优化已知类图像和文本特征建模聚类中心和语义原型，并采用双阶段发现机制动态分离样本。
- 实验或效果：在CIFAR-100数据集上，未知类准确率提升高达25.3%，首次展示对长尾分布的鲁棒性。

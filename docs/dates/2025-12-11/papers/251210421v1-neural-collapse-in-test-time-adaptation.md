...
**一句话要点**：提出NCTTA以解决测试时适应中样本对齐崩溃问题，提升域偏移鲁棒性。

**关键词**：测试时适应, 神经崩溃, 样本对齐, 域偏移鲁棒性, 特征-分类器对齐, 伪标签优化

## 3 点简述
- 核心问题：测试时适应中性能下降源于样本特征与分类器权重对齐崩溃，伪标签不可靠。
- 方法要点：基于样本对齐崩溃现象，设计NCTTA方法，结合几何邻近性和预测置信度进行特征-分类器对齐。
- 实验或效果：在ImageNet-C等实验中，NCTTA显著优于现有方法，如比Tent提升14.52%。

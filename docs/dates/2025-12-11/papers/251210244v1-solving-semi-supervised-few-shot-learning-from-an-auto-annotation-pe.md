...
**一句话要点**：提出SWIFT方法以解决半监督少样本学习中视觉语言模型微调失效的问题

**关键词**：半监督少样本学习, 视觉语言模型, 自动标注, 温度调优, 伪标签, 阶段式微调

## 3 点简述
- 核心问题：视觉语言模型在微调时产生平坦概率分布，导致未标记数据利用率和监督信号弱
- 方法要点：通过分类器初始化和温度调优提升伪标签置信度，并设计阶段式微调策略
- 实验或效果：在五个基准测试中超越现有方法约5个准确率点，接近全监督学习性能

...
**一句话要点**：评估深度学习与LLM在漏洞检测中的现实适用性，揭示学术基准与真实部署的差距

**关键词**：漏洞检测, 深度学习模型, 大语言模型, 现实评估, 代码表示分析, 泛化性能

## 3 点简述
- 核心问题：基于深度学习的漏洞检测方法在基准数据集上表现良好，但真实世界效果未知，存在泛化挑战
- 方法要点：系统评估ReVeal和LineVul模型在四个数据集上，并部署于新构建的VentiVul数据集，分析代码表示
- 实验或效果：模型在表示空间难以区分漏洞代码，在时间外分布数据集上性能急剧下降，暴露现实部署差距

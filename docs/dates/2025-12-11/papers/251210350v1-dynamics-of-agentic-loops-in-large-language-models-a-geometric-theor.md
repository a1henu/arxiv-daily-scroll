...
**一句话要点**：提出几何框架分析大语言模型代理循环轨迹，通过校准嵌入空间测量收敛与发散动态。

**关键词**：代理循环, 几何框架, 嵌入空间校准, 动态系统, 收敛分析, 大语言模型

## 3 点简述
- 核心问题：代理循环在语义嵌入空间中的几何行为（如收敛或发散）缺乏理解。
- 方法要点：引入几何框架，区分语言变换空间与嵌入空间，应用各向同性校准消除余弦相似度偏差。

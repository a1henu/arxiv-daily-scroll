...
**一句话要点**：提出ClusIR框架，通过可学习聚类建模退化语义，实现多退化图像恢复的统一自适应处理。

**关键词**：图像恢复, 退化建模, 聚类引导, 频率调制, 自适应处理

## 3 点简述
- 核心问题：现有AiOIR方法难以显式建模退化类型，对复杂或混合退化适应能力不足。
- 方法要点：引入PCGRM分离退化识别与专家激活，结合DAFMM进行自适应频率调制，协同提升恢复保真度。

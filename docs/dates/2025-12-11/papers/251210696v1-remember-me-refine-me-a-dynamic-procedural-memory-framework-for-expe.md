...
**一句话要点**：提出ReMe动态程序记忆框架，以解决LLM代理经验驱动进化中静态记忆的局限性。

**关键词**：程序记忆, LLM代理, 经验蒸馏, 动态推理, 记忆精炼, 终身学习

## 3 点简述
- 核心问题：现有LLM代理记忆框架多为被动积累，缺乏动态推理能力，导致经验利用效率低。
- 方法要点：通过多面蒸馏提取细粒度经验、上下文自适应重用历史洞察、基于效用的精炼维护高质量记忆池。
- 实验或效果：在BFCL-V3和AppWorld上实现SOTA，Qwen3-8B+ReMe超越更大无记忆模型，展示内存缩放效应。

...
## 3 点简述
- 核心问题：非线性逆问题常涉及噪声、不可微或计算昂贵的残差评估，导致基于雅可比矩阵的求解器不可靠。
- 方法要点：RSES通过采样高斯探针构建残差代理，利用最小二乘求解更新，避免雅可比矩阵或协方差计算，每次迭代成本低。
- 实验或效果：在标定、回归和去卷积问题中，RSES在确定性和随机设置下均能持续降低失配，性能优于或匹配xNES和NEWUOA，与EKI竞争。

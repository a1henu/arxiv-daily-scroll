...
**关键词**：视觉语言模型, 联合嵌入预测, 连续嵌入预测, 选择性解码, 开放词汇分类, 视频检索

## 3 点简述
- 核心问题：传统视觉语言模型在标记空间自回归生成，可能忽略语义抽象，导致效率低下。
- 方法要点：采用联合嵌入预测架构，在抽象表示空间预测目标文本嵌入，减少表面语言变异性干扰。
- 实验或效果：在相同视觉编码器和训练数据下，参数减少50%，性能更强，支持选择性解码和多种任务。

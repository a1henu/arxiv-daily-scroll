...
**一句话要点**：提出基于状态链的两阶段框架，将非正式数学证明转换为Lean4形式化证明。

**关键词**：形式化证明转换, 状态链表示, 战术生成, Lean4证明助手, 数学证明自动化

## 3 点简述
- 核心问题：在有限计算预算下，将自然语言非正式数学证明转换为Lean4形式化证明。
- 方法要点：先提取状态链对齐非正式推理结构，再生成战术连接状态，降低战术生成复杂度。
- 实验或效果：构建专用数据集和基准，方法显著优于基线，提高证明成功率。

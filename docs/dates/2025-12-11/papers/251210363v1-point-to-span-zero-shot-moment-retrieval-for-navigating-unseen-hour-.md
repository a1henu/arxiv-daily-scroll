...
**一句话要点**：提出Point-to-Span框架，以零样本方式解决小时级长视频时刻检索中的搜索低效和精炼高成本问题。

**关键词**：零样本学习, 长视频时刻检索, 自适应跨度生成, 查询分解, 训练免费框架

## 3 点简述
- 核心问题：零样本长视频时刻检索面临搜索阶段候选爆炸和精炼阶段依赖高成本视觉语言模型验证的双重挑战。
- 方法要点：通过自适应跨度生成器防止候选爆炸，并利用查询分解精炼候选，无需高成本视觉语言模型验证。
- 实验或效果：在MAD数据集上，P2S超越监督方法，如R5@0.1指标提升3.7%。

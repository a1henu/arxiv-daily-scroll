...
**一句话要点**：提出基于移动设备的实时物理治疗运动识别与评估算法框架

**关键词**：姿态估计, 实时运动识别, 动态规划, 移动健康, 物理治疗评估

## 3 点简述
- 核心问题：实时识别和评估物理治疗运动，支持远程监督和移动健康应用。
- 方法要点：使用姿态估计网络提取关键点，转换为角度特征，结合轻量分类器和动态规划进行序列匹配。
- 实验或效果：系统在客户端运行，实验验证了方法的有效性和实时性能。

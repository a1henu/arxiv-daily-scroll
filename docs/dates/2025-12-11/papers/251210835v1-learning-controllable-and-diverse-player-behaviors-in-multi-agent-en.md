...
**一句话要点**：提出强化学习框架以实现多智能体环境中可控且多样的玩家行为，无需人类游戏数据。

**关键词**：强化学习, 多智能体系统, 行为控制, 游戏AI, 可解释AI

## 3 点简述
- 核心问题：现有方法依赖大规模玩家轨迹或缺乏可解释行为参数与策略的直接映射，限制可扩展性和可控性。
- 方法要点：在N维连续空间中定义行为，通过目标行为向量输入和距离减少奖励，使策略学习动作对行为统计的影响。
- 实验或效果：在自定义Unity游戏中，相比仅赢取基线，产生显著更多行为多样性，并能可靠匹配指定行为向量。

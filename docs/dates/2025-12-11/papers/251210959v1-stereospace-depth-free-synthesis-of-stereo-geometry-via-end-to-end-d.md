...
**一句话要点**：提出StereoSpace，通过端到端扩散在规范空间中实现无需深度的单目到立体合成。

**关键词**：立体合成, 扩散模型, 视点条件, 规范空间, 端到端评估, 几何一致性

## 3 点简述
- 核心问题：单目到立体合成依赖深度或变形，导致几何建模复杂且易泄露信息。
- 方法要点：基于扩散模型，在规范矫正空间中通过视点条件建模几何，无需显式深度或变形。
- 实验或效果：引入无泄露评估协议，在感知舒适度和几何一致性上超越现有方法，适用于分层和非朗伯场景。

...
**一句话要点**：提出CLAPS算法，通过对称感知的保形预测量化动作不确定性，适用于非欧几里得机器人配置空间。

**关键词**：不确定性量化, 保形预测, 李群, 非欧几里得配置空间, 机器人控制, 动作预测集

## 3 点简述
- 核心问题：现有不确定性量化方法依赖强假设或未校准估计，难以保证安全控制，且忽略机器人非欧几里得配置空间结构。
- 方法要点：基于保形预测框架，利用李群分析配置误差，扩展理论保证至SE(2)等非欧几里得空间，构建概率保证的动作预测集。

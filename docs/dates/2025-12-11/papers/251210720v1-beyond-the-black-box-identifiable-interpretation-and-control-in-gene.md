...
**一句话要点**：提出基于因果最小性原则的生成模型可解释框架，实现潜在表示的可识别控制

**关键词**：生成模型可解释性, 因果最小性, 层次化选择模型, 潜在变量识别, 模型控制, 概念图提取

## 3 点简述
- 核心问题：生成模型作为黑盒，缺乏理论保证的可解释性，阻碍人类理解和控制
- 方法要点：利用因果最小性原则，通过稀疏或压缩约束学习可解释的层次化潜在表示
- 实验或效果：应用于扩散视觉和自回归语言模型，提取内在概念图并实现细粒度模型引导

...
**一句话要点**：提出基于因果最小性原则的框架，为生成模型提供可解释表示与控制

**关键词**：生成模型可解释性, 因果最小性, 层次选择模型, 扩散模型, 自回归模型, 模型控制

## 3 点简述
- 核心问题：生成模型作为黑盒，缺乏理论保证的可解释性，阻碍理解与控制。
- 方法要点：引入因果最小性原则，通过稀疏或压缩约束，使学习表示等价于真实潜在变量。
- 实验或效果：应用于扩散视觉和自回归语言模型，提取层次概念图，实现细粒度模型操控。

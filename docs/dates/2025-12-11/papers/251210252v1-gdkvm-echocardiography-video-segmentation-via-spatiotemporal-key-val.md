...
**一句话要点**：提出GDKVM模型，通过时空键值记忆与门控增量规则，提升超声心动图视频分割的准确性与鲁棒性。

**关键词**：超声心动图分割, 时空记忆网络, 视频分析, 医学图像处理, 深度学习

## 3 点简述
- 核心问题：超声心动图视频分割面临噪声、伪影及心脏变形运动挑战，现有方法在长程时空依赖与计算效率间难以平衡。
- 方法要点：采用线性键值关联建模帧间相关性，引入门控增量规则高效存储记忆状态，设计关键像素特征融合模块整合多尺度特征。
- 实验或效果：在CAMUS和EchoNet-Dynamic数据集上验证，GDKVM在分割精度、鲁棒性和实时性方面优于现有方法。

...
**一句话要点**：提出同步腱驱动欠驱动机器人手指，以单执行器实现高负载与自适应柔顺性。

**关键词**：欠驱动机器人手指, 腱驱动, 同步腱路由, 单执行器驱动, 结构刚度预测, 自适应柔顺性

## 3 点简述
- 核心问题：紧凑型腱驱动欠驱动机器人手指难以同时实现高负载能力和自适应柔顺性。
- 方法要点：采用同步腱路由机械耦合所有关节，固定角速度比，仅需单执行器驱动。
- 实验或效果：原型测试显示平均偏转预测误差1.0毫米，刚度1.2x10^3 N/m，集成五指手验证有效抓取。

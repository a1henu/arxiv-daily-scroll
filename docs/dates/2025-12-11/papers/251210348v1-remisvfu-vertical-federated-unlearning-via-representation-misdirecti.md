...
**关键词**：垂直联邦学习, 联邦遗忘, 表示误导, 客户端级遗忘, 正交投影, 后门攻击防御

## 3 点简述
- 针对垂直联邦学习（VFL）中数据按特征划分，现有遗忘方法无效的问题，提出表示误导框架。
- 遗忘方将编码器输出坍缩到单位球上的随机锚点，服务器通过正交投影联合优化保留和遗忘损失以维持效用。
- 实验显示，REMISVFU将后门攻击成功率降至自然类先验水平，仅牺牲约2.5%的干净准确率。

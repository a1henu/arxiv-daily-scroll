...
**一句话要点**：提出基于扩散回归的冠状动脉血压预测框架，以支持非侵入性冠心病诊断。

**关键词**：冠状动脉疾病诊断, 血压预测, 扩散回归模型, 计算流体动力学, 冠状动脉CT血管造影

## 3 点简述
- 核心问题：传统计算流体动力学模拟耗时且难以集成临床流程，限制AI模型训练数据获取。
- 方法要点：开发自动化管道提取冠状动脉几何特征，并设计扩散回归模型直接从CCTA特征预测血压。
- 实验或效果：在模拟血流数据集上，模型R2达64.42%，优于基线方法，实现高效血压预测。

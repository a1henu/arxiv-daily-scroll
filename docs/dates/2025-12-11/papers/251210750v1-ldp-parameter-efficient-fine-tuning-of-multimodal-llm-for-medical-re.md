...
**一句话要点**：提出LDP框架，利用多模态大语言模型生成结肠镜息肉诊断报告，以解决医疗数据稀缺导致的报告不一致和幻觉问题。

**关键词**：多模态大语言模型, 参数高效微调, 医疗报告生成, 结肠镜诊断, 直接偏好优化, 医学数据集

## 3 点简述
- 核心问题：结肠镜息肉诊断中，传统自动报告因高质量多模态医疗数据稀缺，常出现不一致和幻觉。
- 方法要点：基于Qwen2-VL-7B，采用LoRA进行参数高效微调，并通过DPO对齐临床标准，构建MMEndo数据集。
- 实验或效果：在自动指标和临床专家评估中优于基线，医生评分7.2/10，训练计算成本比全微调降低833倍。

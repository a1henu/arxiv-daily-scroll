...
**一句话要点**：提出CheXmask-U方法，量化X射线图像中基于解剖标志点分割的不确定性以增强临床部署安全性。

**关键词**：医学图像分割, 不确定性估计, X射线图像, 解剖标志点, 变分潜在空间, 数据集发布

## 3 点简述
- 核心问题：医学图像分割中不确定性估计不足，尤其针对基于解剖标志点的分割方法，影响临床部署的可靠性。
- 方法要点：结合卷积编码器和图生成解码器的混合神经网络，从变分潜在空间推导潜在不确定性和预测不确定性两种互补度量。
- 实验或效果：通过扰动实验验证不确定性随扰动严重度增加，支持不可靠预测识别和分布外检测，并发布大规模不确定性数据集。

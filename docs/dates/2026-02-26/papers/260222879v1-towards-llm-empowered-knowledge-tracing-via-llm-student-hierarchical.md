...
**一句话要点**：提出L-HAKT框架，通过LLM-学生分层行为对齐与双曲空间建模，解决知识追踪中认知状态演化与个性化难度感知不足的问题。

**关键词**：知识追踪, 大语言模型, 双曲空间, 分层行为对齐, 对比学习, 教育数据挖掘

## 3 点简述
- 核心问题：现有知识追踪方法基于ID或浅层文本特征，难以捕捉认知状态的分层演化和个性化问题难度感知。
- 方法要点：使用教师代理解析问题语义构建知识点分层依赖，学生代理模拟学习行为生成合成数据，在双曲空间进行对比学习对齐特征分布。
- 实验或效果：在四个真实教育数据集上验证了L-HAKT框架的有效性，通过优化双曲曲率精确建模知识点树状层次结构。

...
**一句话要点**：提出MoFit框架，通过模型拟合嵌入实现无标注的成员推断攻击，以解决潜在扩散模型训练数据记忆的隐私审计问题。

**关键词**：成员推断攻击, 潜在扩散模型, 隐私审计, 模型拟合嵌入, 无标注条件

## 3 点简述
- 核心问题：潜在扩散模型在文本到图像生成中可能记忆训练数据，现有成员推断攻击依赖真实标注，在仅图像可用时失效。
- 方法要点：MoFit分两阶段，先优化图像扰动构建模型拟合代理，再提取嵌入作为条件，放大成员样本的条件损失差异。
- 实验或效果：在多数据集和扩散模型上，MoFit优于视觉语言模型标注基线，性能接近依赖标注的方法。

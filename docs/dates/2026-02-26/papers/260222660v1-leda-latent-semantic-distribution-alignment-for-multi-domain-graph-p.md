...
**一句话要点**：提出LEDA模型，通过潜在语义分布对齐解决多领域图预训练中的语义对齐与训练指导不足问题。

**关键词**：图预训练, 多领域学习, 语义对齐, 变分推断, 跨域迁移, 少样本学习

## 3 点简述
- 核心问题：现有方法因数据对齐简单和训练指导有限，难以从通用图中学习有效知识。
- 方法要点：引入维度投影单元对齐领域特征，设计变分语义推断模块获取共享潜在分布以指导跨域语义学习。
- 实验或效果：在广泛图数据和下游任务中表现优异，尤其在少样本跨域设置中显著优于基线模型。

...
**一句话要点**：提出运动感知事件抑制框架，实时过滤事件相机中的动态事件以提升下游应用性能。

**关键词**：事件相机, 运动感知抑制, 实时处理, 独立移动物体分割, 下游应用加速

## 3 点简述
- 核心问题：事件相机易受独立移动物体和自身运动干扰，需实时抑制动态事件。
- 方法要点：联合分割当前事件流中的独立移动物体并预测其未来运动，实现前瞻性事件抑制。

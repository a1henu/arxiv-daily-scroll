...
**一句话要点**：提出信息增益作为细胞级评估框架，以直接评估虚拟染色模型预测的后验分布。

**关键词**：虚拟染色, 后验分布评估, 信息增益, 高通量筛选, 生成模型, 评分规则

## 3 点简述
- 核心问题：现有评估协议仅检查数据集上的边缘分布准确性，无法评估预测的后验分布。
- 方法要点：引入信息增益作为严格适当评分规则，提供理论动机，支持可解释性和跨模型比较。
- 实验或效果：在广泛的高通量筛选数据集上评估扩散和GAN模型，显示信息增益能揭示其他指标无法检测的性能差异。

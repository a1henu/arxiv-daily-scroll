...
## 3 点简述
- 核心问题：现有音视频质量评估数据集规模小、内容与质量多样性不足，仅标注总体分数，限制模型开发与多模态感知研究。
- 方法要点：设计众包主观实验框架，突破实验室限制，确保跨环境可靠标注；采用系统数据准备策略，覆盖广泛质量级别与语义场景；扩展额外标注，支持多模态感知机制研究。
- 实验或效果：通过YT-NTU-AVQ数据集验证，该数据集是目前最大且最多样的音视频质量评估数据集，包含1,620个用户生成的音视频序列。

...
**一句话要点**：提出运行时可重配置多精度比特脉动阵列设计，以支持硬件加速器上的混合精度量化神经网络推理。

**关键词**：硬件加速器, 量化神经网络, 混合精度, 脉动阵列, 运行时重配置, FPGA实现

## 3 点简述
- 核心问题：硬件乘法器设计无法在运行时支持多精度量化神经网络模型的精度重配置。
- 方法要点：设计多通道比特脉动阵列，实现运行时多精度乘法运算的可重配置性。
- 实验或效果：在Ultra96 FPGA上实现，推理混合精度模型速度提升1.3185至3.5671倍，支持更高时钟频率（250MHz）。

...
**一句话要点**：提出Pix2Key方法，通过语义分解和自监督视觉词典学习实现可控开放词汇检索，以解决组合图像检索中的意图感知和多样性问题。

**关键词**：组合图像检索, 开放词汇检索, 视觉词典学习, 自监督预训练, 意图感知匹配, 多样性重排序

## 3 点简述
- 核心问题：组合图像检索中传统方法易丢失细粒度线索，零样本方法可能忽略用户隐含意图并返回重复结果。
- 方法要点：将查询和候选图像表示为开放词汇视觉词典，在统一嵌入空间进行意图感知约束匹配和多样性感知重排序，并引入自监督预训练组件V-Dict-AE增强词典表示。
- 实验或效果：在DFMM-Compose基准上，Pix2Key提升Recall@10达3.2点，加入V-Dict-AE额外增益2.3点，同时提高意图一致性和保持高列表多样性。

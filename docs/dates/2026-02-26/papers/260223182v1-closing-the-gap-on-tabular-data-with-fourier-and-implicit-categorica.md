...
**一句话要点**：提出基于统计特征处理与学习傅里叶的方法，以缩小深度学习在表格数据上与树模型的性能差距。

**关键词**：表格数据学习, 特征处理, 深度学习, 傅里叶特征, 类别特征建模, 性能基准

## 3 点简述
- 核心问题：深度学习在表格数据上落后于树模型，因难以有效建模类别特征的非线性交互。
- 方法要点：使用统计方法识别离散化后与目标强相关的特征，并引入学习傅里叶缓解模型对平滑解的偏好。
- 实验或效果：在综合基准测试中，该方法显著提升深度学习性能，接近或超越XGBoost。

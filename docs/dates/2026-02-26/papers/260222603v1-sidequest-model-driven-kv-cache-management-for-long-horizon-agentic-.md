...
**一句话要点**：提出SideQuest模型驱动KV缓存管理方法，以解决长程智能体推理中的内存膨胀问题。

**关键词**：KV缓存压缩, 长程智能体推理, 模型驱动管理, 并行任务执行, 内存优化

## 3 点简述
- 核心问题：长程智能体任务中，外部检索令牌导致KV缓存快速膨胀，影响解码性能。
- 方法要点：利用大型推理模型自身评估令牌有用性，并行执行KV缓存压缩作为辅助任务。

...
## 3 点简述
- 核心问题：商业大语言模型作为黑盒API部署，用户需信任提供商正确执行推理和报告令牌使用，存在模型替换、量化滥用和令牌超收等经济动机偏差风险。
- 方法要点：IMMACULATE采用可验证计算选择性审计少量请求，无需可信硬件或模型内部访问，实现强检测保证并摊销密码学开销。
- 实验或效果：在密集和MoE模型上实验显示，IMMACULATE可靠区分良性恶意执行，吞吐量开销低于1%。

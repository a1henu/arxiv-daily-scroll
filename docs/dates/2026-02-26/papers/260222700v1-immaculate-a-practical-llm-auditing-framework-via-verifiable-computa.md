...
**关键词**：大语言模型审计, 可验证计算, 黑盒API, 经济动机偏差, 模型替换检测, 令牌计费验证

## 3 点简述
- 核心问题：商业大语言模型作为黑盒API部署，用户需信任提供商正确执行推理和诚实报告令牌使用。
- 方法要点：使用可验证计算选择性审计少量请求，无需可信硬件或模型内部访问，检测模型替换、量化滥用和令牌超计费。
- 实验或效果：在密集和MoE模型上实验，可靠区分良性恶意执行，吞吐量开销低于1%。

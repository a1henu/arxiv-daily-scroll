...
**关键词**：蜕变测试, 视觉-语言-动作模型, 机器人测试, 测试预言问题, 轨迹分析

## 3 点简述
- 核心问题：VLA模型面临测试预言问题，包括指令特定预言定义复杂和现有预言无法评估任务执行质量。
- 方法要点：设计两种蜕变关系模式和五种蜕变关系，通过输入变化检测机器人轨迹异常。
- 实验或效果：在五个VLA模型、两个模拟机器人和四个任务中验证，能自动检测多种失败，包括未完成任务。

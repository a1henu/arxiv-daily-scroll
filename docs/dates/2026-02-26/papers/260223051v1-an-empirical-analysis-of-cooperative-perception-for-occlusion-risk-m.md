...
**关键词**：遮挡风险缓解, 风险追踪损失（RTL）, 车联网（V2X）部署, 不对称通信, 自动驾驶安全, 实证分析

## 3 点简述
- 核心问题：遮挡导致自动驾驶感知系统失效，传统风险指标无法充分捕捉累积威胁。
- 方法要点：引入RTL指标聚合遮挡期间风险强度，并提出不对称通信框架提升非连接车辆安全性。
- 实验或效果：大规模统计分析验证RTL有效性，不对称框架在25%渗透率下优于传统对称模型75%渗透率。

...
**关键词**：临床决策评估, 大语言模型基准, 判断确定性, 不完全信息处理, 医疗安全

## 3 点简述
- 核心问题：临床决策常面临信息不全，需识别信息是否足够判断，避免过早结论或不必要弃权。
- 方法要点：基于临床评分系统，将不完全信息场景分解为可确定和不可确定条件，要求考虑所有缺失信息假设。
- 实验或效果：发现近期大语言模型无法识别确定性，产生过早判断和过度弃权，尽管在完全信息下表现良好。

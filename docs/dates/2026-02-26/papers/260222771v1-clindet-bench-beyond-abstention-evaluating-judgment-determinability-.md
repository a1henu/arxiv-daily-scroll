...
## 3 点简述
- 核心问题：临床决策常面临信息不完整，需评估大语言模型能否识别信息是否足以做出判断，避免过早结论或过度弃权。
- 方法要点：基于临床评分系统，将不完整信息场景分解为可确定和不可确定条件，要求模型考虑所有缺失信息的假设。
- 实验或效果：发现近期大语言模型在信息不完整时无法识别确定性，产生过早判断和过度弃权，尽管在完整信息下表现良好。

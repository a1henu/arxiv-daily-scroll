...
**一句话要点**：提出Phys-3D框架，通过物理约束3D运动建模解决铁路站台实时人群计数中的遮挡与动态挑战。

**关键词**：人群计数, 物理约束跟踪, 3D运动建模, 铁路站台安全, 实时视觉系统, 遮挡处理

## 3 点简述
- 核心问题：铁路站台人群计数因密集遮挡、相机运动和透视变形而不可靠，现有方法忽略物理一致性。
- 方法要点：结合YOLOv11m检测、EfficientNet-B0外观编码与物理约束卡尔曼模型，统一检测、外观和3D运动推理。
- 实验或效果：在MOT-RPCH数据集上计数误差降至2.97%，验证了在动态条件下鲁棒性能。

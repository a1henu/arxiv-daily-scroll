...
**一句话要点**：提出自动核计数算法以解决小数据集下大鼠脊髓小胶质细胞计数及不确定性分析问题

**关键词**：细胞计数, 小数据集训练, 不确定性分析, 非参数方法, 图像预处理

## 3 点简述
- 核心问题：自动计数大鼠脊髓切片中的小胶质细胞，避免耗时的手动标注和传统颜色方法的局限性。
- 方法要点：采用非参数非线性核计数方法，通过预处理过滤噪声，仅需一个超参数，支持小数据集训练和不确定性估计。
- 实验或效果：在人工和真实数据集上验证，结果有前景，并提供Matlab代码。

...
**关键词**：EEG视觉解码, 自回归生成, 多尺度预测, 脑机接口, 对比学习, 轻量模型

## 3 点简述
- 问题：EEG与图像模态差异大，现有方法多阶段适应导致误差累积，扩散模型计算开销高。
- 方法：基于对比学习对齐EEG与图像表示，采用自回归框架从粗到细预测多尺度图像令牌。
- 效果：在图像检索与重建任务中优于先前方法，参数仅10%，生成过程反映视觉感知层次。

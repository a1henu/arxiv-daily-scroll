...
**一句话要点**：提出SPARTA框架以自动生成大规模表格-文本多跳问答基准，解决现有基准规模小、问题浅的问题。

**关键词**：表格-文本问答, 多跳推理, 基准生成, 聚合操作, 跨模态推理, 自动合成

## 3 点简述
- 现有表格-文本问答基准规模小、手动构建易错，且问题多为浅层，缺乏多跳推理和聚合操作。
- SPARTA框架通过构建参考事实数据库和合成嵌套查询，自动生成大规模基准，采用来源优化和真实结构强制技术确保可执行性和流畅性。
- 在SPARTA上，当前最先进模型性能显著下降，暴露跨模态推理弱点，基准和代码已开源。

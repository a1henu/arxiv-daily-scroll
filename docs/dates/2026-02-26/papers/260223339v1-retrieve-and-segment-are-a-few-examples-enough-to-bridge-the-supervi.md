...
**一句话要点**：提出检索增强测试时适配器，通过少量示例融合文本与视觉支持特征，以缩小开放词汇分割的监督差距。

**关键词**：开放词汇分割, 少样本学习, 检索增强, 测试时适配, 视觉语言模型, 像素级预测

## 3 点简述
- 核心问题：开放词汇分割因图像级监督粗糙和自然语言语义模糊，性能落后于全监督方法。
- 方法要点：引入少样本设置，结合文本提示和像素标注支持集，学习轻量级每图像分类器，实现模态间协同融合。
- 实验或效果：显著缩小零样本与监督分割差距，保持开放词汇能力，适用于细粒度任务如个性化分割。

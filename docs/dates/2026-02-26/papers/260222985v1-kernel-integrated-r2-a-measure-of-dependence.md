...
**一句话要点**：提出核集成R²以衡量多变量、函数和结构化数据的统计依赖性

**关键词**：统计依赖性度量, 再生核希尔伯特空间, 条件均值嵌入, K近邻估计, 非参数统计, 高维数据分析

## 3 点简述
- 核心问题：扩展集成R²以处理非标量响应数据，如多变量、函数和结构化数据，同时保持对尾部行为和振荡依赖的敏感性。
- 方法要点：结合集成R²的局部归一化原则与再生核希尔伯特空间的灵活性，定义取值在[0,1]的依赖性度量，并开发基于K近邻和条件均值嵌入的估计器。
- 实验或效果：在模拟和真实媒体标注数据上验证，显示在非线性和结构化关系场景中优于现有依赖性度量。

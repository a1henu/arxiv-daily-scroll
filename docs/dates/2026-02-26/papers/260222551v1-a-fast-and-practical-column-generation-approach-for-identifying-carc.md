...
**一句话要点**：提出基于列生成的快速方法，以识别致癌多基因突变组合，降低计算需求。

**关键词**：癌症基因组学, 多基因突变组合, 约束规划, 混合整数规划, 列生成, 计算优化

## 3 点简述
- 核心问题：癌症多基因突变组合识别，建模为最大化覆盖肿瘤样本、最小化覆盖正常样本的二分类问题。
- 方法要点：采用约束规划和混合整数规划，结合列生成启发式，实现高效求解。
- 实验或效果：在真实癌症基因组数据上，单CPU一分钟内达到先进方法性能，部分实例可最优求解。

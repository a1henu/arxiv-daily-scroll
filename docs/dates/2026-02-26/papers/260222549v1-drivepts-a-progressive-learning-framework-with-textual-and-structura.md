...
**一句话要点**：提出DrivePTS框架，通过渐进学习、文本增强和结构损失提升驾驶场景生成的保真度和可控性。

**关键词**：驾驶场景生成, 扩散模型, 渐进学习, 视觉语言模型, 结构损失, 数据增强

## 3 点简述
- 核心问题：现有方法因几何条件间隐式依赖导致生成失败，且语义和结构细节不足。
- 方法要点：采用渐进学习策略减少条件依赖，利用视觉语言模型生成多视图描述，引入频率引导结构损失。
- 实验或效果：实验显示DrivePTS在保真度和可控性上达到最优，能生成罕见场景，泛化能力强。

...
**一句话要点**：提出基于不可变推理快照的诊断对齐框架，以量化临床AI与专家验证的校正动态。

**关键词**：临床AI诊断对齐, 不可变推理快照, 专家验证建模, 视觉大语言模型, 医学实体提取, 结构化信号分析

## 3 点简述
- 核心问题：安全关键临床AI中，模型推理与专家校正间的过渡缺乏结构化信号分析。
- 方法要点：集成视觉大语言模型、BERT医学实体提取和SLMI步骤，生成不可变推理状态进行对齐比较。
- 实验或效果：在21个皮肤病案例中，综合对齐率达100%，显示二元词汇评估低估临床意义对齐。

...
**一句话要点**：提出DMAligner，通过扩散模型合成新视角以增强图像对齐，解决遮挡和光照变化问题。

**关键词**：图像对齐, 扩散模型, 视角合成, 动态场景处理, 数据集构建

## 3 点简述
- 核心问题：传统光流图像扭曲方法易受遮挡和光照变化影响，导致对齐质量下降。
- 方法要点：采用基于扩散的生成框架，结合动态感知掩码模块，自适应区分动态前景与静态背景。
- 实验或效果：在自建DSIA数据集和广泛视频数据集上验证了方法的优越性，代码已开源。

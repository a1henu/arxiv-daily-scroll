...
**关键词**：公平机器学习, 后处理优化, 资源约束, 阈值调整, 伦理权衡, 高风险应用

## 3 点简述
- 核心问题：现有公平性干预假设资源无约束，违反反歧视法规，需在资源受限环境中操作化公平性。
- 方法要点：引入模型无关的后处理阈值优化，使用参数化伦理损失函数和全局决策阈值，确保法律合规和资源不超限。
- 实验或效果：在严格容量限制下，框架保持高风险识别能力，而标准无约束启发式方法效用接近零。

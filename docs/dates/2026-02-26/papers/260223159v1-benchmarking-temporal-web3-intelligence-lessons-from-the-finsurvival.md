...
**关键词**：时间Web3基准, 生存预测, 去中心化平台, 时间特征构建, 用户行为建模

## 3 点简述
- 核心问题：Temporal Web3领域缺乏捕捉现实世界时间动态（如删失和非平稳性）的共享基准，阻碍方法进展。
- 方法要点：基于Aave v3协议的2180万交易记录，设计16个生存预测任务，建模用户行为转换。
- 实验或效果：领域感知的时间特征构建显著优于通用建模方法，为下一代时间基准提供经验。

...
**一句话要点**：提出锚定技术以分析独立训练模型间的预测分歧，并应用于多种算法证明分歧收敛性。

**关键词**：模型分歧, 锚定技术, 独立训练, 实值预测, 强凸损失, 算法分析

## 3 点简述
- 核心问题：研究独立训练模型在实值预测中的分歧，定义为预测平方差的期望。
- 方法要点：开发基于锚定到两模型平均的通用技术，用于证明分歧上界。
- 实验或效果：应用该技术证明堆叠聚合、梯度提升、神经网络架构搜索和回归树训练中分歧随参数趋零。

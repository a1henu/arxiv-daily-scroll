...
**一句话要点**：提出AgentSentry框架，通过时序因果诊断与上下文净化，缓解LLM代理中的间接提示注入攻击。

**关键词**：间接提示注入, LLM代理安全, 时序因果诊断, 上下文净化, 推理时防御, 工具增强代理

## 3 点简述
- 核心问题：LLM代理依赖外部工具时易受间接提示注入攻击，攻击者通过工具输出或检索内容在多轮交互中隐秘操控代理行为。
- 方法要点：首次将多轮间接提示注入建模为时序因果接管，通过受控反事实重执行定位接管点，并基于因果引导的上下文净化移除攻击偏差。
- 实验或效果：在AgentDojo基准测试中，AgentSentry消除所有成功攻击，攻击下平均效用达74.55%，比最强基线提升20.8至33.6个百分点。

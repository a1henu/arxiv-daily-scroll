...
**关键词**：无人机自主探索, 骨架图构建, 分层规划, 计算效率优化, 未知环境感知

## 3 点简述
- 核心问题：现有方法依赖频繁全局优化，导致高计算延迟和轨迹振荡，尤其在资源受限设备上。
- 方法要点：基于骨架图增量构建和隐式未知区域分析，采用分层按需规划策略，包括近端规划器和区域序列规划器。
- 实验或效果：仿真评估显示，SCOPE在保持探索性能的同时，平均降低计算成本86.9%，真实实验验证了其鲁棒性和低延迟。

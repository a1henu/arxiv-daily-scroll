...
**关键词**：无源域自适应目标检测, 对象中心学习, 槽感知适应, 类引导对比, DETR检测器, 隐私敏感适应

## 3 点简述
- 核心问题：无源域自适应目标检测中，现有方法忽视跨域数据的对象级结构线索。
- 方法要点：集成层次槽感知模块和类引导槽对比模块，以槽表示作为视觉先验，促进领域不变适应。
- 实验或效果：在多个跨域数据集上优于先前方法，验证了对象中心设计在隐私敏感场景中的潜力。

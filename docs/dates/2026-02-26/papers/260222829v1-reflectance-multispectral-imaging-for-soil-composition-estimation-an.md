...
**一句话要点**：提出基于反射多光谱成像的土壤成分估计与USDA质地分类方法，用于农业与岩土工程现场部署。

**关键词**：多光谱成像, 土壤质地分类, USDA质地三角形, 回归模型, 现场部署, 非破坏检测

## 3 点简述
- 核心问题：土壤质地传统检测方法耗时费力，现有传感技术成本高或分辨率不足，难以支持常规现场应用。
- 方法要点：开发低成本多光谱成像系统（365-940 nm，13个波段），结合回归模型估计黏土、粉砂、砂百分比，并直接或间接分类USDA质地类别。
- 实验或效果：在混合数据上评估，成分预测R^2高达0.99，质地分类准确率超过99%，表明方法准确、非破坏且适合现场部署。

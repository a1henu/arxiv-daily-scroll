...
**一句话要点**：提出模态坍缩为解码器失配问题，揭示多模态大语言模型的信息提取极限

**关键词**：多模态大语言模型, 模态坍缩, 解码器失配, 广义互信息, 信息提取极限, 评分规则

## 3 点简述
- 核心问题：多模态LLMs无法有效利用语音身份、情感等非文本信息，尽管编码器保留了这些特征
- 方法要点：将问题形式化为解码器失配，用广义互信息理论界定可访问信息边界
- 实验效果：通过控制实验和干预训练验证解码器评分规则是瓶颈，调整目标可提升特定信息可访问性

...
**一句话要点**：提出MUG方法以解决异质图通用预训练中的统一表示和元路径适应挑战

**关键词**：异质图预训练, 元路径感知, 通用图表示学习, 输入统一模块, 共享编码器

## 3 点简述
- 核心问题：异质图因类型多样和元路径差异，难以构建通用预训练编码器
- 方法要点：通过输入统一模块和共享编码器，实现跨图对齐和结构模式捕获
- 实验或效果：在真实数据集上验证了MUG的有效性，提升下游任务泛化能力

...
**一句话要点**：提出波斯语短答案评测框架TARAZ，结合形态归一化与混合相似度模块以评估大语言模型文化能力。

**关键词**：波斯语文化评测, 短答案评估, 形态归一化, 混合相似度模块, 大语言模型评估, 跨文化研究

## 3 点简述
- 现有波斯语文化评测依赖选择题和英语中心指标，无法处理波斯语形态复杂性和语义细微差别。
- 引入波斯语特定短答案评测，结合规则形态归一化与混合句法语义相似度模块，实现超越精确匹配的软匹配评分。
- 系统评估15个先进模型，混合评测相比精确匹配基线提升10%评分一致性，并公开框架作为首个标准化波斯语文化理解基准。

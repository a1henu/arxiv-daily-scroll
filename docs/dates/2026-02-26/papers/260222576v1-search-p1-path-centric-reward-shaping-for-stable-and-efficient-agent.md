...
**一句话要点**：提出Search-P1框架，通过路径中心奖励塑形解决Agentic RAG训练中的稀疏奖励和低样本效率问题。

**关键词**：检索增强生成, 强化学习训练, 路径中心奖励, 样本效率, 推理轨迹评估, 问答基准测试

## 3 点简述
- 核心问题：Agentic RAG的RL训练存在稀疏结果奖励和低样本效率，丢弃中间信号且失败样本无贡献。
- 方法要点：引入路径中心奖励，通过顺序无关步骤覆盖和软评分评估推理轨迹结构质量，并采用双轨路径评分结合自一致性和参考对齐。
- 实验或效果：在多个QA基准测试中，Search-P1相比Search-R1等基线平均准确率提升7.7点。

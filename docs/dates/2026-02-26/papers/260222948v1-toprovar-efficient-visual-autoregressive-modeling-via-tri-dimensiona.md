...
**一句话要点**：提出ToProVAR框架，通过三维熵感知语义分析与稀疏性优化，高效加速视觉自回归模型生成过程。

**关键词**：视觉自回归模型, 注意力熵分析, 稀疏性优化, 生成加速, 语义保真度, 三维优化策略

## 3 点简述
- 核心问题：视觉自回归模型在生成后期面临效率瓶颈，传统方法如FastVAR和SkipVAR依赖启发式跳过策略，效果有限。
- 方法要点：利用注意力熵分析模型架构中不同维度的语义投影，识别参数动态，并基于token、层和尺度三维稀疏性模式设计细粒度优化策略。
- 实验或效果：在Infinity-2B和Infinity-8B模型上评估，实现最高3.4倍加速，语义保真度和细节保留显著优于传统方法，质量损失最小。

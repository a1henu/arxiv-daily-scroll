...
**一句话要点**：提出S2O方法，通过在线置换实现稀疏注意力的早期停止，以提升长上下文推理效率。

**关键词**：稀疏注意力, 长上下文推理, 在线置换, 早期停止, 计算优化, Transformer加速

## 3 点简述
- 核心问题：注意力机制随序列长度二次方缩放，现有块粒度稀疏化方法存在稀疏性上限，难以进一步提升效率。
- 方法要点：S2O基于在线置换和重要性引导的加载策略，将显式置换转化为轻量级索引指导的离散加载，并引入早期停止规则，从高到低重要性计算，在误差预算控制下跳过低贡献块。
- 实验或效果：在Llama-3.1-8B的128K上下文上，S2O在匹配稀疏度下降低单算子MSE 3.82倍，在匹配MSE下降低预填充计算密度 3.31倍，同时保持端到端精度，实现注意力加速 7.51倍和端到端加速 3.81倍。

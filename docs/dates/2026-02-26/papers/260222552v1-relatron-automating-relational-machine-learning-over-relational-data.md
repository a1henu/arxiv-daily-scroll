...
**一句话要点**：提出Relatron任务嵌入元选择器，以自动化关系数据库上的机器学习模型选择与优化。

**关键词**：关系机器学习, 模型选择, 任务嵌入, 架构搜索, 关系数据库, 自动化优化

## 3 点简述
- 核心问题：关系深度学习与深度特征合成在关系数据库任务中性能差异不明确，缺乏任务感知的模型选择原则。
- 方法要点：通过统一设计空间分析，引入任务同质性和亲和嵌入信号，指导Relatron在RDL与DFS间选择并剪枝搜索。
- 实验或效果：Relatron在联合超参数-架构优化中，相比基线提升达18.5%，成本降低10倍，解决过度调优性能下降问题。

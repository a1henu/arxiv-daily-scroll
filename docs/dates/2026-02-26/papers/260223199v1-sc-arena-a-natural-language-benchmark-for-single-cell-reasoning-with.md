...
**一句话要点**：提出SC-Arena评估框架，以解决单细胞生物学中LLM评估的碎片化与缺乏生物学基础问题。

**关键词**：单细胞生物学, 大语言模型评估, 知识增强评估, 虚拟细胞抽象, 自然语言任务

## 3 点简述
- 核心问题：现有单细胞生物学LLM评估基准任务分散、格式不切实际、指标缺乏可解释性和生物学基础。
- 方法要点：引入虚拟细胞抽象统一评估目标，定义五个自然语言任务，并采用知识增强评估整合外部知识。
- 实验或效果：实验显示模型在复杂任务上表现不均，知识增强评估提高了生物学正确性、可解释性和判别能力。

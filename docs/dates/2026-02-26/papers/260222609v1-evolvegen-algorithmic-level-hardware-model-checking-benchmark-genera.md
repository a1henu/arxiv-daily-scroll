...
**一句话要点**：提出EvolveGen框架，通过强化学习与高层次合成生成硬件模型检查基准，以解决基准不足与偏差问题。

**关键词**：硬件模型检查, 基准生成, 强化学习, 高层次合成, 算法抽象, 求解器评估

## 3 点简述
- 硬件模型检查面临基准数量有限、格式受限且难度极端，阻碍技术评估与求解器优化。
- 结合强化学习与高层次合成，在算法层面构建计算图，生成功能等效但结构不同的硬件设计对。
- 实验表明，EvolveGen能高效生成多样化基准，有效揭示先进模型检查器的性能瓶颈。

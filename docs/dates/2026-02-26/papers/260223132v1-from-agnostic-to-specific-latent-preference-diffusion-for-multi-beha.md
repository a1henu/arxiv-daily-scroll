...
**一句话要点**：提出FatsMB框架，通过潜在偏好扩散从行为无关到行为特定，以解决多行为序列推荐中的不确定性和多样性问题。

**关键词**：多行为序列推荐, 扩散模型, 潜在偏好学习, 行为特定建模, 不确定性处理

## 3 点简述
- 核心问题：现有方法忽略用户潜在偏好，且偏好评分范式难以处理从低熵行为到高熵项目的不确定性，导致推荐效果不佳。
- 方法要点：设计多行为自动编码器构建统一潜在空间，结合行为感知RoPE进行信息融合，并通过多条件引导层归一化进行去噪，实现行为特定偏好转移。
- 实验或效果：在真实数据集上广泛实验，验证了模型在准确性和多样性方面的有效性。

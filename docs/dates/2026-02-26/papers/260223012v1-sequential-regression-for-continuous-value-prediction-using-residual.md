...
**关键词**：连续值预测, 残差量化, 序列学习, 推荐系统, 表示学习, 长尾分布

## 3 点简述
- 核心问题：连续值预测面临数据分布复杂长尾，现有生成方法依赖参数假设，性能受限。
- 方法要点：将目标值表示为有序量化码之和，通过残差量化递归预测，并引入表示学习对齐嵌入空间。
- 实验或效果：在LTV、观看时长预测基准和工业短视频平台GMV预测中，超越现有方法，展现强泛化能力。

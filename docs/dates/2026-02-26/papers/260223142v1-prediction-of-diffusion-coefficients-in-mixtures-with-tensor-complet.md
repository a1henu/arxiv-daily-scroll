...
## 3 点简述
- 核心问题：现有矩阵补全方法仅限单温度预测，依赖高质量实验数据，难以准确预测温度依赖扩散系数。
- 方法要点：采用Tucker分解的混合张量补全方法，结合贝叶斯训练框架，利用SEGWE模型作为先验知识，实现温度线性外推。
- 实验或效果：通过主动学习策略扩展实验数据库，结合新测量数据，显著提升预测准确性，优于现有模型。

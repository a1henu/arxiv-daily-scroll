...
**一句话要点**：提出FedWQ-CP方法，在联邦学习中通过保形预测解决双重异构性下的不确定性量化问题。

**关键词**：联邦学习, 不确定性量化, 保形预测, 双重异构性, 覆盖可靠性

## 3 点简述
- 核心问题：联邦学习面临数据与模型双重异构性，现有不确定性量化方法常忽视其联合影响，导致覆盖可靠性不足。
- 方法要点：FedWQ-CP在单轮通信中执行代理-服务器校准，代理计算本地分位数阈值并传输至服务器，服务器加权平均生成全局阈值。
- 实验或效果：在七个公共数据集上，FedWQ-CP在分类和回归任务中保持代理级和全局覆盖，同时产生最小的预测集或区间。

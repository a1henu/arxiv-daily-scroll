...
**一句话要点**：提出利用LLM生成文本相关性标签以增强应用商店搜索排序，解决专家标签稀缺问题。

**关键词**：搜索排序优化, LLM生成标签, 文本相关性, 行为相关性, 应用商店搜索, A/B测试验证

## 3 点简述
- 核心问题：商业搜索系统中文本相关性标签稀缺，而行为相关性标签丰富，影响排序优化。
- 方法要点：通过系统评估LLM配置，采用微调模型生成数百万文本相关性标签，作为生产排序器的补充信号。
- 实验或效果：离线NDCG提升，全球A/B测试显示转化率显著增加+0.24%，尾部查询性能改善最明显。

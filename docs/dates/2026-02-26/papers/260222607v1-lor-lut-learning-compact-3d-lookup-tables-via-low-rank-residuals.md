...
**一句话要点**：提出LoR-LUT，通过低秩残差学习紧凑3D查找表以提升图像增强效率与可解释性。

**关键词**：3D查找表, 低秩残差, 图像增强, 模型压缩, 可解释性, 风格迁移

## 3 点简述
- 核心问题：传统3D-LUT方法依赖密集基LUT融合，导致模型参数多、效率低。
- 方法要点：引入低秩残差校正与基LUT联合优化，减少参数同时保持三线性插值复杂度。
- 实验或效果：在MIT-Adobe FiveK数据集上实现专家级调色效果，模型尺寸小于1MB，并开发交互可视化工具增强可解释性。

...
**一句话要点**：提出FlashOptim优化器套件以减少训练内存占用，保持模型质量与API兼容性。

**关键词**：内存高效训练, 优化器量化, 混合精度训练, 模型微调, 参数压缩

## 3 点简述
- 标准混合精度训练中，每个参数需多字节存储，导致大模型训练内存需求高。
- 通过改进主权重分割和设计压缩函数，减少量化误差，降低内存使用。
- 实验在视觉和语言基准上应用，包括Llama-3.1-8B微调，未见质量下降。

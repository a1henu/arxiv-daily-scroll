...
## 3 点简述
- 核心问题：大型推理模型在低复杂度查询中常出现过思考行为，现有方法在精度-效率权衡上不稳定且对异构推理行为鲁棒性差。
- 方法要点：采用混合微调初始化，结合正确性保持优势塑造和长度感知梯度调节进行自适应强化学习，避免抑制正确长链推理并稳定优化。
- 实验或效果：在Qwen2.5-1.5B和7B模型上实验，精度提升最高达+3.7点，生成令牌减少超40%，并在不同难度和分布外任务中验证鲁棒性。

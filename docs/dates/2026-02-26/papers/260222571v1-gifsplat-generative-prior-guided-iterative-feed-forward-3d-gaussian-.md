...
**关键词**：3D重建, 稀疏视图, 前馈网络, 迭代优化, 生成先验, 高斯溅射

## 3 点简述
- 核心问题：现有前馈方法在稀疏视图下性能有限，引入生成先验后难以保持秒级推理时间。
- 方法要点：通过迭代残差更新逐步优化3D场景，并蒸馏扩散先验为高斯级线索，无需梯度反向传播。
- 实验或效果：在多个数据集上优于现有方法，PSNR提升最高达+2.1 dB，保持秒级推理时间。

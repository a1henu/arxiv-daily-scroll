...
**关键词**：成员推理攻击, 差分隐私, 中间表示, 层间自适应, 隐私-效用权衡, 嵌入即接口

## 3 点简述
- 核心问题：中间表示分布泄露训练集成员信息，现有DP-SGD忽略层间MIA风险异质性。
- 方法要点：基于影子模型估计层间MIA风险，动态调整梯度贡献权重，实现层间自适应隐私保护。
- 实验或效果：相同隐私预算下，降低峰值MIA风险，保持模型效用，提升隐私-效用权衡。

...
**一句话要点**：提出基于因子图的蓝牙相控阵辅助惯性导航方法，用于GNSS拒止场景的实验验证。

**关键词**：蓝牙相控阵导航, 因子图优化, 惯性导航辅助, GNSS拒止场景, 稳健估计, 无人机实验

## 3 点简述
- 核心问题：在GNSS拒止场景（如仓库物流）中，低成本蓝牙相控阵系统测量噪声大、范围短，需稳健导航策略。
- 方法要点：采用因子图优化估计器，结合蓝牙角度测量、距离或气压数据，评估稳健估计策略。
- 实验或效果：通过多旋翼无人机飞行实验数据，比较不同辅助测量下的性能，验证系统可行性。

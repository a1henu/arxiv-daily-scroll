...
**关键词**：医学视觉语言模型, 不确定性校准, 分形预测, 转导适应, 拉普拉斯平滑, 零样本学习

## 3 点简述
- 医学视觉语言模型在域移下不确定性校准效率低且类别覆盖不平衡，传统方法易破坏交换性保证。
- LATA基于图像-图像k-NN图平滑零样本概率，结合失败感知分数，无需训练或标签更新，保持分形预测有效性。
- 在三个医学视觉语言模型和九个下游任务中，LATA减少预测集大小和类别覆盖差距，匹配或收紧目标覆盖，计算轻量。

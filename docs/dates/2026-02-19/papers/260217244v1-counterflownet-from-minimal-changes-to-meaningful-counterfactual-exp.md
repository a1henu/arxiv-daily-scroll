...
**一句话要点**：提出CounterFlowNet，基于条件生成流网络生成高质量反事实解释，解决表格数据中多约束解释难题。

**关键词**：反事实解释, 生成流网络, 表格数据, 可解释人工智能, 序列生成, 约束满足

## 3 点简述
- 现有方法难以生成同时满足稀疏性、异质特征处理及用户约束的高质量反事实解释。
- 采用条件生成流网络，将反事实生成建模为序列特征修改，通过奖励函数编码有效性、稀疏性等关键需求。
- 在八个数据集上实验显示，该方法在有效性、稀疏性、合理性和多样性间取得优越平衡，完全满足约束。

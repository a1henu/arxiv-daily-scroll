...
**一句话要点**：提出多探针零碰撞哈希以解决大规模推荐系统中嵌入碰撞和模型新鲜度问题

**关键词**：嵌入碰撞缓解, 模型新鲜度增强, 大规模推荐系统, 线性探测哈希, TorchRec开源

## 3 点简述
- 传统哈希索引在ID数量增长时导致嵌入碰撞，降低模型性能和个人化质量
- 基于线性探测的多探针零碰撞哈希通过辅助张量和CUDA内核实现可配置探测和主动驱逐策略
- 在线实验显示用户嵌入零碰撞，显著提升物品嵌入新鲜度和质量，训练和推理效率与现有方法相当

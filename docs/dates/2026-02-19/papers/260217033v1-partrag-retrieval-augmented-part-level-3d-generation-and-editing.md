...
**一句话要点**：提出PartRAG框架，通过检索增强实现单图像3D生成与部件级编辑，解决部件几何多样性和多视图一致性问题。

**关键词**：3D生成, 部件级编辑, 检索增强, 扩散变换器, 多视图一致性

## 3 点简述
- 核心问题：单图像3D生成中部件几何覆盖不足、多视图一致性差，且缺乏精确局部编辑支持。
- 方法要点：结合外部部件数据库与扩散变换器，采用分层对比检索模块和掩码部件编辑器，实现生成与编辑的耦合。
- 实验或效果：在Objaverse等数据集上降低Chamfer距离至0.1528，提升F-Score至0.844，支持快速交互编辑。

...
**一句话要点**：提出高效算法为神经加法模型生成可证明最小基数解释，解决解释方法缺乏可证明保证的问题。

**关键词**：神经加法模型, 可证明解释, 最小基数解释, 后验解释方法, 模型验证, 计算效率

## 3 点简述
- 核心问题：现有神经网络后验解释方法多为启发式，缺乏可证明保证，最小特征子集解释计算不可行。
- 方法要点：针对神经加法模型，设计模型特定算法，通过并行预处理和对数级验证查询生成可证明最小基数解释。
- 实验或效果：算法在解释大小和计算时间上优于现有方法，提供采样技术无法实现的可证明解释优势。

...
**关键词**：语义通信, 联邦学习, 潜在空间对齐, 多用户系统, 目标导向通信

## 3 点简述
- 核心问题：AI原生设备间潜在表示差异导致语义不匹配，阻碍任务执行
- 方法要点：在接入点和用户端部署语义均衡器，通过联邦优化实现去中心化训练
- 实验或效果：数值结果验证了方法在目标导向语义通信中的有效性，揭示了准确性、开销、复杂性和语义邻近度间的权衡

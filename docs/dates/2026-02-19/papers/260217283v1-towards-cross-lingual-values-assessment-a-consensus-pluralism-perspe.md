...
**一句话要点**：提出跨语言价值观评估基准X-Value，以提升大语言模型在内容安全中的深层价值观识别能力。

**关键词**：跨语言价值观评估, 内容安全, 大语言模型基准, Schwartz价值观理论, 共识-多元框架

## 3 点简述
- 当前大语言模型评估忽视内容中的深层价值观维度，存在评估空白。
- 基于Schwartz价值观理论构建多语言数据集，采用共识-多元两阶段标注框架。
- 实验显示现有模型在跨语言价值观评估中表现不足，准确率低于77%，语言间差异显著。

...
**关键词**：自适应免疫库分析, 近次二次检索, 多模态融合, 公平约束聚类, GPU加速计算, 生物信息学平台

## 3 点简述
- 核心问题：自适应免疫库的大规模比较分析受限于近二次的成对亲和力评估成本和数据不平衡。
- 方法要点：结合抗原感知的近次二次检索、GPU加速亲和力核、学习多模态融合和公平约束聚类。
- 实验或效果：在大型病毒和肿瘤库上，SubQuad提高了吞吐量和内存效率，同时保持或改进召回率、聚类纯度和子群公平性。

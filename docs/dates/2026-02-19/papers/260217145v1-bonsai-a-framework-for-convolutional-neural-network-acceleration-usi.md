...
**关键词**：卷积神经网络, 模型剪枝, 加速框架, 准则函数, 迭代优化, 计算效率

## 3 点简述
- 核心问题：CNN模型规模增大导致计算和内存开销上升，现有剪枝方法缺乏统一实现和比较标准。
- 方法要点：引入Bonsai框架，支持基于准则的迭代剪枝，并定义标准语言以比较不同剪枝准则。
- 实验或效果：在VGG类模型上剪枝最多79%滤波器，保持或提升精度，计算量减少最多68%。

...
**关键词**：交互式对象检索, 意图状态, 对比对齐, 开放词汇检测, 用户反馈精炼, 复杂场景消歧

## 3 点简述
- 核心问题：现有开放词汇检测器缺乏基于用户反馈的预测精炼能力，难以处理模糊或多相似对象查询。
- 方法要点：引入意图状态维护正负记忆集，通过对比对齐函数排名候选对象，实现细粒度消歧。
- 实验或效果：在LVIS上AP达35.4，优于基准方法；在LVIS-Ambiguous上单次反馈提升+7.9 AP，交互延迟低于30毫秒。

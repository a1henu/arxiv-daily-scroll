...
**关键词**：基因组基础模型, 联合嵌入预测架构, 潜在空间预测, 功能表示学习, 零样本任务

## 3 点简述
- 核心问题：现有基因组基础模型依赖掩码语言建模或下一标记预测，缺乏全局生物功能上下文。
- 方法要点：结合联合嵌入预测架构与生成目标，在潜在空间监督CLS标记以预测掩码片段的高层功能嵌入。
- 实验或效果：在多样化基因组基准测试中，JEPA-DNA在监督和零样本任务上表现优于仅生成基线。

...
**关键词**：长视野任务, 轨迹分割SFT, 渐进式强化学习, LLM代理训练, 研究论文分析, 自动化数据生成

## 3 点简述
- 核心问题：训练LLM代理处理极长视野任务，如研究论文分析，面临轨迹过长和训练效率挑战。
- 方法要点：采用轨迹分割SFT保留早期上下文并渐进截断，结合渐进式RL分阶段扩展超时以提升能力。
- 实验或效果：KLong在PaperBench上超越Kimi K2 Thinking 11.28%，并在SWE-bench Verified等编码基准上泛化良好。

...
**一句话要点**：提出Wink系统以自动恢复编码代理的异常行为

**关键词**：编码代理, 异常行为恢复, 自干预系统, 工具调用失败, 生产环境部署, 代理轨迹分析

## 3 点简述
- 核心问题：编码代理常出现规范漂移、推理问题和工具调用失败等异常行为，影响开发流程。
- 方法要点：开发轻量级异步自干预系统Wink，通过观察代理轨迹提供针对性纠正指导。
- 实验或效果：在真实轨迹上评估，单次干预成功解决90%异常行为，生产环境A/B测试显著减少工具调用失败和工程师干预。

...
**一句话要点**：提出基于e值的锚定水印框架，以统一最优采样与任意时间有效推断，解决大语言模型内容检测问题。

**关键词**：统计水印, 任意时间有效推断, e值框架, 大语言模型检测, 最优采样, 测试超鞅

## 3 点简述
- 核心问题：现有统计水印方法缺乏采样分布选择原则，且依赖固定范围假设检验，无法支持有效早期停止。
- 方法要点：通过锚定分布近似目标模型，构建检测过程的测试超鞅，实现任意时间有效推断，并推导最优e值和期望停止时间。

...
**关键词**：时序信用分配, 大语言模型, 回顾式上下文学习, 稀疏奖励, 样本效率, 强化学习

## 3 点简述
- 核心问题：稀疏环境反馈下，传统时序信用分配方法依赖任务特定值函数，样本效率低且泛化性差。
- 方法要点：利用大语言模型的预训练知识，通过回顾式上下文学习将稀疏奖励转换为密集优势函数信号。
- 实验或效果：在BabyAI场景中，RICOL框架实现与传统在线强化学习算法相当的收敛性能，样本效率显著更高。

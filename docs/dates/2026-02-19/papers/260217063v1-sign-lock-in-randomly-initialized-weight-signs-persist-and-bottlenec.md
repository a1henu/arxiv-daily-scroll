...
**一句话要点**：提出符号锁定理论及初始化与正则化方法，以降低次比特模型压缩中的符号翻转率

**关键词**：模型压缩, 符号锁定, 初始化方法, 正则化, 次比特压缩, 权重符号

## 3 点简述
- 次比特模型压缩中，符号位成为固定成本瓶颈，且学习到的符号矩阵难以低秩近似
- 符号模式随机性主要源于初始化，权重符号在训练中锁定，翻转率低且服从几何尾分布
- 基于符号锁定理论，提出间隙初始化和轻量外漂正则化，将有效翻转率降至约10^{-3}，困惑度仅轻微增加

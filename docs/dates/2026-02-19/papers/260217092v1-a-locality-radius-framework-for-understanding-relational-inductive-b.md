...
**一句话要点**：提出局部半径框架以理解数据库学习中关系归纳偏置的影响

**关键词**：数据库学习, 关系归纳偏置, 图神经网络, 局部半径, 结构推理, 模式预测

## 3 点简述
- 核心问题：多跳结构推理在数据库任务中何时必要，关系归纳偏置的作用不明确
- 方法要点：引入局部半径作为衡量预测所需最小结构邻域的正式度量，假设性能取决于任务半径与架构深度的对齐
- 实验或效果：通过多任务实证研究，发现一致的偏置-半径对齐效应，支持假设

...
**一句话要点**：提出Deep-Flow框架，利用条件流匹配在谱流形上实现自动驾驶连续异常检测，以解决长尾安全场景识别瓶颈。

**关键词**：自动驾驶安全验证, 异常检测, 条件流匹配, 谱流形, Transformer编码器, 长尾场景

## 3 点简述
- 核心问题：L4自动驾驶安全验证受限于传统规则方法难以扩展检测罕见高风险长尾场景。
- 方法要点：采用OT-CFM在PCA降维的谱流形上建模人类驾驶行为概率密度，结合Transformer编码器处理多模态歧义。

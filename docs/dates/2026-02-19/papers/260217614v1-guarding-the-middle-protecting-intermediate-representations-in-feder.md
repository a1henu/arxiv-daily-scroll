...
**一句话要点**：提出KD-UFSL以保护联邦分裂学习中的中间表示，平衡隐私与模型效用

**关键词**：联邦分裂学习, 中间表示保护, 差分隐私, 数据重构攻击, 隐私增强技术, 大规模数据应用

## 3 点简述
- 核心问题：UFSL中客户端共享的中间表示易暴露私有数据，面临数据重构攻击风险
- 方法要点：结合微聚合和差分隐私，设计k-匿名差分隐私UFSL，减少中间数据泄露
- 实验或效果：在四个基准数据集上，KD-UFSL提升重构误差和降低结构相似性，同时保持全局模型效用

...
**关键词**：量子材料发现, 物理感知学习, 指令调优, 多模态大语言模型, 合成数据生成, 光学图像表征

## 3 点简述
- 核心问题：量子材料光学图像表征因对比度细微、标注数据少和实验条件差异而困难
- 方法要点：结合物理模拟数据生成、指令数据集和物理感知注意力模块提升模型泛化能力
- 实验或效果：建立QF-Bench基准，提供标准化评估协议以验证方法鲁棒性和可重复性

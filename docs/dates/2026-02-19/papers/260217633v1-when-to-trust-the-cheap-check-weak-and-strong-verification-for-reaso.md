...
**一句话要点**：提出弱-强验证策略以优化大语言模型推理中的验证成本与可靠性平衡

**关键词**：弱-强验证策略, 大语言模型推理, 验证成本优化, 在线算法, 错误控制, 校准与锐度

## 3 点简述
- 核心问题：大语言模型推理中，弱验证（如自一致性）快速但嘈杂，强验证（如用户反馈）可靠但资源密集，需决策何时接受、拒绝或转强验证。
- 方法要点：形式化弱-强验证策略，引入错误接受、错误拒绝和强验证频率指标，证明最优策略具有双阈值结构，校准和锐度决定弱验证器价值。
- 实验或效果：开发在线算法，无需假设查询流、语言模型或弱验证器，可证明控制接受和拒绝错误。

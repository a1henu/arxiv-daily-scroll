...
**一句话要点**：提出基于SO(3)的几何逆飞行动力学框架，应用于系留固定翼飞机轨迹设计。

**关键词**：逆飞行动力学, 几何建模, SO(3)群, 系留飞机, 轨迹设计, 协调飞行

## 3 点简述
- 核心问题：在SO(3)上建立坐标无关的逆飞行动力学，避免局部姿态坐标，用于固定翼飞机。
- 方法要点：通过力平衡和协调飞行假设，推导闭式轨迹到输入映射，包括姿态、角速度和推力-攻角对。
- 实验或效果：应用于系留飞行，解析得到所需滚转角，识别零滚转轨迹，展示气动协调与表观重力解耦。

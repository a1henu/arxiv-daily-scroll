...
**一句话要点**：提出RetouchIQ框架，通过通用奖励模型指导MLLM代理进行基于指令的图像精修。

**关键词**：多模态大语言模型, 强化学习, 图像编辑, 奖励模型, 指令执行, 专业软件集成

## 3 点简述
- 核心问题：专业图像编辑中缺乏可靠奖励信号，难以训练MLLM代理执行主观性编辑任务。
- 方法要点：使用RL微调的通用奖励模型，通过多模态推理生成案例特定指标，提供高质量梯度反馈。
- 实验或效果：在19万指令-推理对数据集上验证，显著提升语义一致性和感知质量，优于先前方法。

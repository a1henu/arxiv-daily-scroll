...
**一句话要点**：将MDP规划建模为策略贝叶斯推断，通过变分序列蒙特卡洛近似最优策略分布。

**关键词**：马尔可夫决策过程, 贝叶斯推断, 策略优化, 变分序列蒙特卡洛, 离散域规划, 不确定性建模

## 3 点简述
- 核心问题：将马尔可夫决策过程规划视为策略的贝叶斯推断，以分布形式捕捉最优策略不确定性。
- 方法要点：使用变分序列蒙特卡洛近似策略后验，引入状态一致性扫描和粒子耦合减少模拟器噪声影响。
- 实验或效果：在网格世界、Blackjack等离散域分析策略分布结构，与Soft Actor-Critic比较行为差异。

...
**一句话要点**：将MDP规划建模为策略的贝叶斯推断，以处理最优行为的不确定性。

**关键词**：马尔可夫决策过程, 贝叶斯推断, 策略优化, 变分序列蒙特卡洛, 不确定性建模, 离散规划

## 3 点简述
- 核心问题：将马尔可夫决策过程规划视为策略的贝叶斯推断，以捕捉最优策略的不确定性。
- 方法要点：采用变分序列蒙特卡洛方法近似后验分布，引入策略一致性机制和耦合转移随机性。
- 实验或效果：在多个离散环境中分析推断策略分布，并与离散软演员-评论家方法进行对比。

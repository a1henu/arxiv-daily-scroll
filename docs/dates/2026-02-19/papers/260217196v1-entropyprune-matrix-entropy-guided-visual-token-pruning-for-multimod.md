...
**关键词**：多模态大语言模型, 视觉令牌剪枝, 矩阵熵, 推理加速, 熵崩溃层, 高效计算

## 3 点简述
- 核心问题：多模态大语言模型推理成本高，现有令牌剪枝方法依赖启发式层选择，缺乏可解释性和可迁移性。
- 方法要点：引入矩阵熵视角，识别熵崩溃层作为剪枝阶段，量化视觉令牌信息价值并剪枝冗余令牌，不依赖注意力图。
- 实验或效果：在LLaVA-1.5-7B上实现68.2% FLOPs减少，性能保留96.0%，泛化至高分辨率和视频模型，提升准确性和效率。

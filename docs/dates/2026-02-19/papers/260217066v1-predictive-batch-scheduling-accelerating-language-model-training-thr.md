...
**一句话要点**：提出预测性批次调度以加速语言模型训练，通过动态优先处理高损失样本。

**关键词**：语言模型训练, 批次调度, 样本优先级, 课程学习, 损失预测, Transformer优化

## 3 点简述
- 核心问题：传统课程学习需预定义难度指标，硬样本挖掘需昂贵逐样本损失跟踪。
- 方法要点：使用轻量级线性预测器在线训练，从静态词元级特征估计样本难度。
- 实验或效果：在130M参数Transformer上，PBS实现6-13%更快收敛，预测器相关性从0.14提升至0.44。

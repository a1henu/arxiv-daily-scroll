...
**一句话要点**：提出基于大语言模型和约束优化的通用细粒度对称性推断与执行框架，以解决晶体结构预测中的物理保真度与新颖性限制问题。

**关键词**：晶体结构预测, 对称性推断, 大语言模型, 约束优化, 扩散模型, 材料发现

## 3 点简述
- 核心问题：现有晶体结构预测方法依赖已知结构模板，限制了物理保真度和发现新材料的能力。
- 方法要点：利用大语言模型编码化学语义直接生成Wyckoff模式，并通过约束优化确保代数一致性。
- 实验或效果：在稳定性、唯一性和新颖性基准上达到最先进性能，实现高效探索未知材料空间。

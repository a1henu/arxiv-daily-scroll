...
**一句话要点**：提出细粒度不确定性量化框架以提升长文本语言模型输出的真实性检测

**关键词**：不确定性量化, 长文本生成, 幻觉检测, 细粒度评分, 黑盒方法, 事实性提升

## 3 点简述
- 核心问题：现有不确定性量化方法针对短文本设计，难以泛化至长文本生成场景。
- 方法要点：构建三阶段分类法，包括响应分解、单元级评分和响应级聚合，并形式化基于一致性的黑盒评分器。
- 实验效果：在多个模型和数据集上验证，claim-response蕴含评分表现优异，不确定性感知解码有效提升事实性。

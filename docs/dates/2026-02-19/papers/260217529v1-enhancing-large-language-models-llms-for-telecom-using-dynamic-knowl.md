...
**一句话要点**：提出KG-RAG框架，结合知识图谱与检索增强生成，以提升大型语言模型在电信领域的准确性与可靠性。

**关键词**：知识图谱, 检索增强生成, 大型语言模型, 电信领域, 可解释性, 幻觉减少

## 3 点简述
- 核心问题：通用大型语言模型在电信领域因领域复杂性、标准演变和术语专业，易产生幻觉和输出不准确。
- 方法要点：集成知识图谱提供结构化领域知识，结合检索增强生成动态检索相关事实，以增强模型输出的准确性和可解释性。
- 实验或效果：在基准数据集上，KG-RAG相比仅用LLM和标准RAG基线，平均准确率分别提升21.6%和14.3%。

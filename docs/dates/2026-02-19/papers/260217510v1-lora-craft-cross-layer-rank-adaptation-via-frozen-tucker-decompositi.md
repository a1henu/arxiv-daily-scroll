...
**关键词**：参数高效微调, Tucker分解, 跨层张量, 注意力权重, HOSVD, 轻量适配

## 3 点简述
- 核心问题：现有张量分解方法在参数高效微调中未充分利用预训练权重跨层结构
- 方法要点：对跨层堆叠的注意力权重进行HOSVD分解，冻结因子并训练轻量适配矩阵
- 实验或效果：在GLUE基准上，仅需41K参数达到竞争性能，参数数与模型维度深度无关

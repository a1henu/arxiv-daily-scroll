...
**一句话要点**：提出VCPO以解决异步强化学习中策略梯度方差过高导致训练不稳定的问题。

**关键词**：异步强化学习, 策略梯度方差控制, 大语言模型优化, 有效样本量, 最小方差基线

## 3 点简述
- 核心问题：异步训练中，陈旧样本导致重要性比率重尾分布，策略梯度方差增大，学习不稳定。
- 方法要点：基于有效样本量调整学习率以抑制不可靠更新，并应用闭式最小方差基线减少方差。
- 实验或效果：在数学、通用推理和工具使用任务中提升鲁棒性，训练时间减少2.5倍，性能匹配同步训练。

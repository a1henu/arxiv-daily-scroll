...
**一句话要点**：提出基于隐私保护机制的廉价可验证LLM推理协议，解决第三方托管中的计算保证问题。

**关键词**：隐私保护推理, 可验证推理, 大语言模型, 第三方托管, 计算保证

## 3 点简述
- 核心问题：第三方LLM托管缺乏计算保证，如提供商可能用弱模型替换昂贵模型。
- 方法要点：利用隐私保护推理实现廉价可验证性，添加少量额外计算令牌。
- 实验或效果：协议计算开销低，对下游影响小，验证运行时间优于零知识证明方法。

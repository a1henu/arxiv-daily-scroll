...
**一句话要点**：提出基于Wasserstein自编码器的生成建模框架，以学习光注入激光系统中脉冲形状与下游束流动力学之间的可微分潜在接口。

**关键词**：生成建模, Wasserstein自编码器, 潜在空间学习, 光注入器, 激光脉冲形状, 束流动力学

## 3 点简述
- 核心问题：光注入器中纵向激光脉冲形状控制的设计空间广阔，但系统探索受限于脉冲传播模拟的高成本。
- 方法要点：使用Wasserstein自编码器学习潜在空间，实现脉冲形状的高保真重建和可解释性，支持线性插值平滑过渡。
- 实验或效果：模型从模拟数据泛化到真实实验脉冲测量，准确重建脉冲并嵌入学习流形，减少对昂贵模拟的依赖。

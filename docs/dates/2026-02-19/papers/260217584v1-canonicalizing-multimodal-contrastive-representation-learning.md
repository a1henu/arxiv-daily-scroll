...
**一句话要点**：提出多模态对比表示的正则化方法，通过正交映射对齐独立训练模型，实现向后兼容升级。

**关键词**：多模态对比学习, 表示对齐, 正交映射, 模型升级, 隐私保护

## 3 点简述
- 核心问题：独立训练的多模态对比模型间是否存在系统几何关系，以统一对齐图像和文本编码器。
- 方法要点：证明若模型在锚点集上多模态核一致，则存在单一正交映射同时对齐图像和文本表示。
- 实验或效果：在CLIP、SigLIP和FLAVA等模型上验证正交映射近似成立，支持模型升级和隐私应用。

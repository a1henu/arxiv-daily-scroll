...
**一句话要点**：提出Jolt Atlas框架，通过查找参数实现零知识机器学习推理验证，适用于隐私和对抗环境。

**关键词**：零知识机器学习, 查找参数, ONNX张量操作, 流式证明, 隐私保护, 模型验证

## 3 点简述
- 核心问题：现有零知识机器学习框架在模型推理验证中效率低，难以在内存受限环境中应用。
- 方法要点：基于Jolt证明系统，直接适配ONNX张量操作，利用查找参数和优化技术如神经传送减少查找表大小。
- 实验或效果：展示分类、嵌入、自动推理和小语言模型的实用证明时间，支持流式证明和零知识属性。

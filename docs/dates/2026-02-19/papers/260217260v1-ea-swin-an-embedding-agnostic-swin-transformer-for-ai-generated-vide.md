...
**一句话要点**：提出EA-Swin模型以解决AI生成视频检测中现有方法依赖浅层嵌入轨迹或计算量大的问题。

**关键词**：AI生成视频检测, Swin Transformer, 时空依赖建模, 因子化窗口注意力, 跨分布泛化, 视频嵌入

## 3 点简述
- 核心问题：现有AI生成视频检测方法依赖浅层嵌入轨迹、图像适应或计算量大的MLLMs，难以应对高度逼真的合成视频。
- 方法要点：EA-Swin通过因子化窗口注意力设计，直接在预训练视频嵌入上建模时空依赖，兼容通用ViT风格编码器。
- 实验或效果：在EA-Video数据集上，EA-Swin准确率达0.97-0.99，优于先前方法5-20%，并保持对未见分布的强泛化能力。

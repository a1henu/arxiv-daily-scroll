...
**arXiv**：[2602.17242v1](https://arxiv.org/abs/2602.17242) · [PDF](https://arxiv.org/pdf/2602.17242.pdf)  
**作者**：Takao Inoué  

**一句话要点**：提出TAPO描述逻辑以建模信息行为，扩展静态知识表示与动态交互过程。

**关键词**：描述逻辑, 信息行为建模, 程序盒, 预言盒, 层论语义, 动态知识表示

## 3 点简述
- 核心问题：传统描述逻辑难以建模信息行为的动态、交互特性。
- 方法要点：扩展T-Box/A-Box为四层架构，增加程序盒和预言盒支持动作与外部交互。
- 实验或效果：基于层论语义提供统一框架，将信息真值定义为交互稳定性。

## 摘要（原文）

> We introduce \emph{TAPO-Structured Description Logic} (TAPO--DL), a formal extension of classical description logic designed to model \emph{information behavior} as a structured, dynamic process.

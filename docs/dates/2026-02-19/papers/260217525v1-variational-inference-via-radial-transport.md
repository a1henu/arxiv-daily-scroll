...
**一句话要点**：提出radVI算法以优化径向剖面，提升变分推断在高维分布近似中的覆盖效果。

**关键词**：变分推断, 径向传输, Wasserstein空间, 高斯近似, 优化算法, 理论保证

## 3 点简述
- 变分推断中高斯分布可能无法准确捕获目标分布的径向剖面，导致覆盖不足。
- radVI算法通过优化径向剖面，作为现有变分推断方案的廉价有效附加组件。

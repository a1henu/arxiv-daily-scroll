...
**一句话要点**：提出固定心电图表示评估标准以提升临床相关性与可靠性

**关键词**：心电图表示学习, 临床评估基准, 多标签分类, 不平衡数据集, 随机编码器基线, 患者预测

## 3 点简述
- 核心问题：当前12导联心电图表示学习基准局限于心律失常和波形形态标签，忽略更广泛临床信息如结构性心脏病和患者预测。
- 方法要点：建议扩展下游评估至结构性心脏病和患者级预测，并应用多标签不平衡设置的最佳评估实践。
- 实验或效果：实证显示随机初始化编码器在线性评估中匹配先进预训练方法，支持其作为合理基线模型。

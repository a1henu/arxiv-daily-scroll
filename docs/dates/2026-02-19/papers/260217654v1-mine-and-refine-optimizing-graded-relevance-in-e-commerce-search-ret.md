...
**一句话要点**：提出两阶段'挖掘与精炼'对比训练框架，以优化电商搜索中分级相关性的语义嵌入检索。

**关键词**：电商搜索检索, 语义嵌入, 对比学习, 分级相关性, 双塔模型, 圆损失

## 3 点简述
- 核心问题：电商搜索需处理长尾噪声查询，且相关性常分级，需嵌入能区分不同相关层级以支持混合排序。
- 方法要点：第一阶段用标签感知对比目标训练双塔检索器；第二阶段挖掘难样本并引入多类圆损失精炼嵌入空间边界。
- 实验或效果：离线评估和生产A/B测试显示，框架提升检索相关性，带来显著的用户参与和业务收益增益。

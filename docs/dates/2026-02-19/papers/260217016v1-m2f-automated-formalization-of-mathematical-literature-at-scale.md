...
**一句话要点**：提出M2F框架，实现教科书级数学文献的端到端自动形式化验证。

**关键词**：自动形式化, 数学验证, 代理框架, Lean语言, 教科书规模, 端到端编译

## 3 点简述
- 核心问题：现有自动形式化方法局限于孤立定理，难以处理教科书级跨文件依赖和编译。
- 方法要点：采用两阶段代理框架，先编译声明骨架再修复证明，保持验证器在循环中确认改进。
- 实验效果：在约三周内将479页教科书转化为15万行Lean代码，证明成功率96%，显著提升效率。

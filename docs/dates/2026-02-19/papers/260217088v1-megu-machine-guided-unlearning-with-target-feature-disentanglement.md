...
**一句话要点**：提出MeGU框架，通过概念感知重对齐和目标特征解缠，解决机器遗忘中的权衡问题。

**关键词**：机器遗忘, 特征解缠, 概念感知重对齐, 多模态大语言模型, 隐私保护

## 3 点简述
- 核心问题：现有遗忘方法在擦除目标数据影响与保留模型效用间存在根本权衡。
- 方法要点：利用多模态大语言模型引导概念感知重对齐，引入正负特征噪声对解缠目标概念。
- 实验或效果：实现可控选择性遗忘，有效缓解欠遗忘和过遗忘。

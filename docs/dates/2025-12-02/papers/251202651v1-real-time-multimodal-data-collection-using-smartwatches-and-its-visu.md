...
**一句话要点**：提出Watch-DMLT和ViSeDOPS工具，以解决教育中多模态数据采集与可视化的可扩展性问题。

**关键词**：多模态学习分析, 智能手表数据采集, 实时监控, 教育可视化, 生理信号分析, 课堂部署

## 3 点简述
- 核心问题：教育中缺乏可扩展、同步、高分辨率的多模态数据采集工具，阻碍多模态学习分析的实际应用。
- 方法要点：开发Watch-DMLT用于智能手表实时采集生理和运动数据，ViSeDOPS用于仪表盘可视化同步多模态数据。
- 实验或效果：在65名学生课堂部署中，成功采集心率、运动、注视等多模态数据，验证了系统的可行性和实用性。

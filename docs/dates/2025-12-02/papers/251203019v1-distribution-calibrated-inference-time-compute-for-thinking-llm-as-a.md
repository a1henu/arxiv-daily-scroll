...
**一句话要点**：提出分布校准的推理时计算聚合方法，以提升大语言模型作为评判者的可靠性

**关键词**：大语言模型评判, 推理时计算, 分布校准, 成对偏好评估, Bradley-Terry模型, 噪声聚合

## 3 点简述
- 核心问题：大语言模型作为成对偏好评判者时，单样本噪声大且常见聚合规则在允许平局时不一致
- 方法要点：基于Bradley-Terry-Davidson模型，利用极性（非平局边际）和决断性（非平局率）校准评分分布
- 实验或效果：在多个基准测试中降低MAE、提高成对准确率，匹配或超越人类评判者共识

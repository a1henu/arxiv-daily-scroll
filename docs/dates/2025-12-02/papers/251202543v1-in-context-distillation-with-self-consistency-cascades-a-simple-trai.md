...
**一句话要点**：提出上下文蒸馏与自一致性级联方法，以降低LLM智能体推理成本，无需训练或手动提示工程。

**关键词**：LLM智能体, 上下文蒸馏, 自一致性级联, 推理成本优化, 无训练方法, 智能体基准

## 3 点简述
- 核心问题：LLM智能体大规模推理成本高，传统方法如微调或提示工程开发摩擦大。
- 方法要点：引入上下文蒸馏，通过检索教师演示作为上下文示例，使低成本学生模型动态模仿教师行为；结合自一致性级联自适应信任学生。
- 实验或效果：在ALFWorld基准上以2.5倍低成本匹配教师准确率，AppWorld上实现2倍成本降低，保持准确率。

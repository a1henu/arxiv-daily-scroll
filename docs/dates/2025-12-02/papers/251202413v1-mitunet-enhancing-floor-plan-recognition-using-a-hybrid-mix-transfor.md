...
**一句话要点**：提出MitUNet混合架构以解决室内平面图墙分割中薄结构检测和边界精度不足的问题

**关键词**：墙分割, 混合神经网络, 注意力机制, Tversky损失, 3D重建, 平面图识别

## 3 点简述
- 核心问题：现有方法在墙分割中难以检测薄结构且边界不规则，影响3D重建精度
- 方法要点：结合Mix-Transformer编码器捕获全局上下文，U-Net解码器加scSE注意力块恢复精确边界，使用Tversky损失优化平衡精度与召回
- 实验或效果：在CubiCasa5k和专有数据集上验证，生成结构正确且边界高精度的掩码，优于标准单任务模型

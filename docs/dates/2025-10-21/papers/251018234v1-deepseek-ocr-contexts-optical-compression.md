...
**一句话要点**：提出DeepSeek-OCR以压缩长上下文光学映射，实现高效OCR解码。

**关键词**：光学字符识别, 上下文压缩, 视觉令牌优化, 长文档处理, 模型解码

## 3 点简述
- 核心问题：长上下文光学压缩的可行性，减少视觉令牌数以优化处理。
- 方法要点：使用DeepEncoder和DeepSeek3B-MoE-A570M解码器，实现高压缩比和低激活。
- 实验效果：压缩比<10x时OCR精度达97%，20x时约60%，在基准测试中超越现有模型。

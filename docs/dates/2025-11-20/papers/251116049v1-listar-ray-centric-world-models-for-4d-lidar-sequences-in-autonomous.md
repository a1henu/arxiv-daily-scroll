...
**关键词**：4D LiDAR合成, 生成世界模型, 射线中心变换器, 自动驾驶模拟, 点云生成

## 3 点简述
- 核心问题：4D LiDAR数据合成因球形几何、点云稀疏性和动态场景复杂性而困难。
- 方法要点：采用混合柱面-球面表示和射线中心变换器，提升数据保真度和时序一致性。
- 实验效果：在重建、预测和生成任务中性能领先，如MMD降低76%、IoU提升32%。

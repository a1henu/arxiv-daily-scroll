...
**一句话要点**：提出结合体操领域知识的多相机跟踪方法，以解决体操运动员3D轨迹确定难题。

**关键词**：多相机跟踪, 体操运动员跟踪, 3D轨迹估计, 领域知识集成, 数据关联, 射线平面相交

## 3 点简述
- 核心问题：多相机跟踪中，相机数量有限、光照变化和遮挡导致检测失败，影响3D轨迹准确性。
- 方法要点：引入体操领域知识，使用射线-平面相交生成共面3D轨迹候选，补偿检测不足。
- 实验或效果：在体操世界锦标赛中成功应用，验证方法鲁棒性，优于现有方法。

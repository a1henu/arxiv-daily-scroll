...
**一句话要点**：提出结合人类知识的深度学习策略，以优化未知需求与提前期的易腐品库存系统

**关键词**：易腐品库存管理, 深度学习策略, 端到端学习, 结构引导方法, 边际成本核算, 库存理论集成

## 3 点简述
- 研究易腐品库存系统，需求与提前期分布未知，基于有限历史数据、协变量和系统状态进行决策
- 采用边际成本核算方案，统一损失函数，开发端到端深度学习策略，包括黑盒和结构引导两种变体
- 实验表明结构引导策略优于黑盒，结合提升技术进一步改进，嵌入启发式结构降低模型复杂度并提高学习效率

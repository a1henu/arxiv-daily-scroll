...
**关键词**：掩码扩散语言模型, 并行生成, 生成顺序, 模型评估, 生成-编辑范式, 任务适应性

## 3 点简述
- 核心问题：掩码扩散语言模型在并行生成和任意顺序解码方面的实际能力尚不明确，需量化评估其与自回归模型的差距
- 方法要点：使用平均最终化并行度和肯德尔相关系数，从并行强度和生成顺序两个维度分析模型行为
- 实验或效果：在58个基准测试中，模型表现落后于自回归模型，但任务适应性显示优势，如数独任务中优先填充易解空白

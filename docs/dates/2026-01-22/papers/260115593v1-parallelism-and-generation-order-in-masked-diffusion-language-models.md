...
**关键词**：掩码扩散语言模型, 并行生成, 生成顺序, 自适应解码, 生成-编辑范式, 模型评估

## 3 点简述
- 核心问题：掩码扩散语言模型在并行生成和任意顺序解码方面的实际能力尚不明确，性能落后于自回归模型
- 方法要点：使用平均最终化并行度和肯德尔τ系数量化模型的并行强度和生成顺序，评估八个主流模型
- 实验或效果：模型在任务领域、推理阶段和输出正确性上表现出自适应解码行为，在需要后向信息的任务中优势明显

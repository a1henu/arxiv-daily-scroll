...
**关键词**：地球观测基础模型, 多传感器融合, 计算自适应, 随机化补丁策略, Sentinel卫星数据, 下游任务性能

## 3 点简述
- 当前地球观测基础模型架构僵化，难以处理多传感器数据且受限于固定补丁大小
- THOR统一处理Sentinel-1、-2、-3卫星数据，通过随机化补丁和输入图像大小策略实现计算自适应
- 在THOR Pretrain数据集上预训练，下游基准测试中表现优异，尤其在数据有限场景下验证其灵活性

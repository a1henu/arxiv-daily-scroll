...
**关键词**：地球观测基础模型, 多传感器融合, 计算自适应, 随机化补丁策略, 气候与社会应用, 异构数据处理

## 3 点简述
- 当前地球观测基础模型架构僵化，难以处理异构传感器和固定补丁尺寸，限制实际部署灵活性
- THOR统一处理Sentinel-1、-2、-3卫星数据，通过随机化补丁和输入尺寸策略实现单一权重适应任意补丁尺寸
- 在THOR Pretrain数据集预训练，下游基准测试表现优异，尤其在数据有限场景如PANGAEA 10%分割验证其应用潜力

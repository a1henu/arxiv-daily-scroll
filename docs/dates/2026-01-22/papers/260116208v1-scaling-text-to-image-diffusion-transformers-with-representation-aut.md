...
**一句话要点**：提出表示自编码器框架，通过简化设计与数据组合，提升大规模文本到图像生成的性能与稳定性。

**关键词**：文本到图像生成, 表示自编码器, 扩散变换器, 大规模预训练, 噪声调度, 多模态表示

## 3 点简述
- 研究表示自编码器框架在大规模自由文本到图像生成中的可扩展性，发现数据组合对特定领域如文本生成至关重要。
- 分析并简化表示自编码器设计，去除冗余架构复杂性，保留关键噪声调度，实现更高效的扩散建模。
- 通过对比实验，表示自编码器在预训练和微调中均优于变分自编码器，展现更快收敛、更好生成质量和抗过拟合能力。

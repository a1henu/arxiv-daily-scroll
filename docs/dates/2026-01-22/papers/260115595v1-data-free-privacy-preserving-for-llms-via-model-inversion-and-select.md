...
**一句话要点**：提出数据自由选择性遗忘框架，以解决大语言模型中敏感个人信息的隐私保护问题。

**关键词**：大语言模型隐私保护, 数据自由遗忘, 模型反演, 选择性遗忘, 低秩适应, 令牌级掩码

## 3 点简述
- 核心问题：大语言模型可能记忆训练数据中的敏感个人信息，但传统遗忘技术依赖训练数据访问，这在现实中常不可行。
- 方法要点：通过语言模型反演合成伪敏感信息，构建令牌级隐私掩码，并在低秩适应子空间中使用对比掩码损失进行选择性遗忘。
- 实验或效果：在AI4Privacy PII-Masking数据集上使用Pythia模型实验，表明方法能有效移除目标敏感信息并保持模型效用。

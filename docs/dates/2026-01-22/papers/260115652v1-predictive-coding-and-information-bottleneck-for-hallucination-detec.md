...
**一句话要点**：提出基于预测编码与信息瓶颈的混合检测框架，以高效检测大语言模型幻觉

**关键词**：幻觉检测, 预测编码, 信息瓶颈, 可解释性, 大语言模型, 轻量模型

## 3 点简述
- 核心问题：大语言模型幻觉（看似合理但事实不忠实）阻碍高风险部署，现有方法依赖昂贵检索或大参数黑盒模型。
- 方法要点：结合预测编码（量化内部先验的意外）和信息瓶颈（测量扰动下信号保留），提取可解释信号如实体聚焦、上下文依从和可证伪性评分。
- 实验或效果：在HaluBench上，改进特征使AUROC达0.8669，数据效率高（仅200样本），推理快（5ms），模型轻量（<1M参数）且可解释。

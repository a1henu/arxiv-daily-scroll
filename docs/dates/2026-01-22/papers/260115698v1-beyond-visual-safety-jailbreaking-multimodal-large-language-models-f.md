...
**关键词**：多模态大语言模型, 视觉安全, 越狱攻击, 图像生成, 语义无关输入, 安全对齐

## 3 点简述
- 核心问题：多模态大语言模型在视觉安全方面存在未充分探索的漏洞，尤其在文本与视觉交叉领域。
- 方法要点：采用“重建-生成”策略，通过视觉拼接中和与归纳重组，解耦恶意意图，诱导模型生成有害图像。
- 实验或效果：在GPT-5（2026年1月12日发布）上实现98.21%的越狱成功率，暴露当前模型视觉安全对齐的关键弱点。

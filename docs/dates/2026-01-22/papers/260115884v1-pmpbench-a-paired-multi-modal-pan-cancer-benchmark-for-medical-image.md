...
**一句话要点**：提出首个公开、全配对、多器官医学影像数据集与基准，以解决对比剂合成中的数据限制问题。

**关键词**：医学影像合成, 对比剂增强, 多模态数据集, 图像翻译基准, 全配对数据, 多器官肿瘤成像

## 3 点简述
- 核心问题：现有医学影像合成数据集存在数据限制，如仅关注脑部、配对不完整、标签缺失和资源私有。
- 方法要点：构建包含11个人体器官的配对多模态数据集，涵盖MR动态对比增强序列和CT对比增强与非对比图像。
- 实验或效果：基于数据集建立基准，评估图像翻译方法在1对1、N对1和N对N设置下的性能，促进安全有效的对比合成研究。

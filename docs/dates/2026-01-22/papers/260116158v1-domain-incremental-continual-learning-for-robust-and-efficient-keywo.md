...
**一句话要点**：提出域增量持续学习框架，以解决资源受限系统中关键词检测的鲁棒性和效率问题。

**关键词**：关键词检测, 持续学习, 域增量学习, 去噪技术, 资源受限系统, 边缘计算

## 3 点简述
- 核心问题：边缘设备上关键词检测模型因噪声和录制条件变化导致域偏移，影响准确性和鲁棒性。
- 方法要点：集成双输入CNN、多阶段去噪和原型更新，支持完整量化模型更新，结合伪标签和排练缓冲进行增量训练。
- 实验或效果：在嘈杂测试集上验证，清洁数据准确率达99.63%，-10 dB信噪比下仍保持超过94%的准确率。

...
**一句话要点**：确定临床文本分类训练语料库规模的影响因素，基于词汇属性分析优化样本量需求。

**关键词**：临床文本分类, 训练语料库规模, 词汇属性分析, BERT嵌入, 随机森林分类器, MIMIC-III数据集

## 3 点简述
- 核心问题：临床文本分类中训练数据规模缺乏理论依据，受限于标注成本与时间。
- 方法要点：使用MIMIC-III数据集，结合BERT嵌入和随机森林，分析不同训练规模下的性能变化。
- 实验或效果：600文档可达到10,000文档95%性能，词汇属性如强预测词和噪声词影响学习曲线。

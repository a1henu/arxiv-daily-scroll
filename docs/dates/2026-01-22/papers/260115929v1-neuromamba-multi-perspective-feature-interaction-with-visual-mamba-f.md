...
**关键词**：神经元分割, 视觉Mamba, 多视角特征交互, 长程依赖建模, 体素细节保留, EM图像分析

## 3 点简述
- 核心问题：神经元形态不规则且结构密集交织，现有方法在长程上下文建模和体素细节保留上存在不足。
- 方法要点：设计通道门控边界判别特征提取器和空间连续特征提取器，通过跨调制机制融合多视角特征。
- 实验或效果：在四个公开EM数据集上实现最先进性能，验证了对各向同性和各向异性分辨率的强适应性。

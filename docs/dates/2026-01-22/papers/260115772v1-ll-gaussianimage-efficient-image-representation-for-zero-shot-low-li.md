...
**关键词**：2D高斯泼溅, 低光增强, 压缩域处理, 零样本学习, 专家混合框架, 图像表示

## 3 点简述
- 现有低光增强算法需在像素域处理，导致2DGS压缩图像需解压-增强-再压缩，效率低且引入二次退化
- 设计语义引导的专家混合增强框架，在2DGS稀疏属性空间应用动态自适应变换，无需完全解压到像素网格
- 通过多目标协同损失函数和两阶段优化，在保持高压缩比的同时实现高质量增强，实验验证了该范式的可行性和优越性

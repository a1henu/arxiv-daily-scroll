...
**关键词**：可解释深度伪造检测, 多模态大语言模型, 强化学习, 自混合图像, 跨域泛化, 思维链数据生成

## 3 点简述
- 核心问题：现有深度伪造检测方法缺乏可解释输出，且高质量标注数据集稀缺，阻碍多模态大语言模型应用。
- 方法要点：设计自动化思维链数据生成框架，结合强化学习优化奖励机制和合成数据生成，增强跨域泛化能力。
- 实验或效果：在多个跨数据集基准测试中达到与最先进方法竞争的性能，验证了框架的有效性。

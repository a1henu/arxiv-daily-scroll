...
**关键词**：胎儿脑MRI分割, Segment Anything Model, 图谱提示, 多图谱配准, 灵活分割

## 3 点简述
- 核心问题：传统深度学习方法需大量标注且标签固定，难以适应临床或研究需求变化。
- 方法要点：利用多图谱配准生成密集提示，结合边界框提示，驱动SAM解码器进行逐结构二值分割。
- 实验或效果：在dHCP和内部数据集上，对高对比结构达到与先进基线相当的Dice分数，保持分割任意解剖结构的灵活性。

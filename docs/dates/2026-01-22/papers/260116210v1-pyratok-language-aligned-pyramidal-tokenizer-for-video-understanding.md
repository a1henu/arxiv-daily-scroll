...
## 3 点简述
- 现有视频分词器通常基于单尺度视觉码本，词汇有限且语言监督浅，导致跨模态对齐差和零样本迁移弱。
- PyraTok通过语言对齐金字塔量化模块，在多个时空分辨率上学习语义结构化的离散潜在表示，并使用共享大二进制码本。
- 在十个基准测试中，PyraTok实现了最先进的视频重建，提升文本到视频质量，并在视频分割、时序动作定位和视频理解上创零样本性能新高。

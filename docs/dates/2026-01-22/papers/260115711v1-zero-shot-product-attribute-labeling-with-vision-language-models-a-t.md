...
**一句话要点**：提出三层次评估框架以解决时尚属性零标注中条件属性的系统评测问题

**关键词**：零样本属性标注, 视觉语言模型, 时尚属性预测, 评估框架, 条件属性, 多模态基准

## 3 点简述
- 核心问题：时尚属性常为条件性，需先检测适用性再分类，现有评测不足。
- 方法要点：引入三层次框架，分解为整体性能、适用性检测和细粒度分类。
- 实验或效果：在DeepFashion-MultiModal上评测9个VLMs，发现细粒度分类优但适用性检测差，高效模型性能接近旗舰。

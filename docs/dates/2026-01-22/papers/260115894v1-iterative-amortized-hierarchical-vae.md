...
**一句话要点**：提出迭代摊销分层变分自编码器，结合摊销推断与迭代优化以加速高深度模型推理。

**关键词**：变分自编码器, 摊销推断, 迭代优化, 逆问题, 实时推理

## 3 点简述
- 核心问题：传统分层变分自编码器在深度模型推理中速度慢，影响实时应用。
- 方法要点：采用混合推断方案，先摊销初始猜测，再基于解码器梯度迭代优化。
- 实验或效果：实现35倍加速，在去模糊和去噪等逆问题中重建质量优于基线。

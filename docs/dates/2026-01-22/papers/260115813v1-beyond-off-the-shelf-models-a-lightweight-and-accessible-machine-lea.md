...
**一句话要点**：提出轻量级机器学习管道，帮助生态学家基于图像数据定制分类模型

**关键词**：生态图像分类, 轻量级机器学习管道, 定制化模型开发, 相机陷阱数据分析, 野生动物监测

## 3 点简述
- 核心问题：生态学家应用机器学习分类图像时，依赖现成模型，难以针对本地数据集和特定任务定制。
- 方法要点：结合命令行界面进行预处理、训练和评估，以及图形界面用于标注、错误分析和模型比较。
- 实验或效果：在德国Veldenstein森林的3392张相机陷阱图像上，对红鹿进行年龄和性别分类，最佳模型准确率分别达90.77%和96.15%。

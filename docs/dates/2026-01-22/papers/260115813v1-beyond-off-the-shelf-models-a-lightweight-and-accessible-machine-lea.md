...
**一句话要点**：提出轻量级机器学习管道，帮助生态学家基于图像数据独立开发定制化分类模型

**关键词**：生态图像分类, 轻量级机器学习管道, 相机陷阱数据分析, 模型定制化, 野生动物监测

## 3 点简述
- 针对生态学家应用机器学习处理图像数据时依赖现成模型、缺乏定制化工具的问题
- 设计结合命令行与图形界面的管道，支持预处理、训练、评估及标注、错误分析、模型比较
- 以德国Veldenstein森林的3392张相机陷阱图像为例，年龄和性别分类准确率分别达90.77%和96.15%

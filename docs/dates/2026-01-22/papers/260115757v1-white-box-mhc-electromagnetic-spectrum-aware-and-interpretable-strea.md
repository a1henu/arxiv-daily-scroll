...
**一句话要点**：提出电磁频谱感知白盒mHC框架，以增强高光谱图像分类的可解释性。

**关键词**：高光谱图像分类, 可解释性学习, 白盒模型, 电磁频谱感知, 超连接框架

## 3 点简述
- 问题：现有深度学习模型在高光谱图像分类中依赖不透明的谱-空特征混合，缺乏可解释性。
- 方法：ES-mHC通过结构化方向矩阵显式建模电磁频谱分组间的交互，分离特征表示与交互结构。
- 效果：学习到的超连接矩阵显示空间模式和不对称交互，提供模型内部动态的机制性洞察。

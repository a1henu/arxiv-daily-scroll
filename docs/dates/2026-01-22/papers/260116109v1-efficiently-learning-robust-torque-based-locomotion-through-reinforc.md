...
**关键词**：双足机器人控制, 残差强化学习, 模型监督, 鲁棒行走, 仿真到现实迁移

## 3 点简述
- 核心问题：双足机器人在真实世界不确定性（如模型不准确和传感器噪声）下难以实现鲁棒自适应行走。
- 方法要点：利用模型控制器作为基础策略，通过域随机化训练残差策略，并用基于模型的Oracle策略监督学习。
- 实验或效果：在随机化条件下展示改进的鲁棒性和泛化能力，为仿真到现实迁移提供可扩展方案。

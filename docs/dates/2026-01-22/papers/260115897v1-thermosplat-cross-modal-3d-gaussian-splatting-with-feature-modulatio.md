...
**关键词**：跨模态3D重建, 高斯泼溅, 特征调制, 几何解耦, 热红外数据, 混合渲染

## 3 点简述
- 核心问题：现有3DGS方法在多光谱场景中难以有效利用跨模态互补信息，常忽略模态间相关性或共享表示适应性不足
- 方法要点：引入跨模态FiLM调制机制动态调节共享特征，并采用模态自适应几何解耦方案学习独立不透明度偏移
- 实验或效果：在RGBT-Scenes数据集上验证，ThermoSplat在可见光和热红外光谱均达到先进渲染质量

...
**一句话要点**：提出FARM模型以解决触发-动作自动化中函数级配置问题，生成可执行应用。

**关键词**：触发-动作编程, 自动化配置, 对比学习, 多智能体系统, 函数级预测

## 3 点简述
- 核心问题：现有方法在服务级预测触发-动作自动化，常生成需手动配置的非可执行应用。
- 方法要点：采用两阶段架构，包括基于对比学习的候选检索和基于LLM的多智能体配置。
- 实验或效果：在函数级实现81%联合准确率，优于基线TARGE 23个百分点，生成可执行配置。

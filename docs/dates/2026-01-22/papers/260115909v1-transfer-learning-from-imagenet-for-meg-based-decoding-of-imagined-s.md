...
**一句话要点**：提出基于ImageNet预训练视觉模型的图像化MEG表示方法，以解码想象语音

**关键词**：想象语音解码, 脑磁图信号处理, 预训练视觉模型, 时频表示, 跨被试评估

## 3 点简述
- 核心问题：非侵入式想象语音解码因信号弱、分布广和标记数据有限而具挑战性
- 方法要点：将MEG信号转换为时频表示，通过可学习卷积生成图像化输入，适配预训练视觉模型
- 实验或效果：在21名参与者数据上，模型在想象与静默、静默阅读及元音解码任务中表现优异，最高平衡准确率达90.4%

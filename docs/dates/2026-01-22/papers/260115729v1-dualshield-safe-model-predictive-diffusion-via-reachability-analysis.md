...
**一句话要点**：提出DualShield框架，通过可达性分析确保交互式自动驾驶中扩散模型的安全预测控制。

**关键词**：自动驾驶规划, 扩散模型, 可达性分析, 安全控制, 交互不确定性

## 3 点简述
- 核心问题：扩散模型在自动驾驶运动规划中难以保证车辆动力学约束和依赖准确交互预测，导致安全风险。
- 方法要点：利用Hamilton-Jacobi可达性值函数，既引导扩散去噪过程，又作为反应式安全屏障修改执行动作。
- 实验或效果：在无保护U-turn场景模拟中，相比现有方法，显著提升安全性和任务效率。

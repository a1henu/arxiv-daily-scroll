...
**一句话要点**：提出地理空间基础模型框架以提升滑坡灾害映射的泛化性与数据稀缺适应性

**关键词**：滑坡灾害映射, 地理空间基础模型, 数据稀缺适应, 跨域泛化, 遥感图像分析, 自监督学习

## 3 点简述
- 核心问题：传统深度学习模型在跨传感器、区域及数据稀缺时性能下降。
- 方法要点：基于Prithvi-EO-2.0构建三轴分析框架，结合全局预训练与自适应微调。
- 实验或效果：在多个数据集上优于任务特定模型，展现光谱变化鲁棒性和跨域泛化能力。

...
**一句话要点**：提出线性分式变换模型以解耦光场相机主镜头与微透镜阵列，实现精确标定。

**关键词**：光场相机标定, 线性分式变换, 3D重建, 微透镜阵列, 非线性优化

## 3 点简述
- 核心问题：光场相机内部参数标定对3D重建至关重要，但具有挑战性。
- 方法要点：引入线性分式变换参数α，采用最小二乘解析解和非线性优化。
- 实验或效果：在物理和模拟数据上验证性能，并加速原始光场图像模拟。

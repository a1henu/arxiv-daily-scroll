...
**一句话要点**：提出ParaGate框架，通过寄生参数驱动的域适应迁移学习，从网表预测布局级性能以指导早期优化。

**关键词**：电子设计自动化, 迁移学习, 性能预测, 寄生参数, 时序分析, 全局优化

## 3 点简述
- 传统EDA流程中，布局级性能指标仅在布局布线后获得，阻碍早期全局优化。
- ParaGate采用三步框架：预测寄生参数、依赖EDA工具进行时序分析、基于子图特征全局校准。
- 实验显示，ParaGate在少量微调数据下实现强泛化，如到达时间R2从0.119提升至0.897。

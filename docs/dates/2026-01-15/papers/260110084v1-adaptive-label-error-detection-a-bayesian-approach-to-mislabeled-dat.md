...
**关键词**：标签错误检测, 医学图像分析, 高斯分布建模, 似然比测试, 自适应方法

## 3 点简述
- 核心问题：机器学习分类系统易受错误标注影响，即使专家标注也可能存在误标。
- 方法要点：从深度卷积神经网络提取特征，去噪后建模为高斯分布，通过似然比检测误标样本。
- 实验或效果：在医学图像数据集上，相比现有方法，ALED显著提高检测灵敏度且不损失精度，校正后微调模型可减少33.8%测试错误。

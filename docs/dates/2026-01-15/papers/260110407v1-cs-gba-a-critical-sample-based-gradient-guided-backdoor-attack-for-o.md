...
**关键词**：离线强化学习, 后门攻击, 安全约束算法, 关键样本选择, 梯度引导, OOD检测

## 3 点简述
- 核心问题：离线强化学习易受后门攻击，现有方法对安全约束算法（如CQL）效果差，因随机投毒效率低且触发器易被OOD检测。
- 方法要点：采用自适应关键样本选择，聚焦高TD误差样本；设计相关性破坏触发器，利用状态特征互斥性保持隐蔽；使用梯度引导动作生成，在数据流形内搜索最差动作。

...
**一句话要点**：基于OpenRouter平台分析100万亿令牌，揭示LLM实际使用模式与用户保留效应

**关键词**：大语言模型实证研究, 用户行为分析, 推理平台数据, 开放权重模型, 创意角色扮演, 用户保留效应

## 3 点简述
- 核心问题：LLM实际使用模式与理论认知脱节，缺乏大规模实证数据支撑
- 方法要点：利用OpenRouter平台分析100万亿令牌，覆盖任务、地理和时间维度
- 实验或效果：发现开放权重模型普及、创意角色扮演流行、用户保留存在“玻璃鞋”效应

...
**关键词**：水下成像, 事件相机, 光流估计, 合成数据集, 基准测试, 射线追踪

## 3 点简述
- 水下成像因光衰减、散射和模糊等挑战，难以获取真实运动数据，事件相机虽具优势但缺乏相关数据集。
- 通过基于物理的射线追踪RGBD序列生成合成水下视频，并应用视频到事件管道，产生带密集真值的事件数据流。
- 基准测试了先进的光流预测方法，评估水下光传输对事件形成和运动估计的影响，为算法开发提供新基线。

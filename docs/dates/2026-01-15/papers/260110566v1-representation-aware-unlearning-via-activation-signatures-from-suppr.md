...
**关键词**：知识选择性擦除, 表示感知遗忘, 激活签名, 大语言模型, 参数高效适配, 遗忘评估协议

## 3 点简述
- 核心问题：现有遗忘方法混淆行为抑制与真实知识移除，导致潜在能力在表面拒绝下持续存在。
- 方法要点：引入知识免疫框架，通过动态抑制主题特定表示和参数高效适配，针对内部激活签名而非表面输出。
- 实验或效果：在3B至14B参数模型上实现接近理想的擦除效果，同时保持实用性，突破稳定性与擦除的权衡。

...
**一句话要点**：提出AI驱动分层控制框架，实现非结构化地形上的移动3D打印精度与机动性平衡。

**关键词**：移动3D打印, 非结构化地形, AI扰动预测, 分层控制, 传感器融合, 自主建造

## 3 点简述
- 核心问题：移动3D打印在非结构化地形上，平台机动性与沉积精度存在冲突，现有系统难以兼顾。
- 方法要点：集成AI扰动预测与多模态传感器融合，通过三层控制架构实现闭环感知-学习-执行。
- 实验或效果：户外实验在斜坡和不规则地面展示亚厘米级打印精度，同时保持平台全机动性。

...
**关键词**：半监督医学图像分割, 向量量化, 特征扰动, 一致性学习, 肺癌CT分割, 基础模型指导

## 3 点简述
- 核心问题：现有半监督医学图像分割方法依赖dropout进行特征扰动，其超参数敏感且难以优化，可能导致正则化效果不佳。
- 方法要点：设计向量量化扰动模块，通过离散化特征空间并扰动码书索引空间位置，实现可控正则化；采用双分支架构和Post-VQ特征适配器，减少量化信息损失并引入基础模型指导。
- 实验或效果：在自建大规模肺癌CT数据集和公共基准上验证，性能优于现有先进方法。

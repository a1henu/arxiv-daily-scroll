...
**关键词**：半监督医学图像分割, 向量量化, 特征扰动, 一致性学习, 肺癌分割

## 3 点简述
- 核心问题：现有基于dropout的特征扰动方法需手动调参，超参数敏感且易导致次优正则化。
- 方法要点：设计量化扰动模块，通过离散特征空间和索引位置混洗实现可控正则化；采用双分支架构和特征适配器缓解量化信息损失。
- 实验或效果：在自建肺癌数据集和公共基准上实验，性能优于现有方法。

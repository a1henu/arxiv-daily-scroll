...
**一句话要点**：提出OPIR框架，通过物理退化建模预测任务感知逆算子，实现高效多任务图像恢复。

**关键词**：图像恢复, 多任务学习, 物理退化模型, 逆算子预测, 不确定性感知

## 3 点简述
- 核心问题：现有多任务图像恢复方法因引入额外学习模块而增加复杂性，影响实时应用。
- 方法要点：采用两阶段框架，预测逆算子生成初始恢复图像和不确定性感知图，指导细化恢复。
- 实验或效果：OPIR在实验中展示出优越的多任务恢复性能，同时保持高效性。

...
**关键词**：涡扇发动机健康监测, 无监督学习, LSTM自编码器, 早期故障检测, 自适应阈值, CMAPSS数据集

## 3 点简述
- 核心问题：涡扇发动机健康监测需无失效标签的早期故障检测，以快速部署和扩展。
- 方法要点：通过回归归一化去除工况影响，用健康数据训练LSTM自编码器，自适应阈值触发警报。
- 实验或效果：在CMAPSS数据集上实现高召回率和低误报率，适用于多种工况，作为剩余寿命模型的补充预警层。

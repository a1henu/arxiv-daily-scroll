...
**一句话要点**：提出增强策略以提升分层推理模型的猜测能力，显著提高数独极端任务准确率

**关键词**：分层推理模型, 机制分析, 固定点性质, 数据增强, 输入扰动, 模型自举

## 3 点简述
- 核心问题：分层推理模型在简单谜题中因违反固定点假设而失败，表现为猜测而非推理
- 方法要点：通过数据增强、输入扰动和模型自举三种策略扩展模型的猜测能力
- 实验效果：结合所有方法开发增强HRM，在数独极端任务上准确率从54.5%提升至96.9%

...
**一句话要点**：通过随机图与简化语言模型探究神经缩放定律起源，揭示数据非幂律结构下仍存在缩放规律

**关键词**：神经缩放定律, 随机图训练, Transformer模型, 语言模型简化, 参数效率分析, 计算最优曲线

## 3 点简述
- 核心问题：神经缩放定律是否源于数据中的幂律结构，研究其在简化场景下的起源。
- 方法要点：使用可调复杂度的随机图训练Transformer预测随机游走，并系统简化自然语言生成模型进行训练。
- 实验或效果：在无数据幂律结构下观察到缩放定律，缩放指数随语言模型简化单调演化，并对比了不同参数化方法的效率。

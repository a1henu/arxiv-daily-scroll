...
**关键词**：文本到图像生成, 扩散模型, 大语言模型, 推理增强, 语义对齐, 联合优化

## 3 点简述
- 现有T2I扩散模型仅将LLM用作文本编码器，未利用其推理能力，导致生成图像可能缺乏语义深度。
- 采用轻量级监督微调激活LLM的推理重写模式，并通过Dual-GRPO联合优化LLM编码器与扩散主干，确保推理准确与渲染一致。
- 实验在基于推理的图像生成与编辑基准上显著提升事实一致性、语义对齐和视觉真实感，WISE分数达0.79，接近GPT-4水平。

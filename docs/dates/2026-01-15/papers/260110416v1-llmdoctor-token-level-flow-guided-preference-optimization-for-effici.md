...
**一句话要点**：提出LLMdoctor框架，通过细粒度令牌级流引导偏好优化实现高效测试时对齐大语言模型

**关键词**：大语言模型对齐, 测试时对齐, 令牌级优化, 流引导偏好优化, 患者-医生范式

## 3 点简述
- 传统对齐方法计算成本高且不灵活，测试时对齐方法常依赖扭曲的轨迹级信号或低效采样
- LLMdoctor采用患者-医生范式，结合令牌级奖励获取与流引导偏好优化，用小型医生模型引导冻结患者模型
- 实验显示LLMdoctor优于现有测试时对齐方法，甚至超越DPO等全微调方法

...
**关键词**：载波频率偏移校准, Sim2Real迁移学习, 软件定义无线电, 正交频分复用, 设备级适配, 仿真到现实

## 3 点简述
- 核心问题：异构软件定义无线电平台因硬件损伤未校准，导致OFDM系统载波频率偏移估计性能显著下降。
- 方法要点：结合参数化硬件失真仿真预训练骨干DNN，再针对目标设备用少量真实帧微调回归层，实现设备级适配。
- 实验或效果：在三种SDR设备上实验，相比传统方法，室内多径条件下误码率降低30倍。

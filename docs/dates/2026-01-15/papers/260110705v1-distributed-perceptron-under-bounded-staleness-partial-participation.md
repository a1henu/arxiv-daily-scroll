...
**一句话要点**：提出基于时滞桶聚合的分布式感知机，在时滞、部分参与和噪声通信下保证有限错误界

**关键词**：分布式感知机, 时滞控制, 部分参与, 通信噪声, 参数混合, 错误界分析

## 3 点简述
- 研究分布式感知机训练，处理时滞、部分参与和通信噪声的系统效应
- 引入时滞桶聚合规则，确定性控制更新时滞，无需假设延迟或参与的随机模型
- 在可分性和数据半径有界下，证明有限轮次内累积感知机错误的期望界

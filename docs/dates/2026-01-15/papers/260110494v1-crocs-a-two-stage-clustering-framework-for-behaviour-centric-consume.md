...
**一句话要点**：提出CROCS两阶段聚类框架，基于智能电表数据实现行为驱动的消费者细分，以应对需求侧管理挑战。

**关键词**：智能电表数据, 消费者细分, 两阶段聚类, 需求侧管理, 行为分析, 鲁棒聚类

## 3 点简述
- 核心问题：现有聚类方法难以处理消费者行为多样性、数据异常和大规模部署，影响需求响应效果。
- 方法要点：第一阶段独立聚类日负荷曲线生成代表性负荷集，第二阶段使用加权最小距离和进行消费者聚类。
- 实验或效果：在合成和真实数据集上验证，CROCS能捕捉行为变异性、发现同步异步相似性，并保持鲁棒性和可扩展性。

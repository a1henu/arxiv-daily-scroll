...
**一句话要点**：提出基于指令微调与LoRA的LLaMA-3-8B模型，用于提升金融命名实体识别性能。

**关键词**：金融命名实体识别, 指令微调, LoRA, 参数高效微调, LLaMA-3-8B, 领域适应

## 3 点简述
- 核心问题：免费大语言模型在金融NER中常混淆实体类型或忽略关键信息。
- 方法要点：将标注句子转为指令-输入-输出三元组，结合LoRA进行参数高效微调。
- 实验或效果：在1,693句语料上获得0.894的micro-F1分数，优于多个基线模型。

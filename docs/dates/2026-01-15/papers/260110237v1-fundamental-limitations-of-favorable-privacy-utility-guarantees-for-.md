...
**关键词**：差分隐私, DP-SGD, 隐私-效用权衡, f-差分隐私, 高斯噪声, 对抗模型

## 3 点简述
- 分析DP-SGD在f-差分隐私框架下的隐私-效用权衡曲线，推导出可达到曲线的显式次优上界。
- 证明在标准最坏情况对抗模型下，强制小分离度会严格限制高斯噪声乘子，导致无法同时实现强隐私和高效用。
- 实验验证该限制在实际训练设置中导致显著准确度下降，扩展至泊松子采样也成立。

...
**一句话要点**：提出队列感知推理令牌优化方法，以权衡LLM服务器中准确性与延迟的权衡。

**关键词**：LLM服务器优化, 令牌分配策略, 队列稳定性, 准确性与延迟权衡, M/G/1队列模型, 约束优化

## 3 点简述
- 核心问题：异构查询流中，令牌分配如何平衡准确性与队列延迟。
- 方法要点：基于M/G/1队列模型，构建严格凹优化问题，确保最优令牌分配。
- 实验或效果：通过迭代求解和舍入处理，评估性能损失，验证收敛性。

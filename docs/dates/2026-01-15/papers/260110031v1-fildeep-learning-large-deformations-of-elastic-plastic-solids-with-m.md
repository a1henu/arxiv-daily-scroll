...
**关键词**：弹塑性固体大变形, 多保真度数据学习, 深度学习框架, 注意力机制, 制造应用, 拉伸弯曲问题

## 3 点简述
- 核心问题：弹塑性固体大变形计算中，传统数值方法受限，深度学习依赖高量高精度数据，但数据获取困难，存在数据量与精度权衡的困境。
- 方法要点：针对拉伸弯曲问题，设计FilDeep框架，通过同时训练低保真度（高量低精度）和高保真度（低量高精度）数据，并引入注意力跨保真度模块捕获长程物理交互。
- 实验或效果：实验表明FilDeep在性能上达到先进水平，可高效应用于制造领域，是首个利用多保真度数据的大变形深度学习框架。

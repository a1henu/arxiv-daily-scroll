...
**一句话要点**：提出EvoMorph框架，为连续时间序列外生回归生成生理合理的反事实解释，应用于光电容积描记信号分析。

**关键词**：反事实解释, 时间序列外生回归, 光电容积描记信号, 进化算法, 不确定性量化, 生物医学信号处理

## 3 点简述
- 核心问题：现有时间序列反事实解释方法多限于分类任务，忽略波形形态，常产生生理不合理信号，限制其在连续生物医学时间序列中的应用。
- 方法要点：EvoMorph采用多目标进化框架，优化基于可解释信号描述符的形态感知目标，应用变换以保持波形结构，生成多样且生理合理的反事实。
- 实验或效果：在三个PPG数据集上评估，对比基准方法，并作为不确定性量化工具，将反事实敏感性与引导集成不确定性和数据密度度量关联。

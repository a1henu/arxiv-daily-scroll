...
**一句话要点**：提出TRIM方法，通过目标步级路由提升多步推理任务的效率与准确性

**关键词**：多步推理, 模型路由, 步级干预, 成本效率, 过程奖励模型, 数学问题求解

## 3 点简述
- 核心问题：多步推理任务易因单步错误导致级联失败，现有路由方法将整个查询分配给单一模型，效率低下。
- 方法要点：TRIM在步级操作，识别关键步骤并路由至大模型，小模型处理常规步骤，基于不确定性、预算和过程奖励模型进行决策。
- 实验或效果：在MATH-500上，简单策略成本效率提升5倍，高级策略用80%更少昂贵令牌匹配强模型性能；在AIME上成本效率提升达6倍。

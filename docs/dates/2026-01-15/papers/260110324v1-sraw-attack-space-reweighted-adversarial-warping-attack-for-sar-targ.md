...
**关键词**：合成孔径雷达目标识别, 对抗攻击, 空间变形, 隐蔽性, 对抗迁移性

## 3 点简述
- 核心问题：SAR图像信息稀疏，现有攻击需明显失真，且模型易依赖背景区域，影响对抗鲁棒性。
- 方法要点：通过优化空间变形，在目标与背景区域分配重加权预算，生成隐蔽的对抗样本。
- 实验或效果：实验显示SRAW显著降低SAR-ATR模型性能，在隐蔽性和对抗迁移性上优于现有方法。

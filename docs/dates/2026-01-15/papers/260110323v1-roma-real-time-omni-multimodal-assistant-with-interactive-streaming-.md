...
**关键词**：流式音视频理解, 全模态大语言模型, 主动监控, 同步多模态单元, 轻量级说话头, 实时交互

## 3 点简述
- 核心问题：现有全模态大语言模型在流式音视频理解中模态支持不完整且缺乏主动监控能力。
- 方法要点：采用同步多模态单元对齐密集音频与离散视频帧，并引入轻量级说话头实现响应触发与生成的解耦。
- 实验或效果：在12个基准测试中，ROMA在主动任务上达到最优性能，在反应性任务上表现竞争性，验证了其统一实时全模态理解的鲁棒性。

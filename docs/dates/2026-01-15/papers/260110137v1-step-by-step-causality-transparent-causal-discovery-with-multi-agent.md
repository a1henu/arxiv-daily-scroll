...
**一句话要点**：提出Tree-Query框架，通过多专家LLM树状查询解决数据无关因果发现中的透明性与鲁棒性问题。

**关键词**：因果发现, 大语言模型, 树状查询, 置信估计, 数据无关先验, 透明推理

## 3 点简述
- 核心问题：传统因果发现方法存在误差传播，LLM因果预言机缺乏透明度和置信度。
- 方法要点：采用树结构多专家LLM框架，将成对因果发现分解为后门路径、独立性等序列查询，提供可解释判断和鲁棒置信分数。
- 实验或效果：在数据无关基准测试中提升结构指标，案例研究展示混淆因子筛选和高置信因果结论。

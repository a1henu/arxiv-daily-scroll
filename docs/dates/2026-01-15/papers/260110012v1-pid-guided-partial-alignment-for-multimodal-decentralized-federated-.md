...
**关键词**：多模态联邦学习, 去中心化学习, 部分信息分解, 梯度对齐, P2P网络, 异构代理

## 3 点简述
- 核心问题：多模态去中心化联邦学习中，单模态与多模态代理间的梯度不匹配抑制了异构共享和跨模态交互。
- 方法要点：利用部分信息分解将潜在表示分解为冗余、独特和协同切片，实现切片级部分对齐以促进P2P知识共享。
- 实验或效果：在基准测试和代理混合中，PARSE优于任务、模态和混合共享基线，并通过消融实验验证了设计的效率和鲁棒性。

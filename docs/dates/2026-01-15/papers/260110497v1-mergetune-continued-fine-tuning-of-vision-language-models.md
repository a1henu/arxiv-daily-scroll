...
**一句话要点**：提出MERGETUNE方法，通过持续微调恢复视觉语言模型在适应后丢失的预训练知识。

**关键词**：视觉语言模型, 持续微调, 灾难性遗忘, 线性模式连通性, 模型泛化

## 3 点简述
- 核心问题：视觉语言模型微调常导致灾难性遗忘，现有方法难以完全避免。
- 方法要点：基于线性模式连通性，设计模型无关的持续微调策略，无需架构改动即可恢复知识。
- 实验或效果：在基础-新类泛化上提升CoOp的调和平均5.6%，并在跨数据集迁移中超越CLIP。

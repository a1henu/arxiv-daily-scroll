...
**一句话要点**：将分类不平衡问题框架化为标签偏移下的迁移学习，分析过采样方法的转移成本。

**关键词**：分类不平衡, 迁移学习, 过采样, SMOTE算法, 标签偏移

## 3 点简述
- 核心问题：分类不平衡时，少数类样本稀少，影响模型性能评估。
- 方法要点：将不平衡数据视为源分布，平衡分布为目标，通过过采样（如SMOTE）估计少数类分布以减少转移成本。
- 实验或效果：理论分析表明，在高维中，自助法转移成本低于SMOTE，实验支持此发现，为策略选择提供指导。

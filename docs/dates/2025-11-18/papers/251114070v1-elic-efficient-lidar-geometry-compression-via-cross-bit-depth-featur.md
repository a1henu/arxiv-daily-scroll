...
**一句话要点**：提出ELiC框架以高效压缩LiDAR几何数据，实现实时处理

**关键词**：LiDAR几何压缩, 跨位深特征传播, Bag-of-Encoders, Morton层次结构, 实时处理

## 3 点简述
- 核心问题：现有分层压缩方法独立处理不同位深，重复估计局部上下文，效率低
- 方法要点：跨位深特征传播重用特征，Bag-of-Encoders自适应选择编码网络
- 实验效果：在Ford和SemanticKITTI数据集上实现SOTA压缩，保持实时吞吐

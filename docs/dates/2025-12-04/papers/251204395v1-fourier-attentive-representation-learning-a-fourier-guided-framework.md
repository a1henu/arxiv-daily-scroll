...
## 3 点简述
- 核心问题：大规模预训练视觉语言模型在表示中隐式纠缠了图像的领域不变结构和领域特定风格，限制了小样本泛化。
- 方法要点：利用傅里叶分析，通过双交叉注意力机制分别从相位谱和幅度谱提取结构特征和风格特征，生成解耦的表示令牌。
- 实验或效果：在15个数据集上验证了框架的有效性，通过非对称注入策略提升了视觉语言对齐的鲁棒性。

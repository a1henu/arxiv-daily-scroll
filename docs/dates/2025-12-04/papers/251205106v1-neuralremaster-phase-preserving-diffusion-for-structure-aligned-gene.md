...
**一句话要点**：提出相位保持扩散以解决标准扩散破坏空间结构的问题，适用于图像到图像生成等任务。

**关键词**：相位保持扩散, 结构对齐生成, 图像到图像翻译, 视频生成, 模拟增强

## 3 点简述
- 标准扩散使用高斯噪声破坏相位，导致几何一致性差，不适合结构对齐生成。
- 引入相位保持扩散，保留输入相位并随机化幅度，无需额外参数或架构更改。
- 实验显示在重渲染和模拟增强中实现可控对齐，提升CARLA到Waymo规划器性能50%。

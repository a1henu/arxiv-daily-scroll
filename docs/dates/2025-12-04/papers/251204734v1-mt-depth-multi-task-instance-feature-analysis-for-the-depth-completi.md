...
**一句话要点**：提出MT-Depth框架，通过实例感知引导深度补全，提升稀疏深度数据在自动驾驶等场景下的精度。

**关键词**：深度补全, 实例感知, 跨注意力融合, 自动驾驶, 稀疏深度数据, 对象边界优化

## 3 点简述
- 核心问题：现有深度补全方法依赖语义分割，忽略对象级理解，导致边界和遮挡区域精度不足。
- 方法要点：结合YOLO V11实例分割、U-Net深度补全、跨注意力融合和注意力引导预测头，利用二值实例掩码作为空间先验。
- 实验或效果：在Virtual KITTI 2数据集上验证，相比基线降低RMSE，保持竞争性MAE，有效提升对象边界和薄结构处的深度准确性。

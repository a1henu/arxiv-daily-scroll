...
**一句话要点**：提出基于原型的语义一致性对齐方法，以解决领域自适应检索中的语义对齐与哈希编码质量问题。

**关键词**：领域自适应检索, 语义对齐, 原型学习, 哈希编码, 伪标签可靠性, 特征重构

## 3 点简述
- 核心问题：现有方法忽视类级语义对齐，过度依赖样本对对齐，且伪标签可靠性不足，影响哈希编码质量。
- 方法要点：采用两阶段框架，第一阶段通过正交原型实现类级语义对齐，结合几何邻近性加权伪标签置信度；第二阶段在重构特征上应用量化函数生成跨域统一哈希码。
- 实验或效果：在多个数据集上验证了PSCA的优越性能，提升了检索效果。

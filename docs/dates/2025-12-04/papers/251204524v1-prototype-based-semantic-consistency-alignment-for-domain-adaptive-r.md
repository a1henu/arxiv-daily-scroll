...
**一句话要点**：提出基于原型的语义一致性对齐方法，以解决领域自适应检索中的语义对齐与量化问题。

**关键词**：领域自适应检索, 语义对齐, 原型学习, 哈希编码, 特征重构, 量化函数

## 3 点简述
- 核心问题：现有方法忽视类级语义对齐，过度依赖样本对对齐，且量化受领域偏移影响。
- 方法要点：两阶段框架，第一阶段通过正交原型实现类级语义对齐，第二阶段在重构特征上生成统一哈希码。
- 实验或效果：在多个数据集上验证了性能优越性，提升了检索效果。

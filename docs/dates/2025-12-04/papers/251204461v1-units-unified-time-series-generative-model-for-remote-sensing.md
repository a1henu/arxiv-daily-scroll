...
**一句话要点**：提出UniTS统一时间序列生成模型，以解决遥感多任务中时空特征建模分散的问题。

**关键词**：时间序列生成模型, 遥感图像处理, 流匹配, 时空建模, 多任务学习, 云去除

## 3 点简述
- 核心问题：现有遥感方法需针对不同任务设计专门模型，缺乏多时间序列任务的统一时空建模。
- 方法要点：基于流匹配生成范式，构建从噪声到目标的确定性演化路径，结合自适应条件注入器和时空感知调制器。
- 实验或效果：在低层和高层时间序列任务中表现优异，显著超越现有方法，尤其在云污染、模态缺失和物候变化预测方面。

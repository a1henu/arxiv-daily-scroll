...
**关键词**：3D原生纹理生成, 稀疏潜在表示, 变分自编码器, 条件整流流模型, 纹理保真度, 下游应用

## 3 点简述
- 核心问题：现有3D原生纹理方法因缺乏强大潜在表示，导致纹理保真度和通用性受限。
- 方法要点：使用变分自编码器编码表面外观为稀疏结构化潜在空间，再解码为连续颜色场，实现纹理与网格拓扑的解耦。
- 实验或效果：在重建中PSNR提升超过10 dB，支持多样风格和几何的纹理合成，并应用于材质合成和超分辨率等下游任务。

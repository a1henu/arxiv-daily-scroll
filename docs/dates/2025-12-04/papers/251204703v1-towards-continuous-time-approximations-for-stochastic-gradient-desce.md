...
**一句话要点**：提出基于Young微分方程的连续时间近似，以分析无放回随机梯度下降的收敛性。

**关键词**：随机梯度下降, 连续时间近似, Young微分方程, 收敛分析, 机器学习优化

## 3 点简述
- 核心问题：无放回随机梯度下降的理论分析不足，相比有放回和单次遍历方法。
- 方法要点：使用'分块布朗运动'驱动的Young微分方程，构建带加性噪声的连续时间近似。
- 实验或效果：证明强凸目标下几乎必然收敛，并给出收敛速率上界，优于先前结果。

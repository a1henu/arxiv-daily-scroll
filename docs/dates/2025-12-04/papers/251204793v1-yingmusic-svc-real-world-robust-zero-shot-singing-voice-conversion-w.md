...
**一句话要点**：提出YingMusic-SVC以解决现实歌曲中零样本歌声转换的鲁棒性问题

**关键词**：歌声转换, 零样本学习, 鲁棒性增强, 流匹配损失, 音色解耦, 现实部署

## 3 点简述
- 核心问题：现有零样本歌声转换系统在真实歌曲中因和声干扰、音高错误和缺乏歌唱归纳偏置而脆弱
- 方法要点：结合连续预训练、鲁棒监督微调和Flow-GRPO强化学习，引入歌唱训练的音色转换器和音高感知适配器
- 实验或效果：在分级多轨基准测试中，在音色相似度、可懂度和感知自然度上优于开源基线，尤其在伴奏和和声污染条件下

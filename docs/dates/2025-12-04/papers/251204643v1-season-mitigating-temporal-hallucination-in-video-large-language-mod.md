...
**一句话要点**：提出SEASON方法以缓解视频大语言模型中的时间幻觉问题

**关键词**：视频大语言模型, 时间幻觉, 对比解码, 无需训练方法, 视频理解, 幻觉缓解

## 3 点简述
- 核心问题：视频大语言模型在响应查询时难以有效感知和利用视频中的时间信息，导致时间不一致或因果不合理的描述，引发严重幻觉。
- 方法要点：SEASON是一种无需训练的方法，通过自适应诊断每个输出令牌的幻觉倾向，并对其对应的时间和空间负样本应用自适应对比解码，动态增强输出的时间和空间忠实性。
- 实验或效果：在三个幻觉检测基准上优于现有无需训练的幻觉缓解方法，并在四个通用视频理解基准上进一步提升了视频大语言模型的性能。

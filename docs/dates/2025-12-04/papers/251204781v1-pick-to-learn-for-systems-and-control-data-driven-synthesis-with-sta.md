...
**一句话要点**：提出Pick-to-Learn框架，为数据驱动控制方法提供安全保证，无需预留校准数据。

**关键词**：数据驱动控制, 安全保证, 合成与认证, 最优控制, 可达性分析

## 3 点简述
- 核心问题：数据驱动方法在安全关键系统中需保证安全，但现有方法常牺牲数据或限制算法，导致性能下降。
- 方法要点：P2L框架允许任何数据驱动控制方法联合合成与认证设计，利用全部数据，避免预留校准数据。
- 实验或效果：在最优控制、可达性分析等应用中，P2L提供优于常用方法的设计与证书，展示广泛适用潜力。

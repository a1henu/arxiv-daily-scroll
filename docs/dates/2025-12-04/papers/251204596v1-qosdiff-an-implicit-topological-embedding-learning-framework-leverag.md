...
**一句话要点**：提出QoSDiff框架，利用去噪扩散和对抗注意力实现鲁棒QoS预测，无需显式图构建。

**关键词**：QoS预测, 去噪扩散模型, 对抗注意力, 隐式拓扑嵌入, 服务计算

## 3 点简述
- 核心问题：现有QoS预测方法依赖显式用户-服务交互图，面临可扩展性瓶颈和噪声干扰。
- 方法要点：结合去噪扩散模型恢复潜在结构，并引入对抗注意力模块捕获高阶交互。
- 实验或效果：在真实数据集上超越基线，展现强泛化能力和对数据稀疏与噪声的鲁棒性。

...
**一句话要点**：提出TaskEval以合成任务特定评估器，解决基础模型应用中无标准评估的挑战。

**关键词**：基础模型评估, 任务特定合成, 幻觉检测, 交互协议, 评估自动化, 软件工程应用

## 3 点简述
- 核心问题：基础模型应用中幻觉问题需评估，但缺乏任务特定指标或数据集。
- 方法要点：基于任务无关元模型、交互协议和评估合成器，自动化生成评估程序。
- 实验或效果：在图表数据提取和文档问答任务上，评估准确率分别达93%和90%。

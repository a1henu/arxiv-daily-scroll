...
**关键词**：AI生成图像检测, 视觉Transformer, 特征集成, 门控机制, 泛化能力

## 3 点简述
- 核心问题：现有方法主要依赖CLIP-ViT最终层特征，未充分利用多层特征对检测任务的贡献。
- 方法要点：引入自适应机制MoLD，基于门控动态集成ViT多层特征，利用早期层提供更局部化和泛化性强的特征。
- 实验或效果：在GAN和扩散生成图像上验证，MoLD显著提升检测性能、增强跨模型泛化能力，并展示现实场景鲁棒性。

...
**关键词**：医学图像分类, 迁移学习, 深度学习, 疾病检测, 模型评估, 不确定性分析

## 3 点简述
- 核心问题：医学图像分类中训练大型深度学习模型困难，需高效方法。
- 方法要点：使用六种预训练模型进行迁移学习，分析架构、数据量和领域相似性影响。
- 实验或效果：InceptionV3表现最佳，ResNet随深度提升性能，迁移学习在数据有限时尤其有益。

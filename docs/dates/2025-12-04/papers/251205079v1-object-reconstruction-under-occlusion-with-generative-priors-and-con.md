...
**一句话要点**：提出结合生成先验与接触约束的方法，以解决遮挡下物体重建的歧义问题。

**关键词**：物体重建, 遮挡处理, 生成模型, 接触约束, 3D生成, 机器人操作

## 3 点简述
- 核心问题：相机仅捕获部分观测，遮挡导致物体重建困难，几何信息不完整。
- 方法要点：利用生成模型学习形状先验，结合接触信息提供边界约束，通过接触引导的3D生成整合两者。
- 实验或效果：在合成和真实数据上验证，相比纯生成和接触优化，重建效果提升。

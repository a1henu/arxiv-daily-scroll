...
## 3 点简述
- 核心问题：现有帕金森病步态识别方法存在单模态输入、鲁棒性低和临床可解释性不足的局限。
- 方法要点：采用双YOLOv11编码器提取RGB-D特征，结合多尺度局部-全局提取模块和跨空间融合机制增强时空表示。
- 实验或效果：在多模态步态数据集上验证，相比单模态基线，该框架提高了识别准确率、环境鲁棒性，并生成临床可理解的文本解释。

...
**一句话要点**：提出LatentFM，一种基于潜在流匹配的生成模型，用于医学图像分割以实现高精度和不确定性感知预测。

**关键词**：医学图像分割, 流匹配, 潜在空间建模, 不确定性量化, 生成模型, 变分自编码器

## 3 点简述
- 核心问题：医学图像分割需高精度和不确定性量化，传统方法可能缺乏多样性输出和置信度评估。
- 方法要点：使用两个VAE编码图像和掩码到潜在空间，通过条件流匹配生成多样分割，并计算置信度图。
- 实验或效果：在ISIC-2018和CVC-Clinic数据集上验证，优于基线模型，实现高效准确分割和不确定性量化。

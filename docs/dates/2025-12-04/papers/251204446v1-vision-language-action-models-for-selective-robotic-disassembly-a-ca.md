...
**关键词**：视觉-语言-动作模型, 机器人拆卸, 混合策略, 台式机关键部件, 微调, 规则控制器

## 3 点简述
- 核心问题：台式机关键部件拆卸因产品变异性、不确定性及需精确操作而难以自动化，现有方法泛化能力有限。
- 方法要点：收集定制数据集，微调OpenVLA和OpenVLA-OFT模型，并引入混合策略结合VLA与规则控制器。
- 实验或效果：微调VLA模型能完成早期步骤，但关键子任务失败；混合策略成功实现完整拆卸，揭示VLA模型在灵巧性方面的局限。

...
**关键词**：反射去除, 扩散变换器, 物理渲染, 高效适应, 单图像处理

## 3 点简述
- 核心问题：单图像反射去除，现有数据缺乏多样性和真实感，依赖任务特定架构。
- 方法要点：利用预训练扩散变换器基础模型，通过条件输入和引导实现反射去除，结合物理渲染合成数据。
- 实验或效果：高效LoRA适应结合合成数据，在领域内和零样本基准上达到最先进性能。

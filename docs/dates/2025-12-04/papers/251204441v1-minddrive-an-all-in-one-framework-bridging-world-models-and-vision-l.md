...
**关键词**：端到端自动驾驶, 轨迹规划, 世界模型, 视觉语言模型, 多目标评估, 结构化推理

## 3 点简述
- 核心问题：现有端到端自动驾驶方法在轨迹生成与决策推理间存在割裂，难以兼顾高质量轨迹和全面评估。
- 方法要点：基于世界行动模型进行未来感知轨迹生成，并利用视觉语言模型进行多目标评估，实现结构化推理。
- 实验或效果：在NAVSIM基准测试中达到最先进性能，显著提升安全性、合规性和泛化能力。

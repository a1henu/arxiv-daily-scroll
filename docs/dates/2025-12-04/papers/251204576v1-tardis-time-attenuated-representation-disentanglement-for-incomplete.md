...
**一句话要点**：提出TARDis框架，通过时间衰减表示解耦解决不完整多模态肿瘤分割与分类问题。

**关键词**：肿瘤分割, 多模态学习, 表示解耦, 时间衰减曲线, 条件变分自编码器, 缺失模态处理

## 3 点简述
- 核心问题：CT肿瘤分割与诊断中，因辐射或扫描限制导致多期相数据缺失，忽略血流动力学的时间连续性。
- 方法要点：将缺失模态视为连续时间衰减曲线上的缺失点，解耦特征为时间不变静态解剖和时间依赖动态灌注成分。
- 实验或效果：在大型私有和公共数据集上显著优于现有方法，在极端数据稀疏场景下保持稳健诊断性能。

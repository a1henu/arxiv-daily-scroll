...
**一句话要点**：提出新统计保证以解决小数据集下保形预测器覆盖率的可靠性问题

**关键词**：保形预测, 不确定性量化, 小数据集, 统计保证, 机器学习安全

## 3 点简述
- 核心问题：小校准集导致保形预测覆盖率分布分散，影响不确定性模型的可靠性
- 方法要点：提出新统计保证，为单个保形预测器提供覆盖率的概率信息
- 实验或效果：验证方法在示例中有效，并实现开源软件与现有库兼容

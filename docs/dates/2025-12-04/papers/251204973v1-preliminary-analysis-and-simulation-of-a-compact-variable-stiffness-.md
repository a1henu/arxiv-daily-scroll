...
**一句话要点**：提出紧凑型变刚度并联手腕，通过冗余弹性驱动实现高精度与安全交互。

**关键词**：变刚度执行器, 并联手腕, 冗余弹性驱动, 机器人控制, 仿真验证

## 3 点简述
- 变刚度执行器在非结构化环境中应用受限，因其机械设计导致结构庞大笨重。
- 设计3自由度并联手腕，仅用四个电机实现紧凑轻量，适用于假肢或人形机器人。
- 提出独立调节关节位置与刚度的控制策略，仿真验证高精度和扰动抑制能力。

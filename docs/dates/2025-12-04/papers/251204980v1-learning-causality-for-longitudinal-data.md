...
**一句话要点**：提出CDVAE、RNN-CPC框架和解码器雅可比几何方法，用于高维时序数据的因果推断与表示学习。

**关键词**：因果推断, 时序数据, 表示学习, 个体处理效应, 长程依赖, 可解释性

## 3 点简述
- 核心问题：高维时序数据中因果推断与表示学习，处理未观测异质性和时变混杂。
- 方法要点：CDVAE估计个体处理效应，RNN-CPC框架捕获长程依赖，解码器雅可比几何实现可解释性。
- 实验或效果：CDVAE超越基线，RNN-CPC达到先进水平，几何方法恢复潜在结构无需锚特征。

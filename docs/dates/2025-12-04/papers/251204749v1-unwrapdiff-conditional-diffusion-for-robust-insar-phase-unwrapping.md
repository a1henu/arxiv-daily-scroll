...
**一句话要点**：提出UnwrapDiff，基于条件扩散模型以提升InSAR相位解缠的鲁棒性。

**关键词**：InSAR相位解缠, 条件扩散模型, 去噪扩散概率模型, 合成数据集, 变形监测, 噪声鲁棒性

## 3 点简述
- InSAR相位解缠受噪声和失相关影响，导致变形信号重建困难。
- 方法结合传统SNAPHU输出作为条件引导，利用DDPM框架进行去噪和重建。
- 实验在合成数据集上验证，相比SNAPHU平均降低10.11% NRMSE，在复杂场景如岩墙侵入中表现更优。

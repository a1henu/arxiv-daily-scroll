...
**一句话要点**：提出扩散模型在一般状态空间的理论框架，统一连续与离散域生成建模

**关键词**：扩散模型, 一般状态空间, 连续时间扩散, 离散扩散, 变分训练, 理论框架

## 3 点简述
- 核心问题：现有扩散模型介绍多限于欧几里得数据，缺乏对连续与离散状态空间的统一处理
- 方法要点：基于马尔可夫核和变分方法，推导离散时间与连续时间（SDEs和CTMCs）的扩散过程及训练损失
- 实验或效果：未知，但提供分层介绍，面向不同读者群体，强调可复用的理论原理和证明

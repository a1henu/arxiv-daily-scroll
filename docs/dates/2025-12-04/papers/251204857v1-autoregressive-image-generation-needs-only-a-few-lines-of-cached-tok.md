...
**关键词**：自回归图像生成, 键值缓存压缩, 视觉注意力, 内存优化, 训练无关方法

## 3 点简述
- 自回归图像生成因需缓存所有历史视觉令牌而面临内存瓶颈，导致存储需求高和吞吐量低。
- LineAR是一种无需训练的渐进键值缓存压缩方法，利用视觉注意力特性在线级别管理缓存，逐步淘汰对后续行生成无害的低信息令牌。
- 实验表明，LineAR在多个模型上显著减少内存使用并提升吞吐量，同时保持或改进生成质量，如将ImageNet FID从2.77降至2.68。

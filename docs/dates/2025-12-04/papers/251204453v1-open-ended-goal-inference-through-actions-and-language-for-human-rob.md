...
**一句话要点**：提出BALI方法，通过双向动作-语言推理解决人机协作中的开放目标推断问题。

**关键词**：人机协作, 目标推断, 动作-语言融合, 滚动时域规划, 开放目标集

## 3 点简述
- 核心问题：机器人需推断人类模糊、难以表达或非预设的目标，现有方法依赖固定目标集、仅观察动作或仅依赖指令，导致实际交互脆弱。
- 方法要点：BALI整合自然语言偏好与观察动作，在滚动时域规划树中双向推理，仅在预期信息增益大于中断成本时提问，并选择支持性动作。
- 实验或效果：在协作烹饪任务中评估，BALI相比基线产生更稳定的目标预测和显著更少的错误。

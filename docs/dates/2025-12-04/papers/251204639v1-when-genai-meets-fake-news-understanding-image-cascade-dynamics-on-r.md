...
**一句话要点**：提出集成文本情感、视觉属性和扩散指标的框架，以预测Reddit上AI生成图像和虚假新闻的传播动态。

**关键词**：虚假新闻传播, AI生成图像, 级联动态分析, 社交媒体预测, 视觉内容分析

## 3 点简述
- 核心问题：研究AI生成图像和虚假新闻在Reddit社区中的传播机制，填补视觉内容在虚假信息传播中研究的空白。
- 方法要点：整合文本情感分析、视觉特征提取和扩散指标（如首次转发时间、社区覆盖度），构建预测模型。
- 实验或效果：在五个意识形态多样的Reddit社区进行大规模分析，预测即时帖子级传播（AUC=0.83）和长期级联级传播（AUC=0.998）。

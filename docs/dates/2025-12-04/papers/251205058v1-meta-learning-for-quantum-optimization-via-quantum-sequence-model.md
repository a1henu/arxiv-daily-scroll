...
**一句话要点**：提出量子元学习框架，通过量子序列模型优化QAOA参数初始化以解决收敛慢问题。

**关键词**：量子近似优化算法, 元学习, 量子序列模型, 参数初始化, 量子核长短期记忆, 最大割问题

## 3 点简述
- 核心问题：QAOA参数优化因非凸能量景观导致收敛慢、解质量差。
- 方法要点：训练量子序列模型（如QK-LSTM）作为元学习优化器，生成参数初始化策略。
- 实验或效果：在Max-Cut问题中，QK-LSTM实现最高近似比和最快收敛，参数可迁移至更大规模问题。

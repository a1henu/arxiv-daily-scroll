...
**关键词**：流式视频生成, 蒸馏训练, 注意力机制, 动态质量优化, 高效推理

## 3 点简述
- 核心问题：现有方法使用初始帧作为静态令牌，导致视频帧过度依赖，产生复制和动态减弱。
- 方法要点：引入EMA-Sink令牌，通过指数移动平均融合退出令牌，捕获长期上下文和近期动态；提出Re-DMD，基于视觉语言模型奖励优先动态样本，提升运动质量。
- 实验或效果：在标准基准上达到先进性能，单H100 GPU实现23.1 FPS的高质量流式视频生成。

...
**关键词**：潜在扩散模型, 语义优先建模, 异步去噪, 图像生成, 语义VAE, 高效收敛

## 3 点简述
- 潜在扩散模型生成中语义与纹理同步去噪，忽略语义先于纹理的自然顺序。
- 通过语义VAE提取紧凑语义潜在，结合纹理潜在，采用异步噪声计划优先去噪语义。
- 在ImageNet 256x256上实现FID约1.04，收敛速度提升高达100倍，并改进现有方法。

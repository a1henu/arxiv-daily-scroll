...
**一句话要点**：提出合同治理训练范式，通过观测服务协议图优化地球观测模型覆盖与准确性权衡。

**关键词**：地球观测模型, 合同治理训练, 观测服务协议图, 覆盖准确性权衡, 采样策略优化

## 3 点简述
- 核心问题：地球观测模型训练缺乏对特定区域或类别的服务保障，导致覆盖不均。
- 方法要点：引入服务合同分组样本，使用OSAG监控覆盖并调整采样权重以达成目标服务份额。
- 实验效果：在AVIRIS和Sentinel-2数据集上，OSAG显著减少优先覆盖误差，同时保持全局准确性。

...
**关键词**：大语言模型压缩, 时序逻辑指导, 边缘计算, 形式化方法, 贝叶斯优化, 模型量化

## 3 点简述
- 现有压缩技术如量化和剪枝常损害语言特性且缺乏形式化保证，限制大语言模型在资源受限边缘设备上的应用。
- TOGGLE采用信号时序逻辑形式化指定语言特性，并通过稳健性引导的贝叶斯优化探索层间压缩配置，无需重训练或微调。
- 在四种大语言模型架构上评估，TOGGLE实现计算成本最高3.3倍降低和模型大小最高68.8%减少，同时满足所有语言特性。

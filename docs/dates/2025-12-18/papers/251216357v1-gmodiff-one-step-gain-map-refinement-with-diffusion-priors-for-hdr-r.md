...
**一句话要点**：提出GMODiff，通过增益图一步扩散框架解决多曝光HDR重建中的动态范围限制与推理成本问题。

**关键词**：高动态范围重建, 增益图估计, 潜在扩散模型, 单步去噪, 多曝光图像处理, 感知质量优化

## 3 点简述
- 核心问题：预训练潜在扩散模型直接用于HDR重建存在动态范围受限、推理成本高和内容幻觉挑战。
- 方法要点：将HDR重建重构为条件引导的增益图估计任务，从回归估计初始化实现单步去噪，结合回归先验抑制幻觉。
- 实验或效果：实验表明GMODiff优于多种先进方法，推理速度比基于LDM的方法快100倍。

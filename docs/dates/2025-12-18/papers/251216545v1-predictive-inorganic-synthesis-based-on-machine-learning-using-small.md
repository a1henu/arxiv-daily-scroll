...
**一句话要点**：提出基于小数据集机器学习预测铜纳米粒子尺寸的方法，以优化微波辅助多元醇合成。

**关键词**：机器学习预测, 小数据集, 铜纳米粒子合成, 尺寸控制, 集成回归, 拉丁超立方采样

## 3 点简述
- 核心问题：铜纳米粒子合成对参数敏感，实验优化耗时且数据稀缺，难以实现尺寸可控。
- 方法要点：使用拉丁超立方采样构建25个实验的小数据集，基于AMADEUS框架构建集成回归模型进行预测。
- 实验或效果：模型预测精度高（R²=0.74），优于传统统计方法（R²=0.60），验证小数据集结合可解释ML的可行性。

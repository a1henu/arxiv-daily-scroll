...
**关键词**：加权K调和均值聚类, 无线网络用户关联, 收敛分析, 软分配, 数值稳定性, 随机初始化

## 3 点简述
- 核心问题：传统K调和均值聚类在数值稳定性和软分配方面存在不足，影响无线网络中的用户关联优化。
- 方法要点：引入加权K调和均值聚类，通过逆距离加权实现软分配，提供确定性和随机收敛保证。
- 实验或效果：模拟显示算法在信号强度和负载公平性间取得优越权衡，优于经典和现代基线方法。

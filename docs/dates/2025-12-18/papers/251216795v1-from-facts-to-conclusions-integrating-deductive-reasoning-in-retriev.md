...
**一句话要点**：提出推理轨迹增强的RAG框架，以解决检索信息冲突和不可靠问题。

**关键词**：检索增强生成, 推理轨迹, 冲突分析, 可解释性, LLM评估

## 3 点简述
- 核心问题：RAG在检索源冲突、过时或主观时失效，缺乏统一推理监督。
- 方法要点：引入三阶段结构化推理，包括文档裁决、冲突分析和基于证据的合成。
- 实验或效果：使用Qwen模型，端到端答案正确率从0.069提升至0.883，行为一致性从0.074提升至0.722。

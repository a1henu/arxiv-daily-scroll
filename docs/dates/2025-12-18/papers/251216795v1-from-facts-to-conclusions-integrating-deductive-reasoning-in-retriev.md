...
**一句话要点**：提出推理轨迹增强的RAG框架，以解决检索增强生成中冲突和不可靠信息问题。

**关键词**：检索增强生成, 推理轨迹增强, 冲突分析, LLM评估, 监督微调, 可解释性

## 3 点简述
- 核心问题：检索增强生成在处理冲突、过时或主观信息时表现不佳，缺乏统一推理监督。
- 方法要点：引入三阶段结构化推理，包括文档级裁决、冲突分析和基于证据的合成，并采用冲突感知信任评分管道进行评估。
- 实验或效果：在Qwen模型上，监督微调使端到端答案正确率从0.069提升至0.883，行为一致性从0.074提升至0.722。

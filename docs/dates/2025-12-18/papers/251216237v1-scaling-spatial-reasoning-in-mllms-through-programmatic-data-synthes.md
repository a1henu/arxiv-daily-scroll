...
**一句话要点**：提出SPRITE框架，通过程序化数据合成解决多模态大模型空间推理数据可扩展性与质量困境

**关键词**：空间推理, 程序化数据合成, 多模态大模型, 模拟器验证, 指令调优

## 3 点简述
- 核心问题：现有方法在空间推理数据生成上存在可扩展性与多样性、计算精度间的矛盾
- 方法要点：利用LLMs将空间问题编译为可执行程序，结合模拟器元信息验证，实现高质量数据合成
- 实验或效果：构建包含300k+样本的数据集，训练模型在多个空间基准测试中表现优异，验证可扩展性假设

...
**一句话要点**：提出DVGT以从无位姿多视角图像序列重建全局密集3D点云地图，适应不同驾驶场景和相机配置。

**关键词**：自动驾驶, 3D重建, 视觉Transformer, 多视图几何, 密集点云, 无位姿输入

## 3 点简述
- 核心问题：缺乏适应不同场景和相机配置的驾驶专用密集几何感知模型。
- 方法要点：使用DINO提取特征，通过交替注意力机制推断跨视图和跨帧几何关系，直接预测度量尺度几何。
- 实验或效果：在混合驾驶数据集上训练，显著优于现有模型，无需精确相机参数或外部传感器对齐。

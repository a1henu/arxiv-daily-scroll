...
**一句话要点**：综述信息检索中重排序模型的演进，从启发式方法到大型语言模型，聚焦RAG管道应用。

**关键词**：信息检索重排序, 检索增强生成, 神经网络架构, 大型语言模型, 知识蒸馏, 计算效率

## 3 点简述
- 核心问题：重排序在信息检索中提升结果相关性，尤其在RAG管道中影响输出质量。
- 方法要点：涵盖从基础方法到神经网络架构（如交叉编码器、T5、GNN）及LLM集成，分析效率优化如知识蒸馏。
- 实验或效果：提供结构化综述，比较不同策略的原理、效果、计算特征和实际权衡。

...
**一句话要点**：提出骨架片段对比学习与多尺度特征融合方法，以提升基于骨架的时序动作定位性能。

**关键词**：骨架动作定位, 对比学习, 自监督预训练, 多尺度特征融合, 时序动作检测

## 3 点简述
- 核心问题：骨架时序动作定位需捕捉相邻帧间细微差异，现有自监督预训练方法对此挑战性大。
- 方法要点：设计片段判别前置任务，通过对比学习区分非重叠骨架片段；融合中间特征增强帧级定位分辨率。
- 实验或效果：在BABEL数据集上改进现有方法，在PKUMMD上实现迁移学习SOTA性能。

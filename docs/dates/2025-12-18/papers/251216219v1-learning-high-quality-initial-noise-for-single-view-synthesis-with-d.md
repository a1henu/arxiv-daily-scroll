...
**关键词**：单视图合成, 扩散模型, 初始噪声学习, 编码器-解码器网络, 新视角生成

## 3 点简述
- 核心问题：单视图新视角合成中，扩散模型缺乏学习高质量初始噪声的专用框架。
- 方法要点：设计离散化欧拉反演法构建噪声配对数据集，并训练编码器-解码器网络直接转换随机噪声。
- 实验或效果：框架可无缝集成到SV3D等模型，在多个数据集上实现显著性能提升。

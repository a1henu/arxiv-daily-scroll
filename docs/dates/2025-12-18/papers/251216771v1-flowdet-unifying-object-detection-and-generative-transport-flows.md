...
## 3 点简述
- 核心问题：目标检测通常依赖非生成式方法，FlowDet将其重构为生成式传输问题，以提升灵活性和性能。
- 方法要点：基于DiffusionDet，采用条件流匹配学习更直、更简单的传输路径，支持可变框数和推理步数，无需重训练。
- 实验或效果：在COCO和LVIS数据集上，FlowDet优于基于扩散的检测系统，AP提升达+3.6%，AP$_{rare}$提升达+4.2%。

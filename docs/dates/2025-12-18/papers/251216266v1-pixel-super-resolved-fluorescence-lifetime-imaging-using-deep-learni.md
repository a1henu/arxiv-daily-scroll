...
**一句话要点**：提出基于深度学习的像素超分辨率框架FLIM_PSR_k，以提升荧光寿命成像的空间分辨率与速度。

**关键词**：荧光寿命成像, 像素超分辨率, 条件生成对抗网络, 深度学习, 图像重建, 临床诊断

## 3 点简述
- 核心问题：荧光寿命成像临床应用中存在像素停留时间长、信噪比低和分辨率-速度权衡严格的问题。
- 方法要点：采用条件生成对抗网络框架，从像素尺寸增大的数据中重建高分辨率图像，实现快速推理。
- 实验或效果：在患者肿瘤组织样本上盲测，实现5倍超分辨率因子，图像质量指标显著提升，揭示精细结构特征。

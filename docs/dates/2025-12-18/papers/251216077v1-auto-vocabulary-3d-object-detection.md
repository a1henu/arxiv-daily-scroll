...
**一句话要点**：提出Auto-Vocabulary 3D目标检测框架，自动生成检测对象的类别，无需用户输入。

**关键词**：3D目标检测, 开放词汇检测, 视觉语言模型, 语义生成, 自动类别生成

## 3 点简述
- 核心问题：现有开放词汇3D检测方法依赖用户指定类别，无法自动生成对象类别。
- 方法要点：利用2D视觉语言模型通过图像描述、伪3D框生成和特征空间语义扩展生成丰富语义候选。
- 实验或效果：在ScanNetV2和SUNRGB-D数据集上实现定位和语义质量的最优性能，超越现有方法。

...
**关键词**：能量基模型, 语言建模, GPT架构, 推理过程, 抗过拟合

## 3 点简述
- 核心问题：GPT架构流行，但能量基建模作为不同范式，其推理过程基于能量景观探索。
- 方法要点：通过最小修改GPT设置，将推理概念化为能量景观上的标记探索，证明在某些条件下可简化为梯度下降。
- 实验或效果：在Shakespeare、ListOPS和OpenWebText数据集上表现良好，模型可能更抗过拟合，仅在长时间训练时过拟合。

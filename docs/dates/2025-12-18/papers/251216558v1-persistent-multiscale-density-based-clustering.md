...
**关键词**：密度聚类, 参数选择, 尺度空间聚类, 持续同调, HDBSCAN*, 聚类稳定性

## 3 点简述
- 核心问题：密度聚类算法如DBSCAN和HDBSCAN*需预设参数，缺乏先验知识时难以选择
- 方法要点：PLSCAN基于尺度空间聚类原理，自动识别HDBSCAN*的稳定叶簇，等价于新度量空间上的持续同调
- 实验或效果：在真实数据集上，PLSCAN平均ARI更高，对互达邻居数变化更不敏感，计算成本在低维数据中与k-Means竞争

...
**一句话要点**：提出ResDynUNet++，结合知识驱动与数据驱动模块，用于双能CT重建以解决通道不平衡和界面附近大伪影问题。

**关键词**：双能CT重建, 残差动态卷积, 知识驱动模块, 数据驱动模块, UNet++, 伪影抑制

## 3 点简述
- 核心问题：双能CT重建中通道不平衡和界面附近大伪影的挑战。
- 方法要点：采用OPMT快速生成中间解，再用ResDynUNet++（基于UNet++的残差动态卷积块）进行精炼。

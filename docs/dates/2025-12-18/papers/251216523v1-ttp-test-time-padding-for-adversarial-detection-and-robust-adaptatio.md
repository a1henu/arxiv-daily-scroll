...
**一句话要点**：提出测试时填充方法以增强视觉语言模型的对抗检测与鲁棒适应能力

**关键词**：视觉语言模型, 对抗检测, 测试时适应, 空间填充, 余弦相似度, 鲁棒性增强

## 3 点简述
- 视觉语言模型如CLIP易受对抗扰动攻击，现有测试时防御策略难以可靠区分干净与对抗输入
- TTP通过空间填充前后特征嵌入的余弦相似度偏移进行对抗检测，并利用可训练填充恢复注意力模式
- 实验表明TTP在多种CLIP骨干和细粒度基准上超越现有测试时防御，显著提升对抗鲁棒性且不损害干净准确率

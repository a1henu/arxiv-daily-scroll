...
**一句话要点**：提出RePlan框架，通过推理引导的区域规划解决复杂指令图像编辑中的视觉-指令复杂性挑战。

**关键词**：指令图像编辑, 视觉语言规划, 扩散模型, 区域定位, 强化学习, IV-Complexity

## 3 点简述
- 核心问题：现有模型在指令-视觉复杂性（IV-Complexity）下表现不佳，即复杂指令与杂乱或模糊场景结合时编辑困难。
- 方法要点：采用计划-执行框架，结合视觉语言规划器分解指令并定位目标区域，使用免训练的注意力区域注入机制进行并行多区域编辑。
- 实验或效果：在IV-Edit基准测试中，RePlan在区域精度和整体保真度上优于基线模型，即使训练数据量更少。

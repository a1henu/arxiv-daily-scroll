...
**关键词**：本征分解, 多尺度注意力, 人脸图像, 渲染通道预测, 真实感重光照, 数字孪生

## 3 点简述
- 核心问题：无约束光照下人脸图像的本征分解，是真实感重光照和数字孪生的前提。
- 方法要点：MAGINet采用多尺度注意力引导网络，预测光归一化漫反射反照率图，并通过RefinementNet和Pix2PixHD预测额外渲染通道。
- 实验或效果：在FFHQ-UV-Intrinsics数据集上训练，实现最先进的漫反射反照率估计，并提升完整渲染栈的保真度。

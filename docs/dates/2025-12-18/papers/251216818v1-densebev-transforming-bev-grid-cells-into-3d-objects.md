...
**关键词**：多摄像头3D物体检测, BEV变换器, 锚点生成, 时序建模, 非极大值抑制, 端到端训练

## 3 点简述
- 核心问题：传统BEV变换器使用随机查询作为锚点，效率低且依赖辅助网络检测。
- 方法要点：采用两阶段锚点生成，利用BEV特征细胞作为锚点，结合BEV-NMS和混合时序建模提升性能。
- 实验或效果：在nuScenes和Waymo数据集上显著提升NDS和mAP，尤其对小物体检测效果突出。

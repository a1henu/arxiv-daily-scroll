...
**关键词**：全身运动跟踪, 稀疏信号重建, 状态空间模型, 时空依赖建模, AR/VR应用, 运动学引导

## 3 点简述
- 问题：AR/VR头显稀疏信号难以重建真实多样全身姿态，现有方法计算成本高或时空依赖建模分离。
- 方法：引入运动学引导双向扫描和混合时空表示学习，耦合时空上下文并集成几何角速度损失。
- 效果：在轻量框架下，实验显示KineST在准确性和时间一致性上表现优越。

...
**关键词**：对抗强化学习, 大语言模型推理, 步骤级奖励, 数学推理增强, 判别器模块化

## 3 点简述
- 核心问题：大语言模型在数学推理中仍存在计算错误、逻辑脆弱等过程错误。
- 方法要点：采用同策略联合训练，通过推理器与判别器的对抗学习生成密集步骤级奖励。
- 实验或效果：在多个数学基准上提升性能，如AIME24上DeepSeek模型得分显著提高。

...
**关键词**：零样本对象计数, 示例选择, 开放词汇检测, 密度引导, 特征聚类, 无训练框架

## 3 点简述
- 核心问题：零样本对象计数中，现有方法依赖开放词汇检测器或随机采样，难以准确界定对象实例。
- 方法要点：采用无训练框架，通过检测锚定、密度引导和特征共识三阶段协同，渐进发现多样示例。
- 实验或效果：在多个数据集上优于现有零样本计数方法，并有效泛化至自然、航空和医学领域。

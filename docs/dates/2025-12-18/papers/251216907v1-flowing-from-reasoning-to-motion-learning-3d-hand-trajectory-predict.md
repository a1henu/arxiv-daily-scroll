...
**一句话要点**：提出EgoMAN数据集与模型，通过推理到运动框架解决3D手部轨迹预测中语义与运动脱节问题。

**关键词**：3D手部轨迹预测, 第一人称视频, 推理到运动框架, 视觉语言推理, 轨迹令牌接口, 阶段感知预测

## 3 点简述
- 核心问题：现有3D手部轨迹预测数据集和模型将运动与语义推理弱耦合，限制准确性。
- 方法要点：构建EgoMAN数据集，含219K轨迹和3M问答对；设计EgoMAN模型，通过轨迹令牌接口连接视觉语言推理与运动生成。
- 实验或效果：模型经渐进训练对齐推理与运动动态，在真实场景中实现准确且阶段感知的轨迹预测。

...
**一句话要点**：提出CPU推理的能耗缩放定律与量子压缩方法，以优化边缘设备本地大模型部署。

**关键词**：边缘计算, CPU推理, 能耗缩放定律, 量子压缩, 本地大模型, 分辨率预处理

## 3 点简述
- 核心问题：CPU推理的能耗与计算负载缩放规律未知，影响本地大模型在边缘设备的部署效率。
- 方法要点：系统基准测试CPU推理，发现线性缩放定律与分辨率拐点，并应用量子压缩降低能耗。
- 实验或效果：在MacBook Pro M2和Raspberry Pi 5上验证，量子压缩最高减少71.9%计算使用和62%能耗，保持准确性。

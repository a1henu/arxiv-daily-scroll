...
**一句话要点**：提出GFLAN框架，通过拓扑规划与几何实现两阶段分解，解决自动楼层平面图生成中捕获建筑推理的挑战。

**关键词**：自动楼层平面图生成, 拓扑规划, 几何实现, 图神经网络, 卷积网络

## 3 点简述
- 核心问题：现有深度学习方法难以捕获建筑推理，如拓扑关系优先于几何实例化、功能约束传播和流通模式涌现。
- 方法要点：采用两阶段生成框架，先通过卷积网络分配房间质心，再用图神经网络回归房间边界。
- 实验或效果：未知。

...
**一句话要点**：提出Multi-AD模型，通过跨域无监督异常检测解决医学和工业图像中标注数据不足的问题。

**关键词**：跨域异常检测, 无监督学习, 知识蒸馏, 注意力机制, 医学图像分析, 工业缺陷检测

## 3 点简述
- 核心问题：传统深度学习模型在跨域异常检测中缺乏标注数据，影响医学早期诊断和工业缺陷检测。
- 方法要点：结合SE块增强特征提取，利用知识蒸馏和判别器网络区分正常与异常数据，采用师生架构和多尺度特征集成。
- 实验或效果：在医学和工业数据集上评估，图像级和像素级AUROC均优于现有方法，展现强泛化能力。

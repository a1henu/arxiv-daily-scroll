...
**一句话要点**：提出LeakBoost框架，基于感知损失主动探测模型内部表示以增强成员推断攻击效果

**关键词**：成员推断攻击, 感知损失, 主动探测, 隐私风险, 白盒设置, 图像分类

## 3 点简述
- 成员推断攻击旨在判断样本是否属于训练集，现有方法依赖静态指标，未充分利用主动探测的动态行为
- LeakBoost通过优化感知损失合成探测图像，放大成员与非成员在表示空间的差异，无需修改现有检测器
- 实验显示，结合现有方法显著提升AUC和低误报率下的真阳性率，在多种数据集和架构上验证有效性

...
**一句话要点**：提出概念分布式对齐搜索以通过分布匹配实现忠实双向模型操控

**关键词**：模型操控, 分布式对齐搜索, 分布匹配, 干预方法, 双向控制, 安全对齐

## 3 点简述
- 当前基于干预的模型操控方法易过拟合，因依赖外部偏好而非内部机制识别
- CDAS采用分布式互换干预和分布匹配目标，实现弱监督双向操控，减少超参数调优
- 在安全案例中，CDAS能系统操控模型行为，同时保持通用效用，与偏好优化方法互补

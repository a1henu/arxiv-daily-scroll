...
**一句话要点**：提出多尺度全局-实例提示调优方法，以增强医学图像分割中持续测试时适应的鲁棒性。

**关键词**：持续测试时适应, 医学图像分割, 提示调优, 多尺度学习, 全局-实例知识, 分布偏移

## 3 点简述
- 核心问题：医学图像分布偏移导致预训练模型跨域部署困难，现有持续测试时适应方法易受错误累积和灾难性遗忘影响。
- 方法要点：设计自适应尺度实例提示和多尺度全局提示，通过加权集成捕获全局与实例级知识，提升提示多样性和抗遗忘能力。
- 实验或效果：在医学图像分割基准测试中优于现有方法，实现持续变化目标域的鲁棒适应。

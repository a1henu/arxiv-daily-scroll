...
**一句话要点**：提出TimelyFreeze自适应参数冻结机制，以优化流水线并行训练中的吞吐量并减少精度损失。

**关键词**：流水线并行, 参数冻结, 训练优化, 线性规划, 模型训练, 吞吐量提升

## 3 点简述
- 核心问题：流水线并行训练中，现有参数冻结方法常导致过度冻结，引起不必要的精度下降。
- 方法要点：将流水线调度建模为有向无环图，通过线性规划计算最优冻结比率，在精度约束下最小化批次执行时间。
- 实验或效果：在LLaMA-8B上实现高达40%的训练吞吐量提升，同时保持可比的精度，适用于多种流水线并行设置。

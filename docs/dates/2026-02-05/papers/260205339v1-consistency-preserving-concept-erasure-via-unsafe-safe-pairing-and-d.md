...
**关键词**：概念擦除, 文本到图像扩散模型, 语义一致性, 参数高效微调, 不安全-安全配对, Fisher加权适应

## 3 点简述
- 现有概念擦除方法缺乏安全替代指导，导致结构语义一致性受损
- PAIR框架使用不安全-安全配对进行语义重对齐，并采用Fisher加权初始化DoRA
- 实验显示PAIR在擦除效果、结构完整性和生成质量上优于基线方法

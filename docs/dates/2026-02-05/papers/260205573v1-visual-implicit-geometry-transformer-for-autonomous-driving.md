...
**关键词**：自动驾驶几何建模, 连续3D占据场, 无校准架构, 自监督训练, 多传感器融合, 鸟瞰图表示

## 3 点简述
- 核心问题：自动驾驶需从多相机视图估计连续3D几何，传统方法依赖校准和标注，难以泛化。
- 方法要点：采用无校准架构和自监督训练，利用图像-LiDAR对，实现跨传感器配置的通用几何建模。
- 实验或效果：在五个大规模数据集上训练，点图估计任务达到最佳平均排名，Occ3D-nuScenes基准与监督方法性能相当。

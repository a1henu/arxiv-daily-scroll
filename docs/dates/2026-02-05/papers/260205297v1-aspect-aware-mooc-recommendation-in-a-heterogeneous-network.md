...
**一句话要点**：提出AMR框架，通过自动发现元路径和嵌入语义内容，解决MOOC推荐中数据稀疏和依赖预定义元路径的问题。

**关键词**：MOOC推荐, 异构网络, 元路径发现, 方面感知表示, 图神经网络, 语义嵌入

## 3 点简述
- 核心问题：传统MOOC推荐方法面临数据稀疏和过度依赖预定义元路径，导致推荐精度受限和工程成本高。
- 方法要点：AMR自动发现元路径，使用bi-LSTM编码器生成方面感知路径表示，并作为边特征集成到异构网络中。

...
**关键词**：社交媒体审核, 大语言模型, 开源权重模型, Bluesky平台, 有害内容检测

## 3 点简述
- 核心问题：开源权重大语言模型在社交媒体内容审核任务中的开箱即用能力尚不明确。
- 方法要点：在Bluesky平台上，使用真实帖子和人工标注，评估七种先进模型（四种专有、三种开源权重）。
- 实验或效果：开源权重模型在敏感性和特异性上与专有模型有相当重叠，支持隐私保护审核。

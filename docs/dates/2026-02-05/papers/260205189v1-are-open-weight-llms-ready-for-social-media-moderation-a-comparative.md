...
**关键词**：社交媒体审核, 大语言模型, 开源权重模型, 有害内容检测, Bluesky平台, 隐私保护

## 3 点简述
- 核心问题：开源权重大语言模型在社交媒体内容审核中的开箱即用能力是否与专有模型相当
- 方法要点：在Bluesky平台上测试七种先进模型，包括四种专有和三种开源权重模型，使用真实帖子和人工标注
- 实验或效果：开源模型与专有模型在敏感性和特异性上重叠度高，支持隐私保护审核，并揭示不同有害内容类型的检测差异

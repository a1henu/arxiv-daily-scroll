...
**一句话要点**：提出基于LLM的儿童话语评估框架，通过扩展性和独立性维度解决对话语境敏感度不足问题。

**关键词**：儿童话语评估, 对话语境敏感度, LLM评判框架, 扩展性评分, 独立性评分, 语言发展研究

## 3 点简述
- 核心问题：现有儿童话语评估指标如MLU和词汇多样性忽略对话语境，无法捕捉推理深度和话题维持等质量维度。
- 方法要点：引入LLM作为评判者，先分类成人前话语类型，再沿扩展性和独立性两个轴评分儿童回应。
- 实验或效果：验证了年龄相关模式的发展有效性，并提升年龄估计准确性，与人类判断一致。

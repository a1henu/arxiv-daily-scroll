...
**关键词**：合成缺陷建模, Voronoi剖分, 无损检测, 参数化方法, 合成数据生成, 铸造缺陷

## 3 点简述
- 核心问题：工业无损检测中，自动化缺陷检测需要大量高质量训练数据，但真实数据获取困难且难以覆盖罕见缺陷。
- 方法要点：采用参数化方法，基于2D Voronoi剖分建模3D网格缺陷，可生成可控的合成缺陷几何体，适用于多种缺陷类型。
- 实验或效果：通过物理模拟生成合成数据，支持像素级标注，可扩展数据集并包含罕见缺陷，以视觉表面检测为例验证。

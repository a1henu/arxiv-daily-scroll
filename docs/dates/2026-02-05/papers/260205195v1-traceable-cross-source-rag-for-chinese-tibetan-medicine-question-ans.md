...
**关键词**：检索增强生成, 多知识库问答, 藏医药领域, 可追溯性, 证据融合, 跨源检索

## 3 点简述
- 核心问题：藏医药领域多知识库（百科全书、经典、临床论文）检索中，密集条目易主导检索，掩盖权威证据。
- 方法要点：DAKS进行知识库路由与预算检索以减少密度偏差；对齐图指导证据融合与覆盖感知打包，提升跨库证据覆盖。
- 实验或效果：在500查询基准上，系统提升路由质量与跨库证据覆盖，保持强忠实性与引用正确性。

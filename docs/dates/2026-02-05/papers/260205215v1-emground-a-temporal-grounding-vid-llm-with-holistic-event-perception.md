...
**一句话要点**：提出E.M.Ground以解决视频大语言模型在时序视频定位中语义连续性和完整性不足的问题。

**关键词**：时序视频定位, 视频大语言模型, 事件感知, 语义连续性, 帧特征聚合, 噪声平滑

## 3 点简述
- 核心问题：现有方法依赖精确时间戳匹配起止帧，忽略事件语义连续性，导致定位模糊。
- 方法要点：引入<event>令牌聚合事件所有帧信息，使用Savitzky-Golay平滑减少噪声，并采用多粒度帧特征聚合增强匹配可靠性。
- 实验或效果：在基准数据集上实验显示，E.M.Ground显著优于现有最先进视频大语言模型。

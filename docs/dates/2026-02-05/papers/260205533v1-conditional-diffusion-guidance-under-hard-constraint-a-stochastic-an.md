...
**关键词**：条件扩散模型, 硬约束引导, 随机分析, 罕见事件模拟, 鞅表示理论, 离策略学习

## 3 点简述
- 研究扩散模型在硬约束下的条件生成问题，要求样本以概率一满足约束条件。
- 基于Doob's h变换、鞅表示和二次变差过程，开发无需修改预训练分数网络的引导动力学。
- 提出两种离策略学习算法，通过数值实验验证硬约束执行和罕见事件样本生成的有效性。

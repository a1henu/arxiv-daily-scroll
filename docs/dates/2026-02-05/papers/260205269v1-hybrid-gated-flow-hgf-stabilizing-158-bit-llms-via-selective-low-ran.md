...
**关键词**：1.58位量化, 低秩校正, 边缘部署, 混合门控流, 大语言模型, 结构正则化

## 3 点简述
- 核心问题：1.58位量化技术显著降低内存占用，但导致困惑度下降20-25%，影响边缘设备部署。
- 方法要点：采用双流架构，结合1.58位三元主干和由自适应门控制的低秩FP16校正路径，实现选择性校正。
- 实验或效果：在TinyStories数据集上，HGF 5.4验证损失为0.9306，比BitNet提升约55%质量，内存开销仅增加12-15%。

...
**一句话要点**：提出LinguistAgent平台，通过反思多模型架构自动化语言标注，解决人文学科数据标注瓶颈。

**关键词**：语言标注自动化, 反思多模型架构, 双代理工作流, 隐喻识别, 实时评估, 人文学科应用

## 3 点简述
- 核心问题：人文学科中复杂语义任务如隐喻识别的数据标注效率低下，LLMs理论与实际应用存在差距。
- 方法要点：采用双代理工作流（标注者与审阅者）模拟同行评审，支持提示工程、检索增强生成和微调三种范式比较。
- 实验或效果：以隐喻识别为例，提供实时令牌级评估（精确率、召回率、F1分数），对比人类黄金标准验证有效性。

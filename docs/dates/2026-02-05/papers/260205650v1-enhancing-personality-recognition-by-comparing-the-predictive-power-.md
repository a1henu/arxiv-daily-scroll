...
**一句话要点**：提出基于细粒度人格层次（特质、方面、细微特征）的预测模型，以增强音视频交互数据中的人格识别性能。

**关键词**：人格识别, 音视频交互, Transformer模型, 注意力机制, 大五人格模型, 细粒度预测

## 3 点简述
- 核心问题：依赖宽泛特质分数作为真实标签，结合有限训练数据，导致人格识别模型泛化能力受限。
- 方法要点：使用基于Transformer的模型，包含跨模态（音视频）和跨主体（双人感知）注意力机制，探索大五人格模型中更细粒度的层次（方面和细微特征）。
- 实验或效果：在UDIVA v0.5数据集上，细微特征级模型在交互场景中均优于方面和特质级模型，均方误差最多降低74%。

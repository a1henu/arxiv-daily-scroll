...
**关键词**：人格识别, 大五人格模型, Transformer模型, 视听交互数据, 注意力机制, 细粒度预测

## 3 点简述
- 核心问题：人格识别模型依赖宽泛特质分数作为真值，结合有限训练数据，导致泛化挑战，因相似特质可能表现为多样、上下文依赖的行为。
- 方法要点：探索大五人格模型中更细粒度的层次（方面和细微特征），使用基于Transformer的模型，包括跨模态（视听）和跨主体（对偶感知）注意力机制。
- 实验或效果：在UDIVA v0.5数据集上，细微特征级模型在交互场景中一致优于方面和特质级模型，均方误差降低高达74%。

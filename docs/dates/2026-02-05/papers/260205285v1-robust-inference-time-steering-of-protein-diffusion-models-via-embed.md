...
**一句话要点**：提出EmbedOpt以通过嵌入优化在推理时稳健引导蛋白质扩散模型

**关键词**：蛋白质扩散模型, 推理时引导, 嵌入优化, 后验采样, 冷冻电镜拟合, 实验约束

## 3 点简述
- 核心问题：后验采样在目标位于先验低密度区域时需脆弱权重化实验似然。
- 方法要点：在条件嵌入空间优化实验似然，利用序列和共进化信号调整扩散先验。
- 实验或效果：在冷冻电镜图拟合任务中优于坐标后验采样，超参数稳健性高且推理步骤减少。

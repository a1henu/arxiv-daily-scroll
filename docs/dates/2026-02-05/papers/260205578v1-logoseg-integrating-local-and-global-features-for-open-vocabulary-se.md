...
**关键词**：开放词汇语义分割, 视觉语言模型, 局部全局特征融合, 物体存在先验, 区域感知对齐

## 3 点简述
- 核心问题：现有方法依赖图像级预训练，导致空间对齐不精确，在模糊或杂乱场景中产生分割不匹配。
- 方法要点：引入物体存在先验、区域感知对齐模块和双流融合机制，无需外部掩码提议或额外数据集。
- 实验或效果：在六个基准测试中展示竞争性性能和强泛化能力，验证了框架的有效性。

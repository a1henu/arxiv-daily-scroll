...
**一句话要点**：提出路径引导流匹配以解决数据集蒸馏中扩散方法效率低和轨迹不稳定的问题。

**关键词**：数据集蒸馏, 流匹配, 路径引导, ODE合成, 高效采样, 潜在空间学习

## 3 点简述
- 核心问题：基于扩散的数据集蒸馏方法依赖启发式引导，导致采样耗时、轨迹不稳定，影响下游泛化。
- 方法要点：在冻结VAE的潜在空间进行流匹配，通过ODE快速确定性合成，并开发连续路径到原型引导算法控制轨迹。

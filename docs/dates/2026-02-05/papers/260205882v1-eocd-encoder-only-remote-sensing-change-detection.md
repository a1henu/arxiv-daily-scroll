...
**一句话要点**：提出EoCD方法，通过早期融合和参数无关多尺度特征融合模块简化遥感变化检测模型。

**关键词**：遥感变化检测, 早期融合, 参数无关模块, 模型简化, 编码器优化

## 3 点简述
- 现有方法依赖孪生编码器或复杂解码器，导致计算成本和模型复杂度高。
- EoCD采用早期融合时间数据，用参数无关多尺度特征融合模块替代解码器，降低复杂度。
- 在四个数据集上实验显示，EoCD在性能和速度间取得平衡，性能主要依赖编码器。

...
**一句话要点**：提出SpectraKAN，通过输入条件化谱算子以解决偏微分方程多尺度动态建模问题。

**关键词**：谱神经算子, 偏微分方程求解, 输入条件化, 多尺度建模, 傅里叶神经算子

## 3 点简述
- 现有谱算子使用静态傅里叶核，难以捕捉系统全局状态依赖的多尺度、各向异性动态。
- SpectraKAN从时空历史提取全局表示，通过单查询交叉注意力调制多尺度傅里叶主干，实现输入条件化积分算子。
- 在多样偏微分方程基准测试中，SpectraKAN达到最先进性能，均方根误差降低高达49%。

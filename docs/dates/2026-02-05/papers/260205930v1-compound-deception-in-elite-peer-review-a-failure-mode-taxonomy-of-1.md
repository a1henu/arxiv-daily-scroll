...
**一句话要点**：提出五类幻觉分类法，揭示NeurIPS 2025中100个伪造引用的复合欺骗模式，建议自动化验证解决方案。

**关键词**：伪造引用检测, AI幻觉分类, 同行评审失效, 自动化验证, 学术诚信

## 3 点简述
- 分析NeurIPS 2025中100个AI生成伪造引用，揭示其在53篇论文中未被同行评审检测。
- 开发五类失败模式分类法，显示所有幻觉均呈现复合结构，如完全伪造与语义幻觉结合。
- 发现引用验证缺失问题，提出提交时强制自动化验证以防止伪造引用在科学文献中常态化。

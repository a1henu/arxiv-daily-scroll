...
**一句话要点**：提出五类失败模式分类法，分析NeurIPS 2025中100个AI生成伪造引用的复合欺骗现象。

**关键词**：伪造引用检测, 同行评审失败, 大型语言模型幻觉, 学术诚信, 自动化验证

## 3 点简述
- 核心问题：大型语言模型在学术写作中生成不存在的伪造引用，在NeurIPS 2025的53篇论文中未被同行评审检测。
- 方法要点：基于100个伪造引用，开发五类失败模式分类法，包括完全伪造和部分属性腐败等，揭示复合结构特征。
- 实验或效果：发现所有伪造引用均呈现复合失败模式，92%受污染论文含1-2个伪造引用，8%含4-13个，建议强制自动化引用验证。

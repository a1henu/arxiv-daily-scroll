...
**关键词**：早产儿视网膜病变筛查, 上下文感知集成, 主动查询网络, 血管拓扑图, 可解释性AI, 医学图像分析

## 3 点简述
- 核心问题：早产儿视网膜病变自动筛查面临数据有限、结构分期和微血管异常复杂等挑战。
- 方法要点：采用多尺度主动查询网络和血管拓扑图编码，模拟临床推理进行非对称集成。
- 实验或效果：在188名婴儿数据集上，实现宽泛分期F1分数0.93和Plus病检测AUC 0.996。

...
**一句话要点**：提出PMD-mean算法，通过近似对数配分函数解决大语言模型后训练中的策略镜像下降隐式正则化问题。

**关键词**：策略镜像下降, 大语言模型后训练, 隐式正则化, 对数配分函数近似, 强化学习算法

## 3 点简述
- 核心问题：策略镜像下降在LLM后训练中需可靠估计配分函数，但动作空间大、样本有限导致挑战。
- 方法要点：PMD-mean用采样策略的平均奖励近似对数配分项，在log-policy空间进行回归，引入自适应混合KL-χ²正则化。
- 实验或效果：在数学推理任务上，PMD-mean实现更优性能，提升稳定性和时间效率。

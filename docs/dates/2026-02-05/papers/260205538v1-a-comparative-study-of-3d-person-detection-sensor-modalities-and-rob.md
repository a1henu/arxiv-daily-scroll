...
**关键词**：3D行人检测, 传感器融合, 鲁棒性评估, 室内外场景, JRDB数据集

## 3 点简述
- 核心问题：评估3D行人检测在机器人、监控等应用中的传感器模态选择与鲁棒性挑战
- 方法要点：使用JRDB数据集，系统比较BEVDepth（相机）、PointPillars（LiDAR）和DAL（融合）三种模型
- 实验或效果：融合方法在遮挡和距离变化下表现最佳，但对传感器错位和LiDAR噪声敏感

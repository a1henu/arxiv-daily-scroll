...
**关键词**：超关系知识图谱, 归纳链接预测, 图神经网络, Transformer解码器, 结构不变性

## 3 点简述
- 核心问题：现有超关系知识图谱链接预测方法多为直推式，无法泛化到未见词汇。
- 方法要点：引入关系和实体基础图建模超关系知识图谱的结构不变性，使用图编码器和Transformer解码器进行归纳学习。
- 实验或效果：在12个数据集上评估，THOR优于基线方法，在归纳任务中提升显著。

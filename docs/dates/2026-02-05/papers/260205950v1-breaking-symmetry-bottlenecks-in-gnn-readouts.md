...
**一句话要点**：提出投影器不变读出以解决图神经网络中线性不变读出的表达能力瓶颈

**关键词**：图神经网络, 读出设计, 置换不变性, 表达能力瓶颈, 对称感知表示, 非线性池化

## 3 点简述
- 核心问题：线性置换不变读出（如求和池化）通过Reynolds算子投影节点嵌入到固定子空间，消除非平凡对称感知信息，限制GNN表达能力。
- 方法要点：引入投影器不变读出，将节点表示分解为对称感知通道，并用非线性不变统计量汇总，保持置换不变性同时保留信息。
- 实验或效果：仅替换读出即可使固定编码器分离WL-hard图对，并在多个基准测试中提升性能，证明读出设计对GNN表达能力至关重要。

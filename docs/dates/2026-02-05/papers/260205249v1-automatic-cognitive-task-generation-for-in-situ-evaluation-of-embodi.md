...
**一句话要点**：提出动态原位任务生成方法TEA，以解决未知3D环境中具身智能体评估的不足。

**关键词**：具身智能体评估, 动态任务生成, 原位评估, 认知任务, 3D环境交互, 任务图建模

## 3 点简述
- 核心问题：现有基准存在数据污染和缺乏场景特异性，无法有效评估智能体在未知环境中的能力。
- 方法要点：通过结构化图表示定义任务，构建交互-演化两阶段系统，实现任务执行与生成的循环及任务重组。
- 实验或效果：在10个未知场景中自动生成87,876个任务，验证显示任务物理合理且覆盖日常认知能力，模型评估揭示其在感知和推理上的缺陷。

...
**一句话要点**：提出模式依赖校正以稳定PPO训练，解决模式依赖层导致的不稳定问题。

**关键词**：强化学习, PPO训练, 模式依赖层, 批归一化, 稳定性优化, 双阶段训练

## 3 点简述
- 核心问题：批归一化等模式依赖层在PPO中引发训练与评估行为差异，导致策略失配和奖励崩溃。
- 方法要点：提出轻量级双阶段训练过程MDR，无需架构修改，稳定模式依赖层下的PPO优化。
- 实验或效果：在程序生成游戏和真实世界任务中验证MDR提升稳定性和性能，可扩展至其他模式依赖层。

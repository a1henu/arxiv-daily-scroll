...
**一句话要点**：提出CORAL方法，通过正则化推理时引导优化大语言模型的正确性和校准性。

**关键词**：推理时引导, 模型校准, 激活探针, 正则化优化, 迁移学习, 多选问答

## 3 点简述
- 大语言模型在指令微调和偏好对齐后存在校准偏差，训练优化成本高。
- CORAL使用权重衰减MLP探针从模型内部激活提取分布式正确信号进行正则化引导。
- 在三个7B参数模型上平均提升准确率10%和校准误差50%，并展示跨基准的迁移性。

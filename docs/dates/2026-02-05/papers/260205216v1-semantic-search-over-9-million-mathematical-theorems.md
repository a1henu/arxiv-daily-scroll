...
**一句话要点**：提出基于语义检索的方法，在920万定理语料上实现高效数学定理搜索，提升定理与论文级检索效果。

**关键词**：语义检索, 数学定理搜索, 大规模语料, 自然语言表示, 嵌入模型, 检索评估

## 3 点简述
- 核心问题：现有工具检索整篇论文，难以满足对特定定理、引理或命题的精确搜索需求。
- 方法要点：使用短自然语言描述表示定理，系统分析表示上下文、语言模型、嵌入模型和提示策略对检索质量的影响。
- 实验或效果：在专业数学家编写的查询集上，相比基线方法，显著提高了定理级和论文级检索性能。

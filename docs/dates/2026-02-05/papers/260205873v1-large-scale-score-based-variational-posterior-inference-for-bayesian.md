...
**关键词**：贝叶斯神经网络, 变分推理, 分数匹配, 可扩展学习, 深度网络不确定性

## 3 点简述
- 核心问题：传统变分推理方法难以扩展到大规模贝叶斯深度神经网络，存在计算和技术限制。
- 方法要点：结合分数匹配损失和近端惩罚项，避免重参数化采样，支持随机梯度噪声，实现可扩展性。
- 实验或效果：在视觉识别和时间序列预测等基准测试中，验证了方法在大规模网络（如Vision Transformers）上的有效性。

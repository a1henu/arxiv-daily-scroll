...
**一句话要点**：提出学习条件依赖源分布以提升条件流匹配在文本到图像生成中的性能

**关键词**：流匹配, 文本到图像生成, 条件依赖源分布, 方差正则化, 方向对齐, 分布优化

## 3 点简述
- 核心问题：现有流匹配方法多采用标准高斯分布作为源分布，未充分利用条件信号优化源分布。
- 方法要点：在流匹配目标下学习条件依赖源分布，通过方差正则化和方向对齐解决分布崩溃与不稳定问题。
- 实验或效果：在多个文本到图像基准测试中实现稳定改进，包括FID收敛速度提升高达3倍。

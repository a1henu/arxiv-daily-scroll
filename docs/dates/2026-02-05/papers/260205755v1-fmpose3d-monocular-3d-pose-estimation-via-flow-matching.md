...
**一句话要点**：提出FMPose3D，通过流匹配实现单目3D姿态估计，以解决扩散模型推理效率低的问题。

**关键词**：单目3D姿态估计, 流匹配, 概率生成模型, ODE轨迹, 假设聚合, 动物姿态估计

## 3 点简述
- 核心问题：单目3D姿态估计因深度模糊和遮挡而病态，需生成多假设但现有扩散模型推理慢。
- 方法要点：利用流匹配学习ODE定义的流速场，以少量积分步高效生成3D姿态样本，并引入RPEA模块聚合假设。
- 实验或效果：在Human3.6M、MPI-INF-3DHP、Animal3D和CtrlAni3D数据集上超越现有方法，实现SOTA性能。

...
**关键词**：小目标检测, 深度先验, 损失加权, 置信度阈值, 无架构修改, 多基准验证

## 3 点简述
- 核心问题：小目标检测因尺度变化、低分辨率和背景干扰而性能下降，深度信息可改善但现有方法需复杂架构修改。
- 方法要点：DepthPrior通过深度加权损失、分层损失和深度感知置信度阈值，将深度作为训练和推理的先验知识。
- 实验或效果：在多个基准和检测器上验证，小目标mAP提升达9%，推理恢复率高达95:1，无额外传感器或性能成本。

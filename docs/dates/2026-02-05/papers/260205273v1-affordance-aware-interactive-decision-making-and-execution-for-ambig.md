...
**一句话要点**：提出AIDE框架以解决机器人执行模糊指令时交互推理与执行效率低的问题。

**关键词**：机器人决策, 视觉语言模型, 交互探索, 可供性分析, 模糊指令执行

## 3 点简述
- 核心问题：现有VLM方法在模糊指令下识别任务相关对象时，推理效率低且缺乏环境交互，阻碍实时任务规划。
- 方法要点：AIDE采用双流框架，集成交互探索与视觉语言推理，通过多阶段推理和加速决策实现零样本可供性分析。

...
**一句话要点**：提出TACO端到端压缩模型以解决表格基础模型在大型数据集上的效率瓶颈。

**关键词**：表格数据压缩, 端到端学习, 潜在空间表示, 推理效率优化, TabArena基准

## 3 点简述
- 表格基础模型基于注意力机制，数据集规模导致二次复杂度，限制大规模应用。
- TACO在潜在空间压缩训练数据，实现端到端压缩，减少内存和推理时间。
- 在TabArena基准测试中，推理速度提升高达94倍，内存减少97%，性能无明显下降。

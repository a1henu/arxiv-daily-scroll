...
**一句话要点**：提出ShapeUP框架，通过图像条件化监督学习实现可扩展的3D编辑

**关键词**：3D编辑, 图像条件化, 监督学习, 扩散变换器, 潜在映射, 可扩展框架

## 3 点简述
- 核心问题：现有3D编辑方法在视觉可控性、几何一致性和可扩展性间存在权衡，如优化慢、视觉漂移或受限于冻结先验。
- 方法要点：基于预训练3D基础模型，使用3D扩散变换器学习从源3D形状和编辑图像到编辑3D形状的潜在到潜在映射。
- 实验或效果：在身份保持和编辑保真度上优于当前基线，支持细粒度视觉控制和隐式定位，保持结构一致性。

...
**一句话要点**：提出ShapeUP框架，通过图像条件化监督学习实现可扩展的3D编辑，解决视觉可控性与几何一致性的权衡问题。

**关键词**：3D编辑, 图像条件化, 监督学习, 扩散变换器, 几何一致性, 可扩展性

## 3 点简述
- 核心问题：现有3D编辑方法在视觉可控性、几何一致性和可扩展性之间存在难以平衡的权衡，如优化方法慢、多视图传播有视觉漂移、无训练方法受限于固定先验。
- 方法要点：基于预训练3D基础模型，使用图像作为提示，通过监督训练学习从源3D形状到编辑后3D形状的潜在到潜在映射，采用3D扩散变换器实现精细视觉控制和隐式定位。
- 实验或效果：在身份保持和编辑保真度方面优于当前训练和无训练基线，提供稳健且可扩展的原生3D内容创建范例。

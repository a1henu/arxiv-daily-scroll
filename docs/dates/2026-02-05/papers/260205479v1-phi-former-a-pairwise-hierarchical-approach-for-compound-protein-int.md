...
**一句话要点**：提出Phi-Former，通过成对分层表示学习预测化合物-蛋白质相互作用，以解决现有原子级模型与化学现实不符的问题。

**关键词**：化合物-蛋白质相互作用预测, 分层表示学习, 成对预训练, 分子基序, 可解释性模型, 药物发现

## 3 点简述
- 核心问题：现有原子级CPI预测模型未充分考虑分子片段（基序）在生物识别中的关键作用，导致与化学现实不符。
- 方法要点：采用分层表示（原子、基序）和成对预训练框架，系统建模原子-原子、基序-基序和原子-基序的相互作用。
- 实验或效果：在CPI相关任务上表现优异，案例研究显示能准确识别激活的原子或基序，提供可解释性。

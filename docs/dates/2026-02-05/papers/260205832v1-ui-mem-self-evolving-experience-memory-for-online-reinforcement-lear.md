...
**一句话要点**：提出UI-Mem框架，通过分层经验记忆解决移动GUI在线强化学习中的信用分配和跨任务经验迁移问题。

**关键词**：在线强化学习, 移动GUI代理, 经验记忆, 分层采样, 跨任务迁移, 自进化学习

## 3 点简述
- 核心问题：在线强化学习在长视野任务中信用分配效率低，且缺乏跨任务经验迁移导致重复错误。
- 方法要点：引入分层经验记忆存储结构化知识，采用分层组采样注入指导，并设计自进化循环更新记忆。
- 实验或效果：在在线GUI基准测试中显著优于传统基线，并展示了对未见应用的强泛化能力。

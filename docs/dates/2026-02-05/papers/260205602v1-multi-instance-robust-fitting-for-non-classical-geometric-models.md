...
**关键词**：非经典几何模型, 多实例拟合, 鲁棒估计, 元启发式优化, 模型-数据误差, 全局优化

## 3 点简述
- 核心问题：现有鲁棒拟合方法主要针对经典模型，且多专注于单实例重建，难以处理非经典模型的多实例拟合。
- 方法要点：设计无预定义误差阈值的模型-数据误差估计器，结合元启发式算法优化，实现全局最优搜索。
- 实验或效果：在多种非经典模型上验证了方法的有效性，代码已开源。

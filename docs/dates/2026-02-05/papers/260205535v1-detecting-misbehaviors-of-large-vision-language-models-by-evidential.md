...
**一句话要点**：提出证据不确定性量化方法以检测大视觉语言模型的不当行为

**关键词**：大视觉语言模型, 不确定性量化, 证据理论, 模型安全, 幻觉检测, 对抗性输入

## 3 点简述
- 核心问题：大视觉语言模型面对不适当输入时产生幻觉或有害内容，源于认知不确定性。
- 方法要点：基于证据理论，从模型输出特征量化内部冲突与知识缺失，实现细粒度不确定性评估。
- 实验或效果：在幻觉、越狱等四类不当行为检测中优于基线，并分析层间不确定性动态。

...
**一句话要点**：提出SynthStrategy方法，利用大语言模型将合成策略知识转化为代码，以解决计算机辅助合成规划中的战略考量不足问题。

**关键词**：计算机辅助合成规划, 大语言模型, 合成策略提取, 知识蒸馏, 路线检索, 化学信息学

## 3 点简述
- 核心问题：计算机辅助合成规划系统在生成化学有效反应步骤时，难以整合收敛组装、保护基最小化等战略考量。
- 方法要点：通过大语言模型分析合成路线，将战略原则转化为Python函数，实现知识的可验证代码化表示。
- 实验或效果：在基准测试中实现75%的Top-3准确率，支持基于自然语言的路线检索和更精细的路线聚类。

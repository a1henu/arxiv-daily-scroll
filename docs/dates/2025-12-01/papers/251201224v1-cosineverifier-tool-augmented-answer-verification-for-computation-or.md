...
**一句话要点**：提出CoSineVerifier工具增强验证器，以解决计算导向科学问题中的答案验证挑战。

**关键词**：答案验证, 工具增强, 计算导向科学, 强化学习, 符号简化, STEM评估

## 3 点简述
- 核心问题：计算导向科学领域如代数等价检查和物理常数替换的答案验证存在困难。
- 方法要点：采用工具增强方法，结合外部执行器进行精确计算和符号简化。
- 实验或效果：在STEM、通用QA和长推理任务上表现优异，在VerifyBench-Hard和SCI-Bench达到SOTA。
